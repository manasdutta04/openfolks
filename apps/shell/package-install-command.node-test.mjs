...
test("the Ubuntu command resolves dependencies", () => {
  const command = packageInstallCommand("deb", "/home/u/.cache/openfolks-updater/pending/x.deb");

  assert.equal(
    command,
    "sudo apt-get install -y '/home/u/.cache/openfolks-updater/pending/x.deb'",
  );
...
test("a staged path that is gone is not used", () => {
  ...
  try {
    const present = join(workspace, "OpenFolks.deb");
...
test("the whole command parses into the arguments apt-get would receive", { skip: posixShell }, () => {
  const file = "/home/o'brien/.cache/openfolks-updater/pending/OpenFolks-0.1.44-amd64.deb";
...
test("the package marker decides the install path", () => {
  ...
  assert.equal(
    linuxPackageType({
      ...
      appImage: "/home/u/OpenFolks.AppImage",
