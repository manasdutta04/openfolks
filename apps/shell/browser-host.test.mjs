...
function harness() {
  ...
  const manager = {
    size: () => 1,
    isHumanControlled: (botId, profile) => held && botId === "folk-a" && profile === "work",
...
async function register(host, botId = "folk-a", profile = "work", token) {
...
async function request(host, operation, { botId = "folk-a", profile = "work", token, body = {} } = {}) {
...
describe("browser loopback host", () => {
  ...
  it("atomically replaces an earlier capability for the same folk", async () => {
    const { host, calls } = harness();
    await host.start();
    const oldToken = await register(host, "folk-a", "work");
    const nextToken = await register(host, "folk-a", "personal");
    ...
    expect(calls).toContainEqual(["cancelAgentActions", "folk-a"]);
  });

  it("accepts only the capability scoped to the exact route folk and body profile", async () => {
    const { host, calls } = harness();
    await host.start();
    ...
    expect(calls).toContainEqual(["state", "folk-a", "work"]);

    expect((await request(host, "state", { token: MASTER })).response.status).toBe(401);
    const workToken = await register(host, "folk-a", "work");
    expect((await request(host, "state", { botId: "folk-b", token: workToken })).response.status).toBe(401);
    expect((await request(host, "state", { profile: "personal", token: workToken })).response.status).toBe(401);

    const missingProfile = await fetch(`${host.url}/v1/bots/folk-a/state`, {
      method: "POST",
  ...
  it("decodes JSON only after joining split UTF-8 bytes", async () => {
    ...
    const scoped = await register(host, "folk-a", profile);

    const result = await new Promise((resolve, reject) => {
      const target = new URL(`${host.url}/v1/bots/folk-a/navigate`);
    ...
    expect(calls).toContainEqual(["navigate", "folk-a", "https://example.com/search?q=maus🐭", "work"]);
