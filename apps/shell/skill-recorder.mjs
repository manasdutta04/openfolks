...
const BUNDLE = app.isPackaged
  ? path.join(process.resourcesPath, "OpenFolks Recorder.app")
  : recorderHelperBundle;
...
export function startRecorder(win) {
  ...
  ensureBuilt();
  const sessionDir = mkdtempSync(path.join(app.getPath("temp"), "openfolks-recorder-"));
...
export function saveSkillRecording(payload, options = {}) {
  ...
  const dataRoot = options.dataRoot ?? process.env.OPENFOLKS_DATA_DIR ?? process.env.OCREW_DATA_DIR ?? process.env.OMB_DATA_DIR ?? path.join(os.homedir(), ".openfolks");
