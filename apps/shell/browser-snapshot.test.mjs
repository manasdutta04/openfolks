...
describe("browser snapshot", () => {
  ...
  it("presents as the Chrome it is", () => {
    expect(
      browserUserAgent("Mozilla/5.0 (Macintosh) AppleWebKit/537.36 (KHTML, like Gecko) OpenFolks/0.1.38 Chrome/140.0.0.0 Electron/43.4.0 Safari/537.36"),
    ).toBe("Mozilla/5.0 (Macintosh) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36");
  });

  it("derives one durable partition per folk from safe characters only", () => {
    expect(browserPartition("bot_1-A")).toBe("persist:openfolks-browser-bot_1-A");
    expect(browserPartition("../../evil")).toBe("persist:openfolks-browser-evil");
    expect(() => browserPartition("")).toThrow();
    expect(() => browserPartition("../")).toThrow();
  });

  it("maps exact canonical and migrated profile partition ids without normalization", () => {
    expect(browserProfilePartition("work-2")).toBe("persist:openfolks-browser-profile-work-2");
    expect(browserProfilePartition("Work-2")).toBe("persist:openfolks-browser-profile-Work-2");
