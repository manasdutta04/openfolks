...
describe("packaged browser connection transport", () => {
  ...
  it("posts the in-memory descriptor or an explicit unavailable marker", () => {
    ...
    expect(proc.postMessage).toHaveBeenNthCalledWith(1, { type: "openfolks:browser-connection", connection });
    expect(proc.postMessage).toHaveBeenNthCalledWith(2, { type: "openfolks:browser-connection", connection: null });
  });
});
