...
before(() => {
  ...
  botHome = path.join(home, ".openfolks");
  fs.mkdirSync(path.join(botHome, "workspaces", "folk"), { recursive: true });
  fs.writeFileSync(path.join(botHome, "workspaces", "folk", "report.docx"), "docx");
...
describe("save-file path validation", () => {
  it("accepts a file inside the folk home, as a path or a file:// URL", async () => {
    const file = path.join(botHome, "workspaces", "folk", "report.docx");
  ...
  it("accepts a file under a symlinked folk home", { skip: !canSymlink }, async () => {
    ...
    const realBotHome = path.join(realHome, "folk-data");
    fs.mkdirSync(realBotHome, { recursive: true });
    fs.writeFileSync(path.join(realBotHome, "report.docx"), "docx");
    fs.symlinkSync(realBotHome, path.join(linkedHome, ".openfolks"));

    const viaLink = path.join(linkedHome, ".openfolks", "report.docx");
  ...
  it("rejects paths outside the folk home, including via traversal", async () => {
    const rejected = "Only files created by your folks can be saved";
  ...
  it("rejects a symlink inside the folk home pointing outside it", { skip: !canSymlink }, async () => {
    ...
    await assert.rejects(resolveSavablePath(escape, { home }), {
      message: "Only files created by your folks can be saved",
    });
    fs.rmSync(escape);
  });
...
describe("save-file dialog default name", () => {
  it("suggests a name that does not overwrite an existing file", async () => {
    ...
    const source = path.join(botHome, "workspaces", "folk", "report.docx");
