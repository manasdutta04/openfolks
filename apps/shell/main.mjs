import { app, BrowserWindow, WebContentsView, clipboard, desktopCapturer, dialog, ipcMain, Menu, nativeImage, safeStorage, screen, session, shell, systemPreferences, utilityProcess } from "electron";
...
import { installAppMenu, popupApplicationSubmenu } from "./app-menu.mjs";
...
// Packaged builds: your public Composio broker. Override with OMB_COMPOSIO_BROKER_URL.
const DEFAULT_COMPOSIO_BROKER_URL = "https://openfolks-composio.manasdutta512.workers.dev";
let SERVER_PORT = 8799;
// Windows taskbar/window chrome prefers .ico; PNG is fine on macOS/Linux.
const APP_ICON = process.platform === "win32"
  ? path.join(__dirname, "../../build/icon.ico")
  : path.join(__dirname, "resources/app-icon.png");
const APP_ICON_FALLBACK = path.join(__dirname, "../../public/openfolk.png");
const APP_ICON_IMAGE = () => {
...
// `pnpm dev:desktop` on Windows launches a renamed OpenFolks.exe so the
// taskbar can show our mark. Electron then reports isPackaged. The launcher
// sets this flag so we still load Vite and skip the embedded harness.
const DEV_DESKTOP = process.env.OPENFOLKS_DEV_DESKTOP === "1";
function packagedApp() {
  return app.isPackaged && !DEV_DESKTOP;
}
...
function applyUnreadBadge(win = mainWindow) {
  ...
  if (process.platform === "win32") {
    if (!win || win.isDestroyed()) return;
    if (!unreadOverlayIcon) {
      const base = APP_ICON_IMAGE();
      unreadOverlayIcon = base ? base.resize({ width: 16, height: 16 }) : nativeImage.createEmpty();
    }
    win.setOverlayIcon(
...
if (process.platform === "linux") {
  ...
  app.setDesktopName("com.openfolks.app.desktop");
}

// Windows groups the taskbar entry by AppUserModelID. Without this, unpackaged
// `electron .` stays under Electron's identity (and its atom icon).
if (process.platform === "win32") {
  app.setAppUserModelId("com.openfolks.app");
}
...
if (!app.requestSingleInstanceLock()) {
  console.log("[desktop] OpenFolks is already running — focusing that window");
  process.exit(0);
}
...
async function secureComposioConfig() {
  const dataDir = process.env.OPENFOLKS_DATA_DIR || process.env.OCREW_DATA_DIR || process.env.OMB_DATA_DIR || path.join(app.getPath("home"), ".openfolks");
...
function composioBrokerUrl() {
  ...
  return normalizeManagedComposioBrokerUrl(
    configured || (packagedApp() ? DEFAULT_COMPOSIO_BROKER_URL : ""),
  );
}

// The packaged app has no terminal: everything about the server child's life
// goes to server.log in the OS log dir (~/Library/Logs/OpenFolks on macOS,
// Console.app-visible; %APPDATA%\OpenFolks\logs on Windows), which is also
 ...
 * writer shares this state so concurrent saves cannot overwrite each other. */
...
function syncManagedComposioCredentials() {
  ...
  try {
    serverProc.postMessage({
      type: "openfolks:managed-composio",
...
function buildErrorPage({ allPortsOccupied }) {
  ...
  const reason = allPortsOccupied
    ? "Every OpenFolks port answered health checks from another process — likely a second copy of the app, or another program on ports 8799–28799. Quit that program, then quit and reopen OpenFolks."
    : "The background server didn't come up in time — this is usually slow startup, not a port conflict. Quit and reopen OpenFolks.";
  return (
    "data:text/html;charset=utf-8," +
    encodeURIComponent(
      `<body style="margin:0;display:flex;align-items:center;justify-content:center;height:100vh;background:#070707;color:#fcfcfc;font:15px -apple-system,system-ui"><div style="text-align:center;max-width:360px"><div style="font-size:40px">🐭</div><h2 style="font-weight:600;margin:12px 0 6px">Couldn't start the folk server</h2><p style="color:#fcfcfc99;line-height:1.5">${escapeHtml(reason)} If it keeps happening, check <a target="_blank" rel="noopener" href="${serverLogHref}" style="color:#fcfcfc">${escapeHtml(serverLogPath)}</a>.</p></div></body>`,
    )
  );
}
...
function rendererOrigin() {
  return new URL(packagedApp() ? `http://127.0.0.1:${SERVER_PORT}` : DEV_URL).origin;
}
...
function openDesktopViewer(owner, rawUrl, rawTitle, contextId) {
  if (!owner || owner.isDestroyed()) throw new Error("The OpenFolks window is unavailable");
  ...
  const viewer = new BrowserWindow({
    ...
    webPreferences: {
      ...
      partition: "openfolks-desktop-viewer",
    },
  });
...
function ensureDesktopWorkspace(owner) {
  ...
  const manager = createDesktopWorkspaceManager({
    ...
    partitionPrefix: `openfolks-desktop-workspace-${randomUUID()}`,
    notify: (state) => {
...
/** The built-in browser: WebContentsViews per folk inside the app window,
 * plus the loopback host the folk's tools call. The host and its in-memory
...
async function ensureBrowserHost() {
  ...
  try {
    ...
    if (packagedApp()) removeBrowserConnectionDescriptor();
...
ipcMain.handle("browser:set-human-control", (event, botId, held, profile) => {
  ...
  if (!/^[A-Za-z0-9_-]{1,120}$/.test(id)) throw new Error("A folk id is required");
...
function createWindow() {
  ...
  const win = new BrowserWindow({
    ...
    icon: APP_ICON_IMAGE() ?? APP_ICON,
    backgroundColor: "#070707",
    // Windows uses a custom title strip (AppTitleBar); keep the native menu
    // installed for accelerators but never show the extra menu-bar row.
    autoHideMenuBar: true,
  ...
  if (process.platform === "win32") {
    ...
    // Re-apply after construct — some Windows builds keep electron.exe's icon
    // on the window until setIcon runs with a real .ico.
    const icon = APP_ICON_IMAGE();
    if (icon) win.setIcon(icon);
    win.setAppDetails({
      appId: "com.openfolks.app",
      appIconPath: fs.existsSync(APP_ICON) ? APP_ICON : APP_ICON_FALLBACK,
      relaunchDisplayName: "OpenFolks",
    });
  }
...
// Folks hand users files as markdown links to paths inside the OpenFolks
...
// renderer-controlled, so it must resolve inside ~/.openfolks and be a
...
// The renderer owns the skin. On Windows the caption overlay must match so
// brand + menus in the title strip share one row with min/max/close.
ipcMain.handle("desktop:skin", (event, skin) => {
  ...
  if (win && !win.isDestroyed() && process.platform === "win32") {
    ...
    try {
      ...
      // Older Electron builds without overlay support still paint the page.
    }
...
ipcMain.handle("menu:popup", (event, label, x, y) => {
  const win = BrowserWindow.fromWebContents(event.sender);
  if (!win || win.isDestroyed()) return false;
  if (typeof label !== "string" || typeof x !== "number" || typeof y !== "number") return false;
  return popupApplicationSubmenu(label, { browserWindow: win, x, y });
});
...
// live desktop inside OpenFolks instead of sending the person to a browser.
...
// "Hand control back" would close (and release) another folk's viewer.
...
ipcMain.handle("credential:set", async (_event, name, value) => {
  ...
  if (packagedApp() && !(await safeStorage.isAsyncEncryptionAvailable())) {
    throw new Error("The operating-system credential store is unavailable");
  }
  const secret = value.trim();
  const applyToHarness = async () => {
    ...
    const secretStorage = packagedApp() ? "?secretStorage=external" : "";
  ...
  if (!packagedApp()) return applyToHarness();
...
app.whenReady().then(async () => {
  if (packagedApp()) app.setAsDefaultProtocolClient("openfolks");
  if (process.platform === "darwin") {
    const dockIcon = APP_ICON_IMAGE();
  ...
  if (packagedApp() && composioBrokerUrl() && !credentialStoreUnavailable) {
