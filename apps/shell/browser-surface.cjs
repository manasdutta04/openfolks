...
//
// Profiles: a folk has one view per profile it has used — its own private
...
// native modals), and only http(s) navigations honoured. A folk's browser can
...
function botIdOf(value) {
  ...
  if (!BOT_ID.test(id)) throw new Error("A folk id is required");
  return id;
}
 ...
 * only to remove protected flat-tree text before it reaches a folk. */
...
function createBrowserSurfaceManager({
   ...
   * profiles share a session across views, so this must not belong to a folk. */
  const sessionSecurity = new WeakMap();
  /** every live view, keyed by `${botId}\0${partition}` */
  const entries = new Map();
  /** the view a folk currently shows / acts on */
  const active = new Map();
  /** Human control is folk-wide, matching the harness control endpoint. A
   * stale process scoped to another profile must not see around takeover. */
  const botControl = new Map();
  /** A live per-turn capability pins its exact folk/profile view. Hidden
  ...
  const partitionForProfile = (botId, profile) => {
    if (profile === GUEST_PROFILE) return `openfolks-browser-guest-${botId}-${++guestCounter}`;
   ...
   * and left the folk locked out of a browser nobody had taken. */
  ...
  const evictIfNeeded = () => {
    ...
    const candidates = [...entries.values()]
      // `active` means "this folk's selected profile", not "on screen". A
      // workspace with nine folks therefore has nine active-but-hidden views.
    ...
    if (!victim) throw new Error(`Only ${maxViews} folk browsers can be open at once`);
  ...
  const installSessionPolicy = (entry) => {
    ...
    // A download would land on the user's disk under a folk's control; refuse
  ...
  const secure = (entry) => {
    ...
    contents.setWindowOpenHandler(({ url, postBody }) => {
      // target=_blank links stay in this folk's one tab: a second window would
  ...
  /** Things the page does on its own that a folk must hear about. */
  const onProtocolEvent = (entry, method, params) => {
    if (method === "Page.javascriptDialogOpening") {
      // alert/confirm/prompt would otherwise be a native modal over the app
      // window that nobody can answer for the folk. Alerts are harmless to
  ...
  const create = (botId, profile) => {
    evictIfNeeded();
    if (owner.isDestroyed?.()) throw new Error("The OpenFolks window is unavailable");
    ...
    const view = createView({
      webPreferences: {
        ...
        // page, which claimed the control lease and locked the folk out of
        // its own browser. The folk's tab must never steal focus from the
  ...
  /** The view a folk should be looking at: `undefined` keeps whatever is
   * active (callers that don't know the profile never evict a tab); "" is
   * the folk's own session; "guest" a throwaway; anything else a named
   ...
   * the wrong post-action document. Different profiles and folks still run in
   * parallel. */
  ...
  const ensureProtocol = async (entry) => {
    ...
    const ready = (async () => {
      ...
      // Never show a native file picker for a folk. Unlike focus emulation,
  ...
  const ensureIsolatedContext = async (entry) => {
    ...
    const ready = (async () => {
      ...
      const { executionContextId } = await cdp(entry, "Page.createIsolatedWorld", {
        frameId,
        worldName: "openfolks-browser-snapshot",
  ...
  const neutralizeAgentInput = async (entry) => {
    ...
    const pending = (async () => {
      ...
      const release = async (method, params) => {
        ...
        // person taking control and leave the folk stuck behind a false hold.
   ...
   * loading must not hang the folk). */
  ...
  const scrollHint = async (entry) => {
    try {
      ...
      // saying otherwise sent folks into scroll/read loops that re-fetched the
  ...
  const api = {
    /** Create or switch the folk's view; hidden until laid out. */
     ...
     * and local, while scoped folk capabilities get this inspected form. */
    ...
    /** Position the folk's active view over the renderer's rectangle (or hide
    ...
    layout(botId, bounds, profile, mode, layoutOwner) {
      ...
      if (bounds === null || bounds === undefined) {
        ...
        // the newer native surface for the same folk and profile.
      ...
      if (!entry.visible) {
        ...
        // switch, workspace overlay, another folk). Re-adding raises this
     ...
     * simply settles — bounded, so a folk never hangs on a page that stalls. */
     ...
     * folk. The loopback host calls this before revoking a turn capability,
    ...
    /** Drop every folk's view on a named profile — before its data is cleared. */
    ...
    /** Drop every view a folk has (all profiles). */
    close(botId) {
      const id = botIdOf(botId);
