...
//
// A folk's tools run inside its agent CLI, which the harness spawned — two
...
// renderer, never a debugging port on OpenFolks itself. The manager is
...
function createBrowserHost({ manager, token = randomBytes(32).toString("hex"), now = Date.now }) {
  ...
  const manageCapability = async (operation, req, res) => {
    ...
    if (!BOT_ID.test(botId) || !PROFILE_PARTITION_ID.test(profile)) {
      return json(res, 400, { error: "a valid folk and browser profile are required" });
    }
  ...
  const handle = async (req, res) => {
    ...
    if (!surface) return json(res, 503, { error: "the OpenFolks window is closed — open it to use the browser" });
    ...
    try {
      ...
      // Stale refs, refused navigations and timeouts are the folk's to correct;
