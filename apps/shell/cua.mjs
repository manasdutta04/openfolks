...
//    EmbeddedCuaDriverHost so TCC grants attribute to OpenFolks and the
//    driver inherits them. One prompt, named OpenFolks, out of the box.
...
const HOST_BUNDLE_ID = "com.openfolks.app";
...
async function loadEmbeddedSdk() {
  ...
  process.env.OPENFOLKS_CUA_SDK_LIBRARY = path.join(
...
async function attachStandalone() {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed OpenFolks.
...
async function startEmbedded(binary) {
  ...
  // OpenFolks rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart OpenFolks`);
  }
...
export async function startCua() {
  ...
  const wantEmbedded =
    app.isPackaged || process.env.OPENFOLKS_CUA_EMBEDDED === "1";
  let nextConnection;
