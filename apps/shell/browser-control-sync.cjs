...
function applyBrowserControlHold(message, take) {
  ...
  if (message.type !== "openfolks:browser-control") return false;
...
function decodeBrowserLifecycleMessage(message) {
  ...
  if (message.type === "openfolks:browser-bot-deleted") {
  ...
  if (message.type === "openfolks:browser-profile-deleted") {
...
function browserLifecycleResult(requestId, ok) {
  ...
  return { type: "openfolks:browser-lifecycle-result", requestId: id, ok: ok === true };
}
