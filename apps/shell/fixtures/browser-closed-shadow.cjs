...
async function run() {
  ...
  try {
    manager.ensure("fixture-folk", "");
    manager.layout("fixture-folk", { x: 20, y: 30, width: 400, height: 250 }, "", "compact");
    ...
    try {
      await manager.read("fixture-folk", "");
    } catch (error) {
    ...
    const protectedSnapshot = await manager.snapshot("fixture-folk", "");
    ...
    let refused = false;
    try {
      await manager.screenshot("fixture-folk", "");
    } catch (error) {
    ...
    const safeClosedSnapshot = await manager.snapshot("fixture-folk", "");
    ...
    manager.setHumanControl("fixture-folk", true, "");
    ...
    manager.setHumanControl("fixture-folk", false, "");
    ...
    const taintedSnapshot = await manager.snapshot("fixture-folk", "");
    ...
    const richSnapshot = await manager.snapshot("fixture-folk", "");
    ...
    const actionSnapshot = await manager.snapshot("fixture-folk", "");
    ...
    await manager.click("fixture-folk", reviewedRef, { profile: "" });
    ...
    await manager.scroll("fixture-folk", "down", 600, "");
    await new Promise((resolve) => setTimeout(resolve, 50));
    ...
    const compactShot = await manager.screenshot("fixture-folk", "");
    ...
    manager.layout("fixture-folk", { x: 10, y: 20, width: 820, height: 600 }, "", "expanded");
    await waitForFixedViewport(browserView, "expanded action");
    const expandedSnapshot = await manager.snapshot("fixture-folk", "");
    ...
    await manager.click("fixture-folk", expandedRef, { clickCount: 2, profile: "" });
    ...
    await manager.scroll("fixture-folk", "down", 300, "");
    await new Promise((resolve) => setTimeout(resolve, 50));
    ...
    const expandedShot = await manager.screenshot("fixture-folk", "");
    ...
    for (const [selector, key] of [["#empty-password", "Enter"], ["#empty-secret-editor", "Backspace"]]) {
      ...
      try {
        await manager.press("fixture-folk", key, "");
      } catch (error) {
    ...
    let overlayError = "";
    try {
      await manager.click("fixture-folk", reviewedRef);
    } catch (error) {
    ...
    const relabelSnapshot = await manager.snapshot("fixture-folk", "");
    ...
    try {
      await manager.click("fixture-folk", relabelRef);
    } catch (error) {
