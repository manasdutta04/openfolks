...
// the cloud box's CDP helper (server/remote-computer.ts) so a folk that learned
...
function browserPartition(botId) {
  ...
  if (!safe) throw new Error("A folk id is required");
  return `persist:openfolks-browser-${safe}`;
}
...
function browserProfilePartition(partitionId) {
  ...
  return `persist:openfolks-browser-profile-${id}`;
}
