...
function fakeView(partition) {
  ...
  const webContents = {
    session: {
      getUserAgent: () => "Mozilla/5.0 Chrome/1 Electron/43 OpenFolks/1",
...
describe("browser surface manager", () => {
  it("creates one sandboxed, partitioned view per folk only when something needs it", () => {
    const { manager, owner, views } = harness();
    expect(manager.layout("folk-a", null)).toMatchObject({ botId: "folk-a", open: false });
    expect(views).toHaveLength(0);

    // a view that exists but is not laid out still has a real viewport
    manager.ensure("folk-z", "");
    expect(views[0].bounds).toEqual({ x: 0, y: 0, width: VIEWPORT.width, height: VIEWPORT.height });
    expect(views[0].visible).toBe(false);
    manager.close("folk-z");
    views.length = 0;

    const state = manager.layout("folk-a", { x: 20.4, y: 30.6, width: 5000, height: 300 }, "", "compact");
    expect(views).toHaveLength(1);
    expect(views[0].partition).toBe("persist:openfolks-browser-folk-a");
    ...
    expect(state).toMatchObject({ botId: "folk-a", open: true, visible: true, url: "about:blank", profile: "", mode: "compact" });
  ...
  it("scales the same fixed desktop viewport to fit compact and expanded boxes", () => {
    ...
    manager.layout("folk-a", { x: 0, y: 0, width: 1100, height: 700 }, "", "expanded");
    ...
    expect(manager.state("folk-a").mode).toBe("expanded");
  });
  ...
  it("raises a hidden native view when its surface becomes visible again", () => {
    const { manager, owner, views } = harness();
    manager.layout("folk-a", BOUNDS, "", "compact");
    manager.layout("folk-a", null, "", "compact");
    manager.layout("folk-b", BOUNDS, "", "compact");
  ...
  it("moves without resetting emulation and reapplies it only when compact scale changes", () => {
    ...
    manager.layout("folk-a", { ...BOUNDS, x: 80, y: 90 }, "", "compact");
    ...
    manager.layout("folk-a", { ...BOUNDS, x: 80, y: 90, width: 320 }, "", "compact");
  ...
  it("reapplies expanded fixed-viewport emulation after a main-frame navigation commits", () => {
    const { manager, views } = harness();
    manager.layout("folk-a", { x: 10, y: 20, width: 960, height: 700 }, "", "expanded");
    const view = views[0];
    expect(view.calls.filter(([name]) => name === "enableDeviceEmulation")).toHaveLength(1);
  ...
  it("navigates only to web pages and answers with the page's elements plus a scroll hint", async () => {
    const { manager, views } = harness();
    await expect(manager.navigate("folk-a", "file:///etc/passwd")).rejects.toThrow(/http and https/);
    const page = await manager.navigate("folk-a", "example.com");
  ...
  it("reloads the active page without manufacturing a new history entry", async () => {
    const { manager, views } = harness();
    await manager.navigate("folk-a", "https://example.com");

    await manager.reload("folk-a", "", { source: "user" });
  ...
  it("blocks DNS-private redirects without replaying public redirects or form navigations as GETs", async () => {
    ...
    await expect(manager.navigate("folk-a", "https://private.example")).rejects.toThrow(/private-network/);
    await manager.navigate("folk-a", "https://public.example");
    ...
    const page = await manager.snapshot("folk-a");
  ...
  it("keeps native history traversal instead of canceling and appending a fresh load", async () => {
    const { manager, views } = harness();
    await manager.navigate("folk-a", "https://example.com/second");
    const loadsBefore = views[0].calls.filter(([name]) => name === "loadURL").length;
    await manager.back("folk-a", "");
  ...
  it("refuses POST popups instead of replaying their bodyless URL as a GET", async () => {
    ...
    expect((await manager.snapshot("folk-a")).notes).toContain(
  ...
  it("keeps one live view per profile and switches by showing another, never rebuilding", async () => {
    ...
    manager.layout("folk-a", BOUNDS, "", "compact");
    await manager.navigate("folk-a", "https://own.example");
    // switch to a named profile: a second view in the shared partition takes the same rectangle
    manager.layout("folk-a", BOUNDS, "work", "compact");
    expect(views).toHaveLength(2);
    expect(views[1].partition).toBe("persist:openfolks-browser-profile-work");
    expect(views[0].visible).toBe(false);
    expect(views[1].visible).toBe(true);
    expect(views[1].bounds).toEqual(BOUNDS);
    expect(manager.state("folk-a")).toMatchObject({ profile: "work", url: "about:blank" });
    await manager.navigate("folk-a", "https://work.example");
    expect(manager.state("folk-a", "")).toMatchObject({ profile: "", url: "https://own.example/" });
    expect(manager.state("folk-a", "work")).toMatchObject({ profile: "work", url: "https://work.example/" });
    ...
    // another folk on the same named profile shares the session, not the view
    manager.layout("folk-b", BOUNDS, "work", "compact");
    expect(views[2].partition).toBe("persist:openfolks-browser-profile-work");
    expect(views[1].calls.filter(([name, event]) => name === "sessionOn" && event === "will-download")).toHaveLength(1);
    expect(manager.list().filter((entry) => entry.active).map((entry) => entry.botId).sort()).toEqual(["folk-a", "folk-b"]);
    expect(states.some((state) => state.botId === "folk-a" && state.profile === "work")).toBe(true);
  ...
  it("ignores a stale profile-scoped hide after another profile is active", () => {
    ...
    const state = manager.layout("folk-a", null, "", "compact");

    expect(state).toMatchObject({ profile: "work", visible: true });
    expect(views[1].visible).toBe(true);
    manager.layout("folk-a", null, "work", "compact");
    expect(views[1].visible).toBe(false);
  });

  it("ignores cleanup from an older layout owner on the same profile", () => {
    const { manager, views } = harness();
    manager.layout("folk-a", BOUNDS, "", "compact", "compact-owner");
    manager.layout("folk-a", { ...BOUNDS, width: 800, height: 500 }, "", "expanded", "expanded-owner");

    const state = manager.layout("folk-a", null, "", "compact", "compact-owner");

    expect(state).toMatchObject({ profile: "", visible: true, mode: "expanded" });
    expect(views[0].visible).toBe(true);
    manager.layout("folk-a", null, "", "expanded", "expanded-owner");
    expect(views[0].visible).toBe(false);
  });
  ...
  it("reuses a cold own-profile view after the active shared profile is removed", async () => {
    ...
    expect(manager.state("folk-a").open).toBe(false);
    expect((await manager.snapshot("folk-a")).url).toBe("https://own.example/");
    expect(views).toHaveLength(2);
  });
  ...
  it("evicts the coldest view nobody is showing when the cap is reached", async () => {
    ...
    manager.layout("folk-a", BOUNDS, "", "compact");
    manager.layout("folk-a", BOUNDS, "p1", "compact");
    manager.layout("folk-a", BOUNDS, "p2", "compact");
    expect(manager.size()).toBe(3);
    // the fourth view evicts the least recently used inactive one (own session)
    manager.layout("folk-a", BOUNDS, "p3", "compact");
    expect(manager.size()).toBe(3);
    expect(views[0].calls.some(([name]) => name === "close")).toBe(true);
    expect(manager.list().map((entry) => entry.profile).sort()).toEqual(["p1", "p2", "p3"]);
  });

  it("evicts hidden idle views selected by other folks instead of locking out the next folk", () => {
    const { manager, views } = harness({ maxViews: 3 });
    manager.ensure("folk-a", "");
    manager.ensure("folk-b", "");
    manager.ensure("folk-c", "");
    expect(manager.size()).toBe(3);
    manager.ensure("folk-d", "");
    expect(manager.size()).toBe(3);
    expect(views[0].calls.some(([name]) => name === "close")).toBe(true);
    expect(manager.list().map(({ botId }) => botId).sort()).toEqual(["folk-b", "folk-c", "folk-d"]);
  });

  it("does not evict a hidden view while a live turn capability pins it", () => {
    const { manager, views } = harness({ maxViews: 2 });
    manager.ensure("folk-a", "");
    manager.setCapabilityActive("folk-a", "", true);
    manager.ensure("folk-b", "");
    manager.ensure("folk-c", "");
    expect(views[0].calls.some(([name]) => name === "close")).toBe(false);
    expect(views[1].calls.some(([name]) => name === "close")).toBe(true);
    expect(manager.list().map(({ botId }) => botId).sort()).toEqual(["folk-a", "folk-c"]);
  });
  ...
  it("never evicts a hidden view while an operation is still using it", async () => {
    ...
    const pending = manager.screenshot("folk-a", "");
    await started;
    expect(() => manager.ensure("folk-b", "")).toThrow(/Only 1 folk browser/);
    resume();
    await pending;
    expect(() => manager.ensure("folk-b", "")).not.toThrow();
    expect(manager.list().map(({ botId }) => botId)).toEqual(["folk-b"]);
  });
  ...
  it("serializes concurrent agent operations on the same browser entry", async () => {
    ...
    const first = manager.click("folk-a", "b11");
    await started;
    const second = manager.click("folk-a", "b11");
  ...
  it("clicks at the centre of a known ref and refuses stale or unknown ones", async () => {
    const { manager, views } = harness();
    await manager.navigate("folk-a", "https://example.com");
    await expect(manager.click("folk-a", "b99")).rejects.toThrow(/stale or unknown/);
    await expect(manager.click("folk-a", "nope")).rejects.toThrow(/stale or unknown/);
    await manager.click("folk-a", "b11");
    ...
    await expect(manager.click("folk-a", "b11")).rejects.toThrow(/changed since/);
  });
  ...
  it("maps fixed-viewport CSS coordinates into compact and expanded native surfaces", async () => {
    ...
    manager.layout("folk-a", { x: 0, y: 0, width: 800, height: 600 }, "", "expanded");
    await manager.click("folk-a", "b11");
    ...
    await manager.scroll("folk-a", "down", 600);
  ...
  it("emits a real double-click sequence instead of only a detail-2 pair", async () => {
    const { manager, views } = harness();
    await manager.navigate("folk-a", "https://example.com");
    await manager.click("folk-a", "b11", { clickCount: 2 });
  ...
  it("invalidates relabelled refs and refuses a late overlay before mouse-down", async () => {
    ...
    await expect(manager.click("folk-a", "b11")).rejects.toThrow(/stale because the page changed/);
    expect(cdpCalls(views[0]).some(([name, params]) => name === "Input.dispatchMouseEvent" && params.type === "mousePressed")).toBe(false);

    await manager.snapshot("folk-a");
    views[0].setHitTarget(999, false);
    await expect(manager.click("folk-a", "b11")).rejects.toThrow(/covers that ref/);
    expect(cdpCalls(views[0]).some(([name, params]) => name === "Input.dispatchMouseEvent" && params.type === "mousePressed")).toBe(false);
  });
  ...
  it("hands native input to the user immediately and gates agent actions until hand-back", async () => {
    ...
    expect(interactions).toEqual([{ botId: "folk-a", profile: "" }]);
    await expect(manager.click("folk-a", "b11")).rejects.toThrow(/held by the user/);
    // Renderer-driven address-bar navigation stays available while the user
    // has control; an agent call remains blocked.
    await manager.navigate("folk-a", "https://example.org", "", { source: "user" });
    expect(manager.setHumanControl("folk-a", false, "")).toBe(true);
    await expect(manager.click("folk-a", "b11")).resolves.toBeTruthy();
  });

  it("asks Chromium not to focus the folk's view when it navigates", () => {
    ...
    manager.layout("folk-a", BOUNDS, "", "expanded");
  ...
  it("never treats a bare focus event as the person taking the wheel", async () => {
    ...
    expect(manager.controlLease("folk-a", "")).toMatchObject({ held: false, epoch: 0 });
    // The folk keeps working: this is the lockout the lease used to cause.
    await expect(manager.click("folk-a", "b11")).resolves.toBeTruthy();
    await expect(manager.navigate("folk-a", "https://example.com/second")).resolves.toBeTruthy();
    expect(manager.controlLease("folk-a", "")).toMatchObject({ held: false, epoch: 0 });
  });
  ...
  it("does not blame the user for a synthetic click whose native event outlives its dispatch", async () => {
    ...
    expect(interactions).toEqual([]);
    expect(manager.controlLease("folk-a", "")).toMatchObject({ held: false });
    await expect(manager.read("folk-a")).resolves.toMatchObject({ title: "Loaded" });
  });
  ...
  it.each(["mouseDown", "contextMenu", "mouseWheel"])("keeps compact human %s input watch-only while expanding", async (type) => {
    ...
    if (type === "mouseDown") {
      ...
      manager.layout("folk-a", { x: 0, y: 0, width: 900, height: 650 }, "", "expanded");
    ...
    manager.setHumanControl("folk-a", false, "");
    await expect(manager.read("folk-a")).resolves.toMatchObject({ title: "Loaded" });
  });
  ...
  it("reopens an already-controlled compact browser without activating its page", async () => {
    ...
    const { manager, views } = harness({ onUserInteraction: (event) => interactions.push(event) });
    manager.layout("folk-a", BOUNDS, "", "compact");
    manager.setHumanControl("folk-a", true, "");
    const event = { preventDefault: vi.fn() };
  ...
  it("preserves the originating profile on terminal surface events", () => {
    ...
    expect(states.at(-1)).toMatchObject({
      ...
      open: false,
      profile: "work",
      partition: "persist:openfolks-browser-profile-work",
      mode: "compact",
      code: "renderer-gone",
    });
  });
  ...
  it("does not mistake a just-finished CDP input event for a human click", async () => {
    ...
    expect(interactions).toEqual([{ botId: "folk-a", profile: "" }]);
    manager.setHumanControl("folk-a", false, "");
    await expect(manager.read("folk-a")).rejects.toThrow(/browser_read is unavailable/);
  });
  ...
  it.each(["mouseDown", "contextMenu"])("taints an autofill-copy-clear page after a human %s", async (type) => {
    ...
    await expect(manager.read("folk-a")).rejects.toThrow(/browser_read is unavailable/);
    const snapshot = await manager.snapshot("folk-a");
  ...
  it("stops a multi-step agent action after a fast take-control and hand-back", async () => {
    ...
    const pending = manager.click("folk-a", "b11");
    await started;
    manager.setHumanControl("folk-a", true);
    manager.setHumanControl("folk-a", false);
    resume();
  ...
  it("stops a paused agent action when its turn capability is revoked", async () => {
    ...
    const pending = manager.click("folk-a", "b11");
    await started;
    manager.cancelAgentActions("folk-a");
    resume();
  ...
  it("marks a revocation key release synthetic instead of claiming human control", async () => {
    ...
    const pending = manager.press("folk-a", "Enter", "");
    await started;
    manager.cancelAgentActions("folk-a");
    resume();
    await expect(pending).rejects.toThrow(/turn ended/);
  ...
  it("hovers, drags, and chooses select options through the page", async () => {
    ...
    await manager.select("folk-a", "b13", "India");
    ...
    await expect(manager.select("folk-a", "b13", [])).rejects.toThrow(/option value or label/);
  });

  it("fills a field by focusing it, selecting everything, and inserting text", async () => {
    const { manager, views } = harness();
    await manager.navigate("folk-a", "https://example.com");
    await manager.fill("folk-a", "b12", "running shoes");
  ...
  it("refuses to fill or type into protected credential and payment fields", async () => {
    ...
    await expect(manager.fill("folk-a", "b12", "super-secret")).rejects.toThrow(/require user control/);
    ...
    await expect(manager.type("folk-a", "123456")).rejects.toThrow(/require user control/);
    ...
    await expect(manager.type("folk-a", "cannot-leak")).rejects.toThrow(/proven ordinary editable field/);
  });
  ...
  it("cannot submit or mutate a user-entered protected form through another action", async () => {
    ...
    await manager.navigate("folk-a", "https://example.com");
    views[0].setProtectedScreenshotValue(true);
    await expect(manager.click("folk-a", "b11")).rejects.toThrow(/protected .* field contains a value/);
    expect(cdpCalls(views[0]).some(([name, params]) => name === "Input.dispatchMouseEvent" && params.type === "mousePressed")).toBe(false);

    views[0].setProtectedScreenshotValue(false);
    views[0].setSensitive("active-element");
    await expect(manager.press("folk-a", "Enter")).rejects.toThrow(/require user control/);
    await expect(manager.press("folk-a", "Backspace")).rejects.toThrow(/require user control/);
  ...
  it("rechecks focus during fill so a page cannot redirect typing into a protected field", async () => {
    ...
    await expect(manager.fill("folk-a", "b12", "must-not-land")).rejects.toThrow(/require user control/);
  ...
  it("presses named keys, scrolls the fixed viewport, and screenshots through the protocol", async () => {
    const { manager, views } = harness();
    await manager.navigate("folk-a", "https://example.com");
    await expect(manager.press("folk-a", "F13")).rejects.toThrow(/unsupported key/);
    await manager.press("folk-a", "Enter");
    ...
    await manager.scroll("folk-a", "down");
    ...
    await expect(manager.scroll("folk-a", "sideways")).rejects.toThrow(/direction/);
    const shot = await manager.screenshot("folk-a");
  ...
  it("normalizes a narrow capturePage fallback to the fixed screenshot size", async () => {
    ...
    await expect(manager.screenshot("folk-a")).resolves.toMatchObject({
      format: "jpeg",
      width: 1024,
      height: 640,
  ...
  it("uses normalized native capture on Linux without issuing a hanging CDP screenshot", async () => {
    const { manager, views } = harness({ platform: "linux" });
    manager.layout("folk-a", { x: 10, y: 20, width: 820, height: 600 }, "", "expanded");
    await manager.navigate("folk-a", "https://example.com");
    const resize = vi.fn(({ width, height }) => ({
      getSize: () => ({ width, height }),
  ...
  it("refuses screenshots with populated protected fields and discards captures that overlap takeover", async () => {
    ...
    await manager.navigate("folk-a", "https://example.com");
    views[0].setProtectedScreenshotValue(true);
    await expect(manager.screenshot("folk-a")).rejects.toThrow(/protected field contains a value/);
    expect(cdpCalls(views[0]).some(([name]) => name === "Page.captureScreenshot")).toBe(false);
  ...
  it("fails observations closed while protected or human-typed page text could echo a secret", async () => {
    ...
    await expect(manager.read("folk-a")).rejects.toThrow(/browser_read is unavailable/);
    const protectedPage = await manager.snapshot("folk-a");
    expect(protectedPage).toMatchObject({ url: "", title: "Protected content hidden", yaml: null, elements: [] });
    await expect(manager.agentState("folk-a", "")).resolves.toMatchObject({ url: "", title: "Protected content hidden" });
    ...
    manager.setHumanControl("folk-a", false, "");
    await expect(manager.read("folk-a")).rejects.toThrow(/browser_read is unavailable/);
    expect(await manager.snapshot("folk-a")).toMatchObject({ title: "Protected content hidden", elements: [] });

    // Only a committed document navigation clears the conservative taint.
    views[0].listeners.get("did-navigate")?.();
    views[0].setPageText("safe page");
    views[0].setTitle("Safe");
    await expect(manager.read("folk-a")).resolves.toMatchObject({ title: "Safe", text: "safe page" });
  });
  ...
  it("cuts a long page at the read limit and says so in the text the model gets", async () => {
    ...
    const read = await manager.read("folk-a");
  ...
  it("says how many interactive elements the accessibility fallback left out", async () => {
    ...
    // and has no way to page. Dropping the rest in silence let a folk believe
  ...
  it("does not tell the reader that scrolling pages the snapshot", async () => {
    ...
    const page = await manager.navigate("folk-a", "https://example.com");
    const notes = page.notes.join("\n");

    // The tree is whole-document, so "scroll to see it" sent folks into
  ...
  it("waits for text or an address, reads the page, and reports dialogs it answered", async () => {
    ...
    await expect(manager.waitFor("folk-a", { text: "Docs" })).resolves.toMatchObject({ url: "https://example.com/" });
    await expect(manager.waitFor("folk-a", { text: "never there", timeoutMs: 300 })).rejects.toThrow(/timed out waiting for text "never there"/);
    await expect(manager.waitFor("folk-a", { url: "example.com" })).resolves.toBeTruthy();
    ...
    expect((await manager.snapshot("folk-a")).dialogs).toEqual([]);
  });
  ...
  it("uses Playwright's snapshot with e-refs when the page carries the script, injecting it once per document", async () => {
    ...
    await expect(manager.click("folk-a", "e9")).rejects.toThrow(/stale or unknown/);
    await expect(manager.click("folk-a", "b11")).rejects.toThrow(/stale or unknown/);
    // fill focuses through the page; select resolves the element handle through the page
    await manager.fill("folk-a", "e2", "shoes");
    expect(cdpCalls(views[0]).some(([name]) => name === "DOM.focus")).toBe(false);
    await manager.select("folk-a", "e2", "India");
    ...
    const fallback = await bare.manager.navigate("folk-a", "https://example.com");
  ...
  it("revalidates rich refs and compositor hit targets in the isolated helper", async () => {
    ...
    await expect(manager.click("folk-a", "e1")).rejects.toThrow(/stale because the page changed/);

    views[0].setRichRefValid(true);
    await manager.snapshot("folk-a");
    views[0].setRichHit(false);
    await expect(manager.click("folk-a", "e1")).rejects.toThrow(/covers that ref/);
    expect(cdpCalls(views[0]).some(([name, params]) => name === "Input.dispatchMouseEvent" && params.type === "mousePressed")).toBe(false);
  });
  ...
  it("runs every page helper in a CDP isolated world and ignores a hostile page global", async () => {
    ...
    expect(cdpCalls(views[0])).toContainEqual([
      ...
      { frameId: "main-frame", worldName: "openfolks-browser-snapshot", grantUniveralAccess: false },
    ]);
  });

  it("drops every folk's view on a deleted profile and leaves other sessions alone", () => {
    const { manager, views } = harness();
    manager.layout("folk-a", BOUNDS, "work", "compact");
    manager.layout("folk-b", BOUNDS, "work", "compact");
    manager.layout("folk-c", BOUNDS, "", "compact");
    expect(manager.forgetProfile("work")).toBe(2);
    expect(manager.size()).toBe(1);
    expect(views[2].calls.some(([name]) => name === "close")).toBe(false);
    expect(manager.state("folk-a")).toMatchObject({ open: false });
  ...
  it("accepts exact migrated partitions but refuses lossy aliases", () => {
    ...
    for (const alias of ["work!", "../work"]) {
      expect(() => manager.layout("folk-b", BOUNDS, alias, "compact")).toThrow(/valid browser profile partition id/);
      expect(() => manager.setCapabilityActive("folk-b", alias, true)).toThrow(/valid browser profile partition id/);
  ...
  it("tears every view down on closeAll and hides them all on hideAll", async () => {
    ...
    manager.close("folk-a");
    expect(manager.size()).toBe(1);
    manager.closeAll();
    expect(manager.size()).toBe(0);
    expect(owner.contentView.children).toEqual([]);
    expect(states.at(-1)).toMatchObject({ botId: "folk-b", open: false });
  });
});
