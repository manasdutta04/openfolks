...
describe("queueDelegation", () => {
  ...
  it("rejects when the target folk does not exist", () => {
    const result = queueDelegation(commsBus, from, {
      toBotId: "ghost",
      message: "where?",
      depth: 0,
...
describe("drainDelegations", () => {
  ...
  it("runs the target's turn via runTarget and mirrors the exchange", async () => {
    ...
    // source/target folk respectively, linking to the same channel.
  ...
  it("skips runTarget and emits a 'no such folk' chip when the target was deleted", async () => {
    queueDelegation(commsBus, from, { toBotId: target.id, message: "do this", depth: 0 }, 1);
    ...
    const chip = await waitFor(() =>
      store
        .messagesFor(from.threadId)
        .find((m) => m.kind === "activity" && (m.tool?.name ?? "").includes("no such folk")),
    );
    expect(chip.tool?.ok).toBe(false);
    expect(runTargetCalls).toEqual([]);
  });

  it("drops a queued handoff when section assignment separates the folks before dispatch", async () => {
    ...
    expect(
      store.messagesFor(from.threadId).some((message) =>
        message.tool?.name.includes("folks now belong to different sections")),
    ).toBe(true);
  });
  ...
  it("asks for approval when approvePeerComms is on, then runs only on allow", async () => {
    ...
    // the source folk's thread shows the options card BEFORE runTarget fires
  ...
  it("no-ops when the source thread no longer resolves to a folk", () => {
    queueDelegation(commsBus, from, { toBotId: target.id, message: "do this", depth: 0 }, 1);
