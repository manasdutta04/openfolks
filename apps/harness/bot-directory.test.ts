...
const entry = (over: Partial<Record<string, unknown>> = {}) => ({
  ...
  category: "Docs",
  integrations: ["GitHub"],
  prompt: "Set up a folk that drafts release notes.",
  detailUrl: "https://botdirectory.ai/folks/release-scribe/",
  ...over,
});
  ...
  it("drops malformed entries instead of failing the whole directory", () => {
    const bots = parseBotDirectory({
      ...
      bots: [
        ...
        entry({ slug: "elsewhere", detailUrl: "https://evil.example/folks/x/" }),
...
describe("matchDirectoryBots", () => {
  ...
  it("returns only folks that overlap the profile, best match first, with the matched terms", () => {
