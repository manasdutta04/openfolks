...
// tools are:
//
//   list_bots()                          → the other folks in this section + their status
//   ask_bot(bot_id, msg)                 → send msg to that folk, wait, return its reply
...
//   list_routines()                       → inspect this folk's scheduled work
...
const ROUTINE_FIELDS_SCHEMA = {
  ...
  instructions: {
    ...
    minLength: 1,
    maxLength: 20_000,
    description: "The complete instructions the folk should follow each time the routine runs.",
  },
  schedule: ROUTINE_SCHEDULE_SCHEMA,
  run_on: {
    type: "string",
    enum: ["maus", "cloud"],
    description: "Where the routine runs. Defaults to maus (this OpenFolks setup).",
  },
  duration_minutes: {
    type: "integer",
    minimum: 15,
...
const TOOLS = [
  {
    name: "list_bots",
    description:
      "List the other bots (agents) in your OpenFolks section, with their model and whether they're busy. Call this before delegate_bot or ask_bot to discover who's available. Use delegate_bot for assignments; use ask_bot only for a short consultation needed inline.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "ask_bot",
    description:
      "SYNCHRONOUS consultation: send a short question to another folk and stay blocked until its reply is returned inline. Use only when that reply is required to write your current response. Do not use for assigning work, background tasks, or potentially long work; use delegate_bot for those. Returns promptly with a note if that folk is busy.",
    inputSchema: {
      type: "object",
      properties: {
        bot_id: { type: "string", description: "The target folk's id (from list_bots)." },
        message: { type: "string", description: "What to say / ask the folk." },
      },
  ...
  },
  {
    name: "delegate_bot",
    description:
      "DEFAULT FOR ASSIGNING WORK. Hand a task to another folk asynchronously: this returns immediately, your turn can end, and you remain available while the peer works. The peer starts after your current turn finishes and its result is delivered automatically to the originating conversation. Acknowledge the assignment; do not call check_delegation or wait_delegation in this same turn.",
    inputSchema: {
      type: "object",
      properties: {
        bot_id: { type: "string", description: "The target folk's id (from list_bots)." },
  ...
  },
  {
    name: "create_bot",
    description:
      "Create a specialist folk in your section. Only a section's Chief of Staff may use this. The new folk inherits the Chief's engine, starts with connected apps and automatic approvals disabled, and can then receive work through delegate_bot. Create only the smallest useful team (maximum four per turn).",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Ask the user for a supported API key through OpenFolks's secure credential card. Use this instead of asking them to paste a secret into chat. The secret is saved by the desktop app and is never returned to you. After calling this tool, end the turn; OpenFolks resumes the task after the user saves or declines.",
    inputSchema: {
      type: "object",
      properties: {
        credential_id: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        for_bot_id: {
          type: "string",
          description:
            "Only when the user asks to schedule this routine for ANOTHER folk in your section: that folk's id from list_bots. Omit to schedule it for yourself. The routine then belongs to that folk and each run uses its engine and permissions.",
        },
      },
  ...
  },
  {
    ...
    description:
      "Prepare a user-requested change to one of this folk's existing routines. This only creates a durable confirmation card; it does NOT apply the change. Use list_routines first to get the routine id. After calling it, end the turn and do not claim the action completed until the user confirms the card.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
  ...
  },
  {
    name: "skills_list",
    description:
      "List this folk's imported skills (enabled and disabled) and any staged skill writes waiting for the user to confirm. Use this before skill_manage to avoid duplicate names. Listing does not enable anything.",
    inputSchema: { type: "object", additionalProperties: false, properties: {} },
  },
  {
    name: "skill_manage",
...
async function callTool(name: string, args: Json): Promise<{ text: string; isError?: boolean }> {
  if (name === "list_bots") {
    ...
    if (!bots.length) return { text: "No other folks in this section yet." };
  ...
  if (name === "ask_bot") {
    ...
    const r = await api(`/api/internal/ask-folk`, {
      method: "POST",
    ...
    if (r.busy) {
      ...
      return { text: `That folk is busy right now — try again after it finishes.` };
    }
    if (r.error) return { text: `Couldn't reach that folk: ${r.error}`, isError: true };
  ...
  if (name === "delegate_bot") {
    ...
    const r = await api(`/api/internal/delegate-folk`, { method: "POST", body: JSON.stringify(body) });
    ...
    // folk's claim ticket for the outcome.
  ...
  if (name === "check_delegation" || name === "wait_delegation") {
    ...
    if (delegationTaskIdsThisTurn.has(taskId)) {
      return {
        text: `Task ${taskId} was delegated during this turn. Finish your response now so the other folk can work; its result will be delivered to this conversation automatically. Do not check or wait for a newly delegated task until a later turn.`,
        isError: true,
      };
    }
  ...
  if (name === "create_bot") {
    ...
    if (createdThisTurn >= MAX_CREATED_PER_TURN) {
      return { text: `You can create at most ${MAX_CREATED_PER_TURN} folks in one turn. Use the team you have before adding more.`, isError: true };
    }
    const r = await api(`/api/internal/create-folk`, {
      method: "POST",
      body: JSON.stringify({
        fromBotId: BOT_ID,
        fromThreadId: THREAD_ID,
  ...
  if (name === "request_credential") {
    ...
    return {
      text: `A secure ${r.label ?? CREDENTIAL_TARGETS[credentialId].label} card is now visible to the user. End this turn; OpenFolks will resume the task after they save or decline. Never ask them to paste the key into chat.`,
    };
  }
  if (name === "list_routines") {
    ...
    if (!routines.length) {
      return { text: `This folk has no routines. Current time: ${now}. Timezone: ${timeZone}.` };
    }
    return {
      text: `This folk's routines (current time: ${now}; timezone: ${timeZone}):\n${JSON.stringify(routines, null, 2)}`,
    };
  }
  ...
  if (name === "skills_list") {
    ...
    if (!skills.length && !staged.length) {
      return { text: "This folk has no imported skills and nothing staged. Use skill_manage action=\"create\" to stage one for the user to confirm." };
    }
