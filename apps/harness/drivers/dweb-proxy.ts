...
// Exposes dweb's HTTP API (the local OpenFolks network daemon) as tools an
// agent can call to inspect the network and run model requests:
//
//   dweb_status          → ping dweb, summarize server + peer state
//   dweb_repo_status     → the OpenFolks repo the daemon is tracking
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Report the state of the OpenFolks repository the dweb daemon is tracking: repo name, checked-out branch, last commit, file count, and checkout path.",
    inputSchema: { type: "object", properties: {} },
  },
  {
