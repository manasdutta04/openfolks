...
beforeAll(async () => {
  stub = createServer((req, res) => {
    ...
    if (req.method === "GET" && req.url?.startsWith("/api/internal/agents")) {
      ...
      return res.end(
        JSON.stringify({
          bots: [{ id: "folk-helper", name: "Helper", model: "fake-model", busy: false }],
        }),
      );
    }
    if (req.method === "POST" && req.url === "/api/internal/ask-folk") {
      let data = "";
    ...
    if (req.method === "POST" && req.url === "/api/internal/delegate-folk") {
      let data = "";
      req.on("data", (c) => (data += c));
      req.on("end", () => {
    ...
    if (req.method === "POST" && req.url === "/api/internal/create-folk") {
      let data = "";
      req.on("data", (c) => (data += c));
      req.on("end", () => {
        lastCreateBody = JSON.parse(data);
        res.writeHead(201, { "content-type": "application/json" });
        res.end(JSON.stringify({ id: "folk-designer", name: "Pixel", section: "Work" }));
      });
      return;
    }
...
describe("agents-proxy MCP surface", () => {
  ...
  it("list_bots renders the roster and authenticates with the shared token", async () => {
    ...
    expect(text).toContain("folk-helper");
  ...
  it("ask_bot forwards sender + depth and returns the reply", async () => {
    ...
    const res = await callTool("ask_bot", { bot_id: "folk-helper", message: "ping" });
  ...
  it("returns queue refusal guidance to the agent as a tool error", async () => {
    ...
    const res = await callTool("delegate_bot", { bot_id: "folk-helper", message: "take this" });
    expect(res.result.isError).toBe(true);
  ...
  it("hands back the task id and rejects sequential same-turn status calls", async () => {
    ...
    const res = await callTool("delegate_bot", { bot_id: "folk-helper", message: "do the thing" });
  ...
  it("check/wait_delegation: flat schemas, guided errors, and the read-back wire", async () => {
    ...
    expect(lastDelegationUrl).toContain("fromBotId=folk-asker");
  ...
  it("lists only the current folk's routines with authoritative time context", async () => {
    routinesResponse = {
      now: "2026-08-28T10:30:00.000Z",
      timeZone: "Asia/Kolkata",
    ...
    expect(query.get("fromBotId")).toBe("folk-asker");
  ...
  it("forwards for_bot_id when the routine is for another folk", async () => {
    lastRoutineRequestBody = null;
    const res = await callTool("propose_routine", {
      name: "Teammate brief",
      instructions: "Summarize for the teammate.",
      schedule: { type: "weekly", time: "08:00", weekdays: ["tuesday"] },
      for_bot_id: "folk-helper",
    });
    expect(lastRoutineRequestBody.forBotId).toBe("folk-helper");
  ...
  it("lists, views, and stages skills without enabling them", async () => {
    ...
    expect(lastSkillQuery).toContain("fromBotId=folk-asker");
