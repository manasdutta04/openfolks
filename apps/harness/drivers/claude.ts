...
//   - the folk's cloud computer (box.ascii.dev) via server/computer-proxy.ts
...
const DENY_TIMEOUT_NOTE =
  "OpenFolks: nobody answered this permission request in time. Skip this action and finish what you can without it.";
const QUESTION_TIMEOUT_NOTE = "OpenFolks: nobody answered in time. Use your best judgment and continue.";
const DUPLICATE_ASK_ID_NOTE = "OpenFolks: duplicate ask id — skipping this request.";
...
function systemEndedReply(kind: Ask["kind"]): { behavior: AskBehavior; message: string } {
  return kind === "question"
    ? { behavior: "answer", message: "OpenFolks: the turn is ending — wrap up." }
    : { behavior: "deny", message: "OpenFolks: the turn ended" };
}
  ...
  async create(input: DriverCreateInput<ClaudeConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      if (turn.integrations?.computer) {
        ...
        // Host tools always route through OpenFolks's permission broker.
      ...
      // own ListAgents look-alike shadows it and "@Folk" asks go nowhere
