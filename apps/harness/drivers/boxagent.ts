...
// separately polls screenshots so the chat shows the folk's screen live.
//
...
export const BoxAgentDriver: ProviderDriver<BoxAgentConfig> = {
  ...
  async create(input: DriverCreateInput<BoxAgentConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      if (!token) throw new Error('box not configured — add {"box":{"token":"…"}} to ~/.openfolks/config.json');
      if (!boxId) {
        throw new Error("this folk has no computer yet — open the Computer panel and provision one");
      }
    ...
    const snapshot = async (): Promise<ProviderSnapshot> => {
      if (!token) {
        return { state: "unavailable", reason: "no Box token — add it in Settings → Connections" };
      }
      try {
        await api("/me");
