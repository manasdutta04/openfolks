...
// snapshot — element refs (`b<id>`), roles and names — so the folk rarely
...
//
// Speaks raw JSON-RPC 2.0 over stdio (house style: agents-proxy).
...
//   OMB_BOT_ID         which bot's tab to drive (one view per folk)
//   OMB_BROWSER_PROFILE named shared session the folk is pointed at ("" = own)
...
const pageSchema = z.object({
  ...
  /** what the surface did or noticed on the folk's behalf: answered dialogs, off-screen content */
...
function wallNote(kind: WallKind): string {
  return kind === "verification"
    ? "This looks like a folk check or verification page. Do not try to solve it: call browser_request_takeover so the user can complete it in the Browser panel, then continue from the page you get back."
...
export async function hostRequest(operation: string, body: object = {}, fetchImpl: typeof fetch = fetch): Promise<unknown> {
  if (!HOST || !TOKEN || !BOT_ID) throw new Error("the built-in browser is not connected for this folk");
...
export const TOOLS = [
  {
    ...
    description:
      "Open a web address in this folk's built-in browser tab (the user can watch it in the Browser panel). Returns the page's interactive elements with refs — do not follow it with browser_snapshot.",
...
async function requestTakeover(reason: string, request: HostRequest, waitMs = TAKEOVER_WAIT_MS, pollMs = TAKEOVER_POLL_MS): Promise<ToolResult> {
  ...
  // while the folk waited out its whole window. The harness keeps an existing
...
async function handle(line: string) {
  ...
  if (method === "initialize") {
    return ok(id, {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "openfolks-browser", version: "1" },
    });
  }
