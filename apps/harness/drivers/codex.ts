    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      const launchAttempt = async (attempt: number): Promise<void> => {
        ...
        if (turn.integrations?.composio) {
          mountMcpServer(appServerArgs, env, "openfolks_connectors", turn.integrations.composio);
        }
        ...
        if (turn.integrations?.computer) {
          ...
          mountMcpServer(appServerArgs, env, "computer", {
            ...
            env: {
              ...
              // panel pauses this folk's hands mid-turn
      ...
      const request = (method: string, params: unknown, timeoutMs = 60_000) =>
        new Promise<any>((resolve, reject) => {
          ...
          // the handshake await hangs forever and the folk stays busy for good
      ...
      const settle = (ok: boolean, stopReason: string | null) => {
        ...
        state.settled = true;
        for (const finish of [...asks.values()]) finish("deny", "OpenFolks: the turn ended", "system");
      ...
      try {
        await request("initialize", { clientInfo: { name: "openfolks", version: "1" } });
        ...
        await request("turn/start", {
          ...
          // sent another, and choosing Default lands on the folk's next new
