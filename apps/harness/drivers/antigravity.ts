...
// Local VM / VPS) and its OpenFolks teammate tools are mounted by upserting
// OpenFolks-owned keys into the global `~/.gemini/config/mcp_config.json`
...
// So OpenFolks tools are mounted by upserting two reserved keys into the global file
...
export const ANTIGRAVITY_COMPUTER_MCP_KEY = "openfolks-computer";
export const ANTIGRAVITY_AGENTS_MCP_KEY = "openfolks-agents";
 ...
 * Cloud boxes go through OpenFolks's REST-to-MCP adapter (the same spec
...
/** Build every OpenFolks-owned agy MCP entry for one turn. */
...
function ensureAntigravityOwnedMcpServers(
  ...
  return () => {
    ...
    // OpenFolks entry is preferable to destroying bytes we cannot interpret.
...
/** Upsert the current turn's OpenFolks MCP entries and remove absent ones.
...
function decodeConfig(raw: unknown): AntigravityConfig {
  ...
  return {
    ...
    // them — producing no output, so a non-fullAuto folk's turns frequently fail.
...
export const AntigravityDriver: ProviderDriver<AntigravityConfig> = {
  ...
  async create(input: DriverCreateInput<AntigravityConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      // exiting, the folk would stay busy forever (agy's own --print-timeout 10m
      ...
      // OpenFolks tools — owns the mount for its complete child lifetime. This keeps
