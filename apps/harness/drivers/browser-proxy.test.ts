...
describe("browser MCP proxy", () => {
  ...
  it("forwards navigation to the folk's own tab and returns the page with a scrubbed address", async () => {
    hits.length = 0;
  ...
  it("acts on refs and relays the host's own sentence when one is stale", async () => {
    ...
    expect(hits.map((hit) => [hit.path, hit.body])).toEqual([
      ["/v1/bots/folk-1/click", { ref: "b1", double: true, profile: "work" }],
      ["/v1/bots/folk-1/fill", { ref: "b2", text: "boots", profile: "work" }],
      ["/v1/bots/folk-1/press", { key: "enter", profile: "work" }],
      ["/v1/bots/folk-1/scroll", { direction: "down", amount: 300, profile: "work" }],
    ]);
...
describe("browser takeover", () => {
  ...
  it("pages the user even when the wheel is already held", async () => {
    ...
    // so staying quiet here left the folk waiting on a plea nobody was shown.
