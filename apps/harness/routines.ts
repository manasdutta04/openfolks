...
function sanitizeInput(input: RoutineInput): Omit<Routine, "id" | "createdAt" | "updatedAt" | "nextRunAt"> {
  ...
  if (!prompt) throw new Error("Tell the folk what to do");
  if (!botId) throw new Error("Choose a folk");
...
export class RoutineManager {
  ...
  constructor(options: RoutineManagerOptions) {
    ...
    for (const run of this.runs) {
      if (run.status === "running" || run.status === "waiting") {
        run.status = "failed";
        run.error = "OpenFolks restarted while this routine was running";
  ...
  create(input: RoutineInput, request?: RoutineRequestCommitFor<"create">): Routine {
    ...
    if (this.options.botState(clean.botId) === "missing") throw new Error("That folk no longer exists");
    const at = this.now();
  ...
  disableForBot(botId: string) {
    ...
    for (const run of this.runs) {
      ...
      run.attention = undefined;
      run.finishedAt = this.now();
      run.error = "The assigned folk was deleted";
      this.emitRun(run);
   ...
   * reuses this manager so busy-folk ordering, task creation and VM routing stay
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      for (const run of [...this.runs].reverse()) {
        ...
        if (state === "missing") {
          this.failRun(run, "The assigned folk no longer exists");
          continue;
        }
        // A webhook is an incoming message, so make its task the folk's live
  ...
  handleRuntimeEvent(event: RuntimeEvent): RoutineRun | null {
    ...
    if (event.type === "request.opened") {
      ...
      if (!event.ok) {
        this.failRun(run, event.stopReason ?? run.error ?? "The folk did not complete this run");
        queueMicrotask(() => void this.tick());
        return cloneRun(run);
      }
