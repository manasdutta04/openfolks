...
import { pickPebbleSeed } from "../../shared/pebble-avatars.ts";
...
/**
 * The face a folk rests on, as one of the engine's state names. Kept as a plain
...
export interface Message {
  ...
  /** Terminal receipt for a bounded multi-folk channel goal. */
  ...
  /** screen messages: a frame of the folk's computer (base64 image) */
  png?: string;
  mime?: string;
  at: number;
...
export interface GroupRecord {
  ...
  unread: boolean;
  createdAt: number;
  /** true for auto-created folk⇄folk channels (ask_bot exchanges live here;
...
/** Everything the FOLK authored is scrubbed of content-shaped secrets before
...
function redactBotAuthored<T extends Omit<Message, "id" | "at"> & { at?: number }>(message: T): T {
  ...
  if (out.card) {
    ...
    // Routine definitions are executable folk-authored text stored behind the
...
/** The states in which the folk cannot take a new message. */
...
export interface BotRecord {
  ...
  threadId: ThreadId;
  /** every task this folk has, newest first */
  tasks?: TaskRecord[];
  name: string;
  title: string;
  description: string;
  ...
  /** App-owned attachment served as this folk's custom profile image. */
  avatarUrl?: string;
  ...
  cwd?: string;
  /** Auto mode: the folk approves its own tool permissions and keeps
  ...
  /** Tools this folk may always use without asking, even outside auto mode
   * (set by "Always allow" on an approval card). */
  alwaysAllow?: string[];
  /** Speak this folk's replies aloud as they settle, without being asked.
  ...
  /** This bot's own voice id, so a room of folks doesn't sound like one
  ...
  pinnedMessageId?: string;
  /** The coordinator for this folk's sidebar section. The store enforces
   ...
   * false = this folk never receives the connection. Imported team members
...
/** Prefer a color no current bot uses; otherwise cycle from `seed`. */
function pickBotColor(used: Iterable<MausColor | undefined>, seed = 0): MausColor {
 ...
 * "New Folk"), names match case-insensitively, longest name wins (so
 * "@New Folk 2" never half-matches "New Folk"), hidden bots skipped, results
...
export class Store {
  ...
  constructor(defaultSelection: () => ModelSelection) {
    ...
    this.bots.forEach((b, index) => {
      if (b.avatarSeed?.trim()) return;
      b.avatarSeed = pickPebbleSeed(this.bots.map((other) => other.avatarSeed), index);
      botsMigrated = true;
    });
    for (const b of this.bots) {
  ...
  branchMessage(threadId: string, sourceId: string, text: string): Message | null {
    ...
    mdb.appendMessage(threadId, full);
    this.emit({ type: "message", threadId, message: full });
    // Clients only auto-follow children of the current leaf. An edit is a
    // sibling of the old prompt, so they stay on version 1 unless we
    // announce the new head.
  ...
  createBot(
    profile: Partial<
      Pick<
        BotRecord,
        "name" | "title" | "description" | "color" | "mascotExpression" | "mascotShape" | "avatarSeed" | "modelSelection" | "section"
      >
    > = {},
    opts: {
    ...
    const bot: BotRecord = {
      ...
      notifications: true,
      color: profile.color ?? pickBotColor(this.bots.map((b) => b.color), seed),
      ...(profile.mascotExpression ? { mascotExpression: profile.mascotExpression } : {}),
      avatarSeed:
        profile.avatarSeed ??
        pickPebbleSeed(
          this.bots.map((b) => b.avatarSeed),
          seed,
        ),
      unread: false,
    ...
    // the folk/thread mapping before they can place either message.
  ...
  deleteBot(id: string): boolean {
    ...
    // every task's transcript goes with the folk, not just the open one
    for (const threadId of new Set([bot.threadId, ...(bot.tasks ?? []).map((t) => t.threadId)])) {
      this.deleteThreadRecord(threadId);
    }
    // the folk's workspace (files + memory) goes with it — same rule as its
    // transcripts: deleting a folk deletes what it knew
    try {
      rmSync(workspaceDir(id), { recursive: true, force: true });
    } catch {}
    // Approval state deliberately lives outside the folk-writable workspace.
    // It still belongs to the folk, so deleting the folk must remove staged
  ...
  /** File visible folks into one sidebar section as a single durable write.
   *
   ...
   * window, including the folk that just handed the role over. */
  ...
  setResumeCursor(botId: string, instanceId: string, cursor: unknown, threadId?: string) {
    const bot = this.bot(botId);
    if (!bot) return;
    // the cursor belongs to the task that produced it, not to the folk
  ...
  /** The folder a task's turn runs in. Pins on first call from the folk's
