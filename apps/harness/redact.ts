...
// travels in an MCP header. Those logs sit in ~/.openfolks/native as
...
// What a folk's own reply, a tool title, or a permission card can carry —
...
export function redactSecrets(input: unknown, depth = 0): unknown {
  ...
  for (const [key, value] of Object.entries(input as Record<string, unknown>)) {
    ...
    // header value, a folk's reply) — the content pass catches those
