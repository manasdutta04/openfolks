...
export function parseBotDirectory(value: unknown): DirectoryBot[] {
  if (!isRecord(value) || value.version !== 1 || !Array.isArray(value.bots)) {
    throw new Error("The folk directory response is not supported");
  }
...
async function readBounded(response: Response, maxBytes: number): Promise<string> {
  const oversized = () => new Error("The folk directory response is too large");
...
export async function fetchBotDirectory(fetcher: Fetcher = fetch): Promise<DirectoryBot[]> {
  ...
  if (!response.ok) throw new Error(`The folk directory returned HTTP ${response.status}`);
 ...
 * project's stacks/summary words and a folk's name, category and
...
// measured live: "Your own team of AI folks" matched a home-value tracker
// purely on "your".
const STOPWORDS = new Set([
  ...
  "simple", "easy", "apps", "tool", "tools", "project", "team", "chat", "folks",
]);
