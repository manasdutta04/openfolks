...
describe("chiefOfStaffSystemPrompt roster caps", () => {
  ...
  it("caps the roster length and says how many were left out", () => {
    const team = Array.from({ length: 60 }, (_, i) => ({ id: `folk${i}`, name: `Folk ${i}` }));
    const prompt = chiefOfStaffSystemPrompt("chief", [{ id: "chief", name: "Atlas" }, ...team], true);
    expect(prompt).toContain("Folk 39");
    expect(prompt).not.toContain("Folk 40 —");
...
describe("chiefOfStaffSystemPrompt", () => {
  ...
  it("includes trusted OpenFolks status only when the Chief caller supplies it", () => {
    const status = "TRUSTED OPENFOLKS STATUS\nfreshness=fresh; runtime_state=degraded";
    ...
    expect(ordinaryPrompt).not.toContain("TRUSTED OPENFOLKS STATUS");
  });
});
