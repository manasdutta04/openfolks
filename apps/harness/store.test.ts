...
describe("Store", () => {
  ...
  it("does not dismiss the quiz for folk-authored messages", () => {
    const store = new Store(selection);
    const bot = store.createBot();
  ...
  it("rotates colors across created folks", () => {
    const store = new Store(selection);
  ...
  it("persists folks and messages across a restart, resetting busy", () => {
    const store = new Store(selection);
    const bot = store.createBot();
  ...
  it("persists a folk's effort level across a restart, defaulting to unset", () => {
    const store = new Store(selection);
    const bot = store.createBot();
  ...
  it("files visible folks atomically without changing Chief roles", () => {
    const store = new Store(selection);
  ...
  it("rejects unavailable or Chief-conflicting section assignments without changing folks", () => {
    const store = new Store(selection);
    const incumbent = store.createBot({ section: "Launch" });
    const incoming = store.createBot({ section: "Research" });
    const teammate = store.createBot({ section: "Personal" });
  ...
  it("seedIfEmpty creates exactly one starter folk, once", () => {
    const store = new Store(selection);
    store.seedIfEmpty();
  ...
  it("branchMessage announces the new leaf so clients leave the old branch", () => {
    const store = new Store(selection);
    const bot = store.createBot();
    const original = store.appendMessage(bot.threadId, { role: "user", kind: "text", text: "v1" });
  ...
  it("deleteBot removes the folk and its durable transcript", () => {
    const store = new Store(selection);
    const bot = store.createBot();
...
describe("Store change stream", () => {
  ...
  it("announces a new folk before its onboarding messages", () => {
    const store = new Store(selection);
    const events = record(store);
    const bot = store.createBot();
  ...
  it("every folk write emits a folk event carrying only the id (the wire shape is the caller's)", () => {
    const store = new Store(selection);
    const bot = store.createBot();
    const events = record(store);
...
describe("Store folk activity state", () => {
  beforeEach(() => {
    rmSync(DATA_DIR, { recursive: true, force: true });
  });
  ...
  it("emits a folk change per transition and skips a no-op", () => {
    const store = new Store(selection);
    const bot = store.createBot();
    const seen: string[] = [];
...
describe("Store redacts folk-authored secrets on write", () => {
  beforeEach(() => {
    rmSync(DATA_DIR, { recursive: true, force: true });
  });

  it("masks a key in folk text, tools and cards — but never in what the user typed", () => {
    const store = new Store(selection);
    const bot = store.createBot();
...
describe("Store task working folder", () => {
  ...
  it("pins the folk's folder onto a task on its first turn, and never again", () => {
    const store = new Store(selection);
    const bot = store.createBot();
    store.patchBot(bot.id, { cwd: "/tmp/project-a" });

    // first turn: nothing pinned yet → takes the folk's folder
    ...
    // a new task starts in the folk's current folder
    const next = store.createTask(bot.id, "second")!;
    expect(store.pinTaskCwd(bot.id, next.threadId)).toBe("/tmp/project-b");
  });

  it("pins the default (null) when the folk has no folder, so a later folder can't move a live session", () => {
    const store = new Store(selection);
    const bot = store.createBot();
  ...
  it("pins a supplied private workspace when the folk has no custom folder", () => {
    const store = new Store(selection);
    const bot = store.createBot();
  ...
  it("a legacy task that already has a session pins to the default, not the folk's new folder", () => {
    const store = new Store(selection);
    const bot = store.createBot();
...
describe("Store task working folder — cloud runs", () => {
  ...
  it("a cloud run pins the default so the folk's host folder never shows for that task", () => {
    const store = new Store(selection);
    const bot = store.createBot();
    store.patchBot(bot.id, { cwd: "/tmp/project-a" });
    expect(store.pinTaskCwd(bot.id, bot.threadId)).toBe("/tmp/project-a");
