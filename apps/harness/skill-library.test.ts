...
const sample: BundledSkill = {
  directory: "/skills/sample-tool",
  instructions: "---\nname: sample-tool\ndescription: test\n---\nUse sample tools.",
  manifest: {
    id: "sample-tool",
    name: "Sample Tool",
    version: "0.1.0",
    description: "Control a sample tool",
    defaultEnabled: true,
    triggerTerms: ["sample", "demo"],
    requiredCapabilities: ["sampleMcp"],
  },
};

describe("bundled skill library", () => {
  it("selects a skill only when both its trigger and capability are present", () => {
    const rendered = skillInstructionsFor("Run the sample demo now", ["sampleMcp"], [sample]);
    expect(rendered).toContain("Use sample tools");
    expect(rendered).not.toContain('root="/skills/sample-tool"');
    expect(skillInstructionsFor("Run the sample demo now", ["sampleMcp"], [sample], { includeRoot: true }))
      .toContain('root="/skills/sample-tool"');
    expect(skillInstructionsFor("Run the sample demo now", [], [sample])).toBe("");
    expect(skillInstructionsFor("Write a poem", ["sampleMcp"], [sample])).toBe("");
  });
  ...
  it("requires the manifest id to match its isolated folder", () => {
    expect(() => parseSkillManifest({
      ...sample.manifest,
      id: "other-skill",
    }, "/skills/sample-tool")).toThrow(/invalid id/);
  });

  it("loads a recorded skill without letting a broken sibling disable it", () => {
    const root = mkdtempSync(join(tmpdir(), "openfolks-skills-"));
  ...
  it("does not let a user skill shadow a bundled skill id", () => {
    expect(mergeSkills([sample], [{ ...sample, instructions: "user replacement" }])).toEqual([sample]);
  });

  it("treats a non-directory user skill root as empty", () => {
    const root = mkdtempSync(join(tmpdir(), "openfolks-skills-root-"));
...
describe("bundled verification skill", () => {
  const skills = loadBundledSkills(join(process.cwd(), "portals"));
  ...
  it("ships one reviewed authoring adapter", () => {
    ...
    expect(instructions).not.toContain("~/.openfolks");
