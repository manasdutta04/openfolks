...
//     reads OpenFolks's temporary `openfolks-agents` entry from agy's
...
function agentsMcpEntry(): McpEntry | null {
  ...
  try {
    ...
    const entry = config.mcpServers?.["openfolks-agents"];
  ...
  return new Promise((resolve, reject) => {
    ...
    write({
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "openfolks-fake-antigravity", version: "1.0.0" },
      },
    });
  });
}
