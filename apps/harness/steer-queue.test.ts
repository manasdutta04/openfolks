...
describe("steer-queue module", () => {
  it("does not append a queued user message until drain", () => {
    const bot = fakeBot("folk-a", "thread-a", true);
  ...
  it("keeps a stable client receipt while a send waits to drain", () => {
    const bot = fakeBot("folk-receipt", "thread-receipt", true);
    const store = fakeStore([bot]);
    ...
    expect(queuedSteeredMessage("other-folk", bot.threadId, "send_1234567890123456")).toBeNull();
  ...
  it("holds the queue while the folk is busy and drains it once when idle", () => {
    const bot = fakeBot("folk-b", "thread-b", true);
    const store = fakeStore([bot]);
  ...
  it("drops a cancelled message so drain does not send it", () => {
    const bot = fakeBot("folk-cancel", "thread-cancel", true);
    const store = fakeStore([bot]);
  ...
  it("keeps reply metadata and the provider-facing reply prompt while queued", () => {
    const bot = fakeBot("folk-reply", "thread-reply", true);
    const store = fakeStore([bot]);
  ...
  it("cancels a pinned-task queue after the folk switches without crossing folk ownership", () => {
    const bot = fakeBot("folk-switch-cancel", "thread-original-cancel", true);
    const queued = queueSteeredMessage(bot.id, bot.threadId, "cancel on the old task");

    bot.threadId = "thread-new-cancel";
    expect(cancelSteeredMessage("some-other-folk", queued.id)).toBe(false);
  ...
  it("fires nothing when nothing is queued", () => {
    const run = vi.fn();
    drainSteeredMessages(fakeStore([fakeBot("folk-c", "thread-c", false)]), run);
    expect(run).not.toHaveBeenCalled();
  });

  it("drops the queue of a deleted folk without running it", () => {
    const bot = fakeBot("folk-d", "thread-d", true);
