// API smoke test: boots the real harness server (node apps/harness/index.ts)
  ...
  for (;;) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (health.app === "openfolks" && health.pid === serverChild.pid && health.static === true) return;
      }
    } catch {
      /* still starting */
    }
...
const storedMessageCount = (threadId: string): number => {
  const db = new DatabaseSync(join(home, ".openfolks", "messages.db"), { readOnly: true });
  try {
...
beforeAll(async () => {
  ...
  mkdirSync(join(home, ".openfolks"), { recursive: true });
  mkdirSync(join(staticDir, "assets"), { recursive: true });
  writeFileSync(join(staticDir, "index.html"), "<!doctype html><title>Packaged OpenFolks</title>");
  ...
  writeFileSync(
    join(home, ".openfolks", "groups.json"),
    JSON.stringify([
      {
        id: "test-dm",
        threadId: "test-dm-thread",
        name: "Private channel",
        memberIds: ["test-folk-a", "test-folk-b"],
      ...
      },
      {
        ...
        name: "Stranded room",
        memberIds: ["test-folk-a"],
        defaultResponder: { kind: "member", botId: "test-folk-a" },
        bulletin: "",
        unread: false,
        createdAt: 3,
      },
  ...
  writeFileSync(
    join(home, ".openfolks", "messages-test-stranded-room-thread.json"),
    JSON.stringify({
      ...
      messages: [
        {
          ...
          from: { botId: "test-folk-a", name: "Test folk A", color: "purple" },
        },
      ],
    }),
  );
  ...
  writeFileSync(
    join(home, ".openfolks", "messages-test-cancel-room-thread.json"),
    JSON.stringify({
...
describe("harness HTTP API", () => {
  ...
  it("identifies itself on /api/health", async () => {
    ...
    expect(body.app).toBe("openfolks");
  ...
  it("serves packaged UI assets and preserves API 404s", async () => {
    ...
    expect(await root.text()).toContain("Packaged OpenFolks");
    ...
    expect(await spa.text()).toContain("Packaged OpenFolks");
  ...
  it("seeds one starter folk with its greeting", async () => {
  ...
  it("rejects non-object folk and channel create bodies without writing records", async () => {
  ...
  it("lets a Chief create operators from its direct and channel tasks but not from channels it cannot access", async () => {
    ...
    try {
      ...
      const createOperator = async (fromThreadId: string, name: string, fromBotId = chief.id) => {
        const response = await fetch(`${BASE}/api/internal/create-folk`, {
          method: "POST",
      ...
      expect(nonChief).toEqual({
        status: 403,
        body: { error: "only a section's Chief of Staff can create operator folks" },
      });
  ...
  it("rejects null and array task, channel, and folk mutation bodies", async () => {
    const bot = (await api("POST", "/api/bots")).body.bot;
  ...
  it("keeps folk-to-folk channels single-threaded and blocks task changes on an open approval", async () => {
  ...
  it("searches transcripts and exports a conversation", async () => {
    const bot = (await api("POST", "/api/bots")).body.bot;
    // every new folk opens with a seeded greeting — a known searchable string
  ...
  it("creates, patches, and deletes a folk", async () => {
    const created = await api("POST", "/api/bots");
    expect(created.status).toBe(201);
    const bot = created.body.bot;
  ...
  it("rejects a sidebar section Chief collision without changing any folk", async () => {
    const incumbent = (await api("POST", "/api/bots")).body.bot;
    const incoming = (await api("POST", "/api/bots")).body.bot;
    const teammate = (await api("POST", "/api/bots")).body.bot;
    try {
  ...
  it("rejects malformed or unavailable sidebar section targets without partially filing folks", async () => {
  ...
  it("explains when archived room members cannot respond", async () => {
    ...
    try {
      ...
      expect(messages.at(-1)).toMatchObject({
        kind: "activity",
        tool: {
          name: "No active room members can respond — restore an archived folk or add an active member.",
          ok: false,
        },
      });
    } finally {
  ...
  it("persists only app-owned folk avatars and supported crop shapes", async () => {
    const created = await api("POST", "/api/bots");
    const bot = created.body.bot;
  ...
  it("exports every visible folk and imports the team without creating a room", async () => {
    ...
    expect(exported.body).toMatchObject({ format: "openfolks.team", version: 2, team: { name: "Field Team" } });
    ...
    expect((await api("POST", "/api/teams/export", {})).body.team.name).toBe("My OpenFolks Team");
    ...
    try {
      ...
      // until the user grants it per folk
      ...
      // every imported folk is announced to other windows. The store emits
      // on every write now, so a folk may produce more than one frame —
  ...
  it("imports a team as a project: one room, on a folder", async () => {
    ...
    try {
      ...
      // the room is made of exactly the folks this import created
  ...
  it("installs a complete folk package with a Chief, room, playbook, connector intent, and paused routine", async () => {
    const packageFile = {
      ...
      package: {
        ...
        summary: "A complete two-folk signal workflow.",
        category: "Research",
  ...
  it("keeps the rest of a duplicate's fields when the source engine is offline", async () => {
    // duplicateBot POSTs a blank folk, then PATCHes the source's whole
  ...
  it("leaves a folk with no effort level untouched", async () => {
    const bot = (await api("POST", "/api/bots")).body.bot;
  ...
  it("grants Auto on this computer only through the warning acknowledgement", async () => {
    ...
    // The important half: a blind PATCH — exactly what a folk curling the
  ...
  it("rejects an empty message and explains an unavailable provider", async () => {
    ...
    // the seeded folk's selection points at the ghost instance — sending a
  ...
  it("validates and persists the global room turn timeout", async () => {
    ...
    const disk = JSON.parse(readFileSync(join(home, ".openfolks", "config.json"), "utf8"));
  ...
  it("mounts the verification skill into a real turn when its trigger appears", async () => {
    ...
    try {
      ...
      expect(system).toContain('<openfolks-skill id="create-verification-skill"');
  ...
  it("mounts the verification skill only for the latest channel request", async () => {
    ...
    try {
      ...
      expect(system).not.toContain('<openfolks-skill id="create-verification-skill"');
      expect(system).not.toContain('<openfolks-skill id="phone-harness"');
    } finally {
      if (room) {
        expect((await api("POST", `/api/groups/${room.id}/interrupt`, {})).status).toBe(200);
        await expect.poll(async () => {
  ...
  it("refuses to delete a folk while it owns an active channel turn", async () => {
    const bot = (await api("POST", "/api/bots", {
      modelSelection: { instanceId: "claude", model: "claude-sonnet-5" },
      requireAvailableModel: true,
    })).body.bot;
  ...
  it("creates, edits, lists, and deletes scheduled multi-folk calls", async () => {
    ...
    try {
      ...
      expect((await api("GET", "/api/calendar-calls")).body.calls).toEqual(
        expect.arrayContaining([expect.objectContaining({ id: callId, name: "Weekly folk sync" })]),
      );
  ...
  it("refuses to delete a folk while one of its routines is active", async () => {
    const bot = (await api("POST", "/api/bots", {
      modelSelection: { instanceId: "claude", model: "claude-sonnet-5" },
      requireAvailableModel: true,
    })).body.bot;
  ...
  it("stops a local folk's exact channel and routine work through the emergency endpoint", async () => {
    const bot = (await api("POST", "/api/bots", {
      modelSelection: { instanceId: "claude", model: "claude-sonnet-5" },
      requireAvailableModel: true,
    })).body.bot;
    ...
    // same hanging provider, then mark the folk local immediately before the
  ...
  it("applies browser disable effects before reporting a removed-profile cleanup failure", async () => {
    ...
    const isolatedData = join(isolatedHome, ".openfolks");
          ...
          postMessage(message) {
            if (message?.requestId && /browser-(?:bot|profile)-deleted/.test(message.type ?? "")) {
              queueMicrotask(() => listener?.({ data: {
                type: "openfolks:browser-lifecycle-result",
  ...
  it("revokes live browser access even when clearing a removed profile reference cannot persist", async () => {
    ...
    try {
      ...
      // the resume-cursor save races the deliberately-invalid folks path.
      await expect.poll(() => {
        try {
  ...
  it("keeps chat-created routines inert until their durable card is confirmed", async () => {
    ...
    try {
      ...
      // A routine proposed "for another folk" binds to that folk, not the sender.
      const badTarget = await fetch(`${BASE}/api/internal/routine-requests`, {
        ...
        body: JSON.stringify({
          ...
          fromThreadId: bot.threadId,
          action: "create",
          forBotId: "folk-that-does-not-exist",
          routine: {
            name: "Nowhere brief",
      ...
      const wrongThread = await fetch(`${BASE}/api/internal/routine-requests`, {
        ...
        body: JSON.stringify({
          fromBotId: bot.id,
          fromThreadId: "not-this-folks-thread",
          action: "pause",
          routineId,
        }),
      });
  ...
  it("only enables the exact learned-skill proposal a current client reviewed", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(home, ".openfolks", "skill-state", bot.id, "staged.json"),
  ...
  it("validates the non-secret VPS alias and keeps old folks on Box by default", async () => {
  ...
  it.skipIf(process.platform === "win32")("stores the credentials file with owner-only permissions", () => {
    expect(statSync(join(home, ".openfolks", "config.json")).mode & 0o777).toBe(0o600);
  });
  ...
  it("creates an independent webhook, accepts a delivery, deduplicates it, and rotates its secret", async () => {
    ...
    if (process.platform !== "win32") {
      expect(statSync(join(home, ".openfolks", "webhooks.json")).mode & 0o777).toBe(0o600);
    }
  });

  it("stores OpenRouter credentials as a configured-only status", async () => {
    const put = await api("PUT", "/api/config", {
      openaiCompat: { key: "sk-or-secret", url: "https://api.groq.com/openai/v1" },
    });
    expect(put.status).toBe(200);
    expect(put.body.openaiCompat).toEqual({
      configured: true,
      url: "https://api.groq.com/openai/v1",
    });
    expect(JSON.stringify(put.body)).not.toContain("sk-or-secret");

    const after = await api("GET", "/api/config");
    expect(after.body.openaiCompat).toEqual({
      configured: true,
      url: "https://api.groq.com/openai/v1",
    });
    expect(JSON.stringify(after.body)).not.toContain("sk-or-secret");
  });
  ...
  it("never hands a client the provider session cursors", async () => {
    ...
    // session state leaving the machine, so nothing carrying a folk may have it.
    ...
    try {
      ...
      // the task alone, not just the folk it came attached to
...
// The memory routes expose plain files in the folk's workspace. The
...
describe("folk memory API", () => {
  ...
  const workspaceOf = (botId: string) => join(home, ".openfolks", "workspaces", botId);

  it("reads empty memory for a fresh folk and 404s a folk that does not exist", async () => {
    const bot = (await api("POST", "/api/bots")).body.bot;
    try {
  ...
  it("refuses every coat of path traversal without reading the target", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".openfolks", "secret.md"), "TOP-SECRET-MARKER sibling");
...
// Hydration is one call that returns every folk's entire transcript. Over
...
describe("computer control API (who is driving)", () => {
  ...
  it("refuses an unknown action and an unknown folk", async () => {
