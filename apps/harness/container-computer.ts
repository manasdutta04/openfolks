...
//
// OpenFolks owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/openfolks/cua-local-vm";
export const IMAGE_LAYER_VERSION = "4";
export const IMAGE_LAYER_LABEL = "com.openfolks.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "openfolks-computer";
export const MANAGED_LABEL = "com.openfolks.local-vm";
export const DRIVER_LABEL = "com.openfolks.cua-driver";
export const BASE_IMAGE_LABEL = "com.openfolks.cua-base";
export const WORKSPACE_LABEL = "com.openfolks.workspace";
export const TARGET_LABEL = "com.openfolks.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/openfolks-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/openfolks/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-openfolks-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-openfolks-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-openfolks-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-openfolks-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-openfolks-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:openfolks-cua-driver]' \\
      'command=/usr/local/bin/start-openfolks-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (status.container === "missing" && !status.create_supported) {
    return "Per-folk Local VMs require Docker or Podman because Apple container requires a fixed host port";
  }
  ...
  if (!status.managed) return "The existing container was not created by OpenFolks; recreate it";
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared OpenFolks derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/openfolks-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
export function containerRunArgs(
  ...
  if (runtime === "container" && target.key !== SHARED_LOCAL_VM_TARGET.key) {
    throw new Error("Per-folk Local VMs require Docker or Podman because Apple container requires a fixed host port");
  }
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "openfolks-cua-image-"));
  try {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/openfolks-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
/** Cloud boxes still use OpenFolks's high-latency REST adapter. Local VMs
