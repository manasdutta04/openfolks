...
export function loadBundledSkills(root = process.env.OMB_SKILLS_DIR || join(process.cwd(), "portals")): BundledSkill[] {
...
export function renderSkillInstructions(
  ...
  return selected.map(({ manifest, instructions, directory }) =>
    `\n\n<openfolks-skill id=${JSON.stringify(manifest.id)} version=${JSON.stringify(manifest.version)}${includeRoot ? ` root=${JSON.stringify(directory)}` : ""}>\n${instructions}\n</openfolks-skill>`,
  ).join("");
}
