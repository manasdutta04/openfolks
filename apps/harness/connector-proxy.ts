...
// into first-class OpenFolks chat cards. The agent never authors an auth
...
function initializeResult(id: unknown, protocolVersion: unknown): Json {
  return {
    ...
    result: {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "openfolks-connectors", version: "1" },
    },
  };
}
...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (slugs.length) {
      ...
      send(textResult(
        id,
        `OpenFolks showed the user a secure connection card for ${slugs.join(", ")}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
    if (/WAIT_FOR_CONNECTIONS$/i.test(name)) {
      send(textResult(id, "OpenFolks is handling connection completion and will continue the task automatically."));
      return;
    }
  }
  try {
