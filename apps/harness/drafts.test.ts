...
import {
  ...
  setDraft,
  setDraftAttachments,
} from "../desk/lib/drafts.ts";
import { fileAttachment, pasteAttachment } from "../desk/lib/composer-attachments.ts";
...
describe("composer drafts", () => {
  it("keeps text and attachments isolated per folk or room", () => {
    const store = memoryStore();
    const paste = pasteAttachment("folk paste");
    const file = fileAttachment("notes.txt", "/tmp/notes.txt", 12);
    setDraft(store, "folk:one", "hello");
    setDraftAttachments(store, "folk:one", [paste, file]);
    setDraft(store, "group:two", "room text");

    expect(getDraft(store, "folk:one")).toBe("hello");
    expect(getDraftAttachments(store, "folk:one")).toEqual([paste, file]);
  ...
  it("clears empty entries and ignores malformed stored attachments", () => {
    const store = memoryStore();
    setDraft(store, "folk:one", "hello");
    setDraft(store, "folk:one", "");
    store.setItem(
      "omb-draft-attachments",
      JSON.stringify({ "folk:one": [{ kind: "paste", id: "broken" }] }),
    );

    expect(getDraft(store, "folk:one")).toBe("");
    expect(getDraftAttachments(store, "folk:one")).toEqual([]);
  });

  it("restores a rejected send into its task draft", () => {
    const store = memoryStore();
    vi.stubGlobal("localStorage", store);
    const draftId = "folk:restore:thread-a";
  ...
  it("preserves the original send separately after a reply-only edit", () => {
    const store = memoryStore();
    vi.stubGlobal("localStorage", store);
    const draftId = "folk:reply-edit:thread-c";
