...
export interface DelegationItem {
  ...
  /** The source folk's comms depth (0 for a user-initiated turn). The
...
/** The durable terminal record of one handoff: what the delegating folk reads
...
/** Source threads currently waiting for this busy folk — the set its idle
...
async function processOne(
  ...
  if (!target) {
    ...
    bus.store.appendMessage(sourceThreadId, {
      role: "bot",
      kind: "activity",
      tool: { name: `error: delegation to ${item.toBotId} failed — no such folk`, ok: false },
    });
    return "settled";
  }
  ...
  const prefixed = `[Delegated by @${sender.name}, another folk in this OpenFolks workspace. Do the work and reply directly.]\n\n${item.message}${reasonLine}`;
...
function dropIfSectionsChanged(
  ...
  bus.store.appendMessage(sourceThreadId, {
    role: "bot",
    kind: "activity",
    tool: { name: `Delegation to @${target.name} canceled — folks now belong to different sections`, ok: false },
  });
  return true;
}
