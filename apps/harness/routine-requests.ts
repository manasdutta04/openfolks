...
function cardCopy(
  ...
  const destination = definition.runOn === "cloud" ? "Cloud VM" : "This OpenFolks setup";
  ...
  // through the folk's MCP response or into the transcript.
  ...
  return {
    ...
    detail: [
      ...
      `Name: ${name}`,
      ...(forBot ? [`For: @${redactSecretsInText(forBot.name)} — each run uses that folk's engine and permissions`] : []),
      `Schedule: ${when}`,
...
function verifyManageSnapshot(
  ...
  if (current.updatedAt !== operation.expectedUpdatedAt) {
    throw new RoutineRequestError(
      "That routine changed after this confirmation card was prepared. Ask the folk to review it and propose the action again.",
      409,
    );
  }
  return current;
...
function revalidateOperation(operation: RoutineRequestOperation, manager: RoutineManager, botId: string, now: number): void {
  ...
  if (schedule?.type === "once" && schedule.at <= now) {
    throw new RoutineRequestError("That one-time schedule is now in the past. Ask the folk to propose a new time.", 409);
  }
