...
import { openFolksStatusSystemPrompt, readOpenFolksStatus } from "./openfolks-status-capsule.ts";
...
function successCapsule(options: {
  ...
  return sign({
    schema: "aos.openfolks_status.v1",
...
function cachePath(capsule: TestCapsule): string {
  const root = mkdtempSync(join(tmpdir(), "openfolks-status-"));
  roots.push(root);
  const parent = join(root, "openfolks");
...
posixOnly("readOpenFolksStatus", () => {
  it("projects only fresh normalized two-VM capability data", () => {
    ...
    const status = readOpenFolksStatus({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
    ...
    const prompt = openFolksStatusSystemPrompt({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
  ...
  it("rejects a signed success capsule whose installed and accepted hashes differ", () => {
    ...
    expect(readOpenFolksStatus({ cachePath: path, now: NOW })).toMatchObject({
      freshness: "unknown",
      reason: "invalid",
  ...
  it("accepts more configured VM folks than the simultaneous instance limit", () => {
  ...
  it("turns stale, future-skewed, failed, and receipt-tampered state into unknown", () => {
    const path = cachePath(successCapsule());
    expect(readOpenFolksStatus({ cachePath: path, now: new Date("2026-08-22T06:35:00Z") })).toMatchObject({
      freshness: "stale", reason: "stale", runtimeState: "unknown", readyCount: 0,
      ui: { twoUp: false },
    });
    expect(readOpenFolksStatus({ cachePath: path, now: new Date("2026-08-22T06:29:59Z") })).toMatchObject({
      freshness: "unknown", reason: "clock_skew", runtimeState: "unknown", readyCount: 0,
    });

    const failurePath = cachePath(failedCapsule());
    expect(readOpenFolksStatus({ cachePath: failurePath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
    ...
    expect(readOpenFolksStatus({ cachePath: tamperedPath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
  ...
  it("rejects signed extra fields, insecure modes, and same-path symlinks", () => {
    ...
    expect(readOpenFolksStatus({ cachePath: extraPath, now: NOW }).reason).toBe("invalid");

    const insecurePath = cachePath(successCapsule());
    chmodSync(insecurePath, 0o644);
    expect(readOpenFolksStatus({ cachePath: insecurePath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkPath = join(dirname(targetPath), "latest-link.json");
    symlinkSync(targetPath, linkPath);
    expect(readOpenFolksStatus({ cachePath: linkPath, now: NOW }).reason).toBe("missing_or_insecure");
  });
  ...
  it("rejects a non-0700 parent and a symlinked parent", () => {
    ...
    expect(readOpenFolksStatus({ cachePath: looseParentPath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkRoot = mkdtempSync(join(tmpdir(), "openfolks-status-parent-link-"));
    roots.push(linkRoot);
    const linkedParent = join(linkRoot, "openfolks");
    symlinkSync(dirname(targetPath), linkedParent, "dir");
    expect(
      readOpenFolksStatus({ cachePath: join(linkedParent, "latest.json"), now: NOW }).reason,
...
it.skipIf(process.getuid !== undefined)(
  ...
  () => {
    ...
    expect(openFolksStatusSystemPrompt({ cachePath: path, now: NOW })).toContain(
      "runtime_state=unknown",
    );
  },
);
