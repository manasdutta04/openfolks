...
const desktopConnectionMessageSchema = z.object({
  type: z.literal("openfolks:browser-connection"),
 ...
 * a previous boot must never send a folk's actions to a recycled port. */
...
export function applyDesktopBrowserConnectionMessage(message: unknown): boolean {
  if (
    ...
    message === null ||
    (message as { type?: unknown }).type !== "openfolks:browser-connection"
  ) {
    return false;
  }
...
export function readBrowserConnection({
  ...
  if (!file) {
    if (userData) {
      ...
      for (const directory of ["OpenFolks", "openfolks"]) {
...
/** One frame of a folk's browser for the preview pipeline (SSE `screen`
