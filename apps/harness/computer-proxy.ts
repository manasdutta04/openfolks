...
// no argv-dispatch fork-bomb hazard). It gives the agent its folk's cloud
...
const CHROME_PROFILE = "$HOME/.openfolks/chrome-profile";
...
const CHROME_PROFILE_SETUP = [
  ...
  "      exit 1",
  "    fi",
  '    mv "$browser_dir" "$browser_dir.pre-openfolks-$(date +%s)-$$"',
  "  fi",
...
const TOOLS = [
  {
    name: "screenshot",
    description:
      "See the folk's cloud computer screen when visual state is needed. First prefer browser_state for Chrome title/URL checks. The frame is captured fresh; byte-identical pixels are not resent.",
    inputSchema: {
      type: "object",
      properties: {
        region: {
  ...
  },
  {
    name: "computer_exec",
    description:
      "Run a shell command on the folk's cloud computer (Linux, passwordless sudo, X11 desktop). Returns stdout/stderr/exit code — and, unlike the UI tools, no screenshot unless you ask for one.",
    inputSchema: {
      type: "object",
      properties: {
        command: { type: "string" },
  ...
  },
  {
    name: "wait_for",
    description:
      "Wait on the folk's cloud computer until a condition holds, then return in ONE call instead of repeatedly polling with computer_exec or screenshots. Give only the fields needed by the chosen condition.",
    inputSchema: {
      type: "object",
      properties: {
        condition: {
...
/** The only tools a folk may use while the person is driving: asking to be
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "openfolks-computer", version: "3" },
      },
    });
  }
