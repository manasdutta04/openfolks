...
describe("default fleet", () => {
  it("ships Hermes and Pi as custom-only engines", () => {
...
describe("customMcpServers", () => {
  ...
  it("mounts url transports through the stdio MCP bridge", () => {
    const out = customMcpServers(cfg({ api: { url: "https://mcp.example/mcp", headers: { Authorization: "Bearer x" } } }));
    expect(out.api?.command).toBe(process.execPath);
    expect(out.api?.args[0]).toMatch(/mcp-url-proxy\.(?:ts|js)$/);
    expect(out.api?.env.OMB_MCP_UPSTREAM_URL).toBe("https://mcp.example/mcp");
    expect(JSON.parse(out.api!.env.OMB_MCP_UPSTREAM_HEADERS!)).toEqual({ Authorization: "Bearer x" });
  });

  it("rejects cleartext remote http URL MCP servers", () => {
    expect(customMcpServers(cfg({ api: { url: "http://example.com/mcp" } }))).toEqual({});
  });
