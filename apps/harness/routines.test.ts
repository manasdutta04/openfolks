...
describe("RoutineManager", () => {
    ...
    expect(h.failed).toMatchObject([
      {
        ...
        status: "failed",
        threadId: "thread-1",
        error: "OpenFolks restarted while this routine was running",
      },
    ]);
  });
  ...
  it("queues behind a busy folk, then dispatches into a detached task", async () => {
    const h = harness();
    h.setBot("busy");
    const routine = h.manager.create({
      name: "Review queue",
  ...
  it("opens webhook jobs in the assigned folk's live chat", async () => {
    const h = harness();
