...
const readinessProbe =
  ...
  "--screenshot-out-file /tmp/openfolks-readiness.png";
const readinessRead = `docker exec ${CONTAINER} base64 -w0 /tmp/openfolks-readiness.png`;
...
describe("containerComputerStatus", () => {
  ...
  it("accepts exact Podman-on-Windows hardening and its WSL-translated durable mount", async () => {
    const derived = perBotLocalVmTarget("folk-win");
    const target: LocalVmTarget = {
      ...derived,
      workspaceDir: "C:\\Users\\light\\.openfolks\\vm-homes\\win-target",
    };
    const detail = JSON.parse(perBotReadyInspect("folk-win", 41629))[0];
    detail.Mounts[0].Source = "/mnt/c/Users/light/.openfolks/vm-homes/win-target";
    detail.HostConfig = {
      ...detail.HostConfig,
    ...
    const fake = runner({
      ...
      [`${targetDriverExec} call get_desktop_state {} --socket ${CUA_SOCKET} --screenshot-out-file /tmp/openfolks-readiness.png`]: "{}\n",
      [`podman exec ${target.containerName} base64 -w0 /tmp/openfolks-readiness.png`]: validPng.toString("base64"),
    });
  ...
  it("keeps per-bot identities, workspaces, and ephemeral viewer ports separate", async () => {
    const target = perBotLocalVmTarget("folk-a");
    const targetDriverExec =
      `docker exec -u cua -e HOME=/home/cua -e DISPLAY=:1 -e CUA_DRIVER_INSTALL_CHANNEL=python_package ` +
      `-e CUA_DRIVER_RS_TELEMETRY_ENABLED=0 ${target.containerName} ${CUA_EXECUTABLE}`;
    const fake = runner({
      ...
      [`docker inspect ${target.containerName}`]: perBotReadyInspect("folk-a", 49152),
      [`${targetDriverExec} --version`]: `cua-driver ${CUA_DRIVER_VERSION}\n`,
      [`${targetDriverExec} status --socket ${CUA_SOCKET}`]: "running\n",
      [`${targetDriverExec} call health_report {} --socket ${CUA_SOCKET}`]: JSON.stringify({
        schema_version: "1",
      ...
      [`${targetDriverExec} call get_desktop_state {} --socket ${CUA_SOCKET} --screenshot-out-file /tmp/openfolks-readiness.png`]: "{}\n",
      [`docker exec ${target.containerName} base64 -w0 /tmp/openfolks-readiness.png`]: validPng.toString("base64"),
    });
  ...
  it("refuses a per-bot container carrying another target's label", async () => {
    const target = perBotLocalVmTarget("folk-a");
    const other = perBotLocalVmTarget("folk-b");
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      "docker info --format {{.ServerVersion}}": "29\n",
      [`docker image inspect ${IMAGE}`]: preparedImageInspect(),
      [`docker inspect ${target.containerName}`]: perBotReadyInspect("folk-a", 49152, other.label),
    });
    ...
    expect(status.problem).toContain("not created by OpenFolks");
  });
  ...
  it("rejects missing or unexpected host mounts instead of exposing them to the folk", async () => {
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      "docker info --format {{.ServerVersion}}": "29\n",
...
describe("Cua integration", () => {
  ...
  it("builds an exact, checksum-verified Cua Driver 0.20.0 image", () => {
    ...
    expect(dockerfile).toContain("prepare-openfolks-workspace.sh");
  ...
  it("captures the preview through Cua Driver rather than xdotool or VNC", async () => {
    const screenshotCall =
      `${driverExec} call get_desktop_state {} --socket ${CUA_SOCKET} ` +
      "--screenshot-out-file /tmp/openfolks-preview.png";
    const png = validPng;
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      ...
      [screenshotCall]: "{}\n",
      [`docker exec ${CONTAINER} base64 -w0 /tmp/openfolks-preview.png`]: png.toString("base64"),
    });
...
describe("setupCommands", () => {
  it("derives opaque, distinct per-bot container and workspace identities", () => {
    const a = perBotLocalVmTarget("folk-a");
    const b = perBotLocalVmTarget("folk-b");

    expect(a).toEqual(perBotLocalVmTarget("folk-a"));
    expect(a.key).not.toBe(b.key);
    expect(a.containerName).not.toBe(b.containerName);
    expect(a.workspaceDir).not.toBe(b.workspaceDir);
    expect(a.containerName).not.toContain("folk-a");
    expect(a.workspaceDir).not.toContain("folk-a");
  });
  ...
  it("uses an explicit local image name so Podman never resolves the managed build on Docker Hub", () => {
    expect(IMAGE).toMatch(/^localhost\/openfolks\/cua-local-vm:/);
    expect(setupCommands("podman", "darwin").run).toContain(IMAGE);
    expect(setupCommands("podman", "darwin").run).not.toContain("docker.io/openfolks");
  });
