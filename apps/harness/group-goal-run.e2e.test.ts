...
const completeReplies = [
  [
    "Scout should verify the draft.\n<openfolks-goal>{\"status\":\"continue\",",
    "\"next\":\"Scout\",\"instruction\":\"Verify the draft and report evidence\",\"detail\":\"Draft prepared\"}</openfolks-goal>",
  ],
  "The draft is accurate and the cited evidence checks out.",
  "The verified draft is ready to ship.\n<openfolks-goal>{\"status\":\"completed\",\"detail\":\"Draft produced and independently verified.\"}</openfolks-goal>",
];

const loopReplies = Array.from({ length: 13 }, (_, index) =>
  index % 2 === 0
    ? `More work is needed.\n<openfolks-goal>{"status":"continue","next":"Looper","instruction":"Try approach ${index / 2 + 1}","detail":"Still working"}</openfolks-goal>`
...
beforeAll(async () => {
  ...
  const data = join(home, ".openfolks");
...
describe("goal-driven channel runs", () => {
  it("returns to the lead until the goal is completed and appends a clean receipt", async () => {
    ...
    expect(JSON.stringify(current.messages)).not.toContain("<openfolks-goal>");
  });
