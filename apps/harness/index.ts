// OpenFolks server — the harness host. Clients hold no transports
...
import { isPebbleSeed } from "../../shared/pebble-avatars.ts";
import { escapeAttribute } from "../desk/lib/composer-attachments.ts";
import {
  CREDENTIAL_TARGETS,
...
import { openFolksStatusSystemPrompt } from "./openfolks-status-capsule.ts";
import {
...
import { checkToolchain, toolchainSummary } from "./openfolks/toolchain.ts";
import { createFirstFolk } from "./openfolks/first-folk.ts";
import { importOpenFolksCrew, prepareProjectFolder } from "./openfolks/import-crew.ts";
import { importBotPackage } from "./team-import-package.ts";
import { isOpenFolksOnboardingComplete, markOpenFolksOnboardingComplete, openfolksOnboardingState } from "./openfolks/onboarding.ts";
import { defaultProjectFolder } from "./openfolks/paths.ts";

const PORT = Number(process.env.OPENFOLKS_PORT || process.env.OMB_PORT || process.env.OGB_PORT || 8799);
...
type DesktopPrivateMessage = BrowserCleanupWireRequest | {
  type: "openfolks:browser-control";
  botId: string;
  held: true;
};
 ...
 * folk/profile capability, plus the who-is-driving endpoint so a person
 * taking the wheel in the panel pauses the folk's hands. */
...
async function browserIntegration(
  ...
  // A profile that no longer exists falls back to the folk's own session.
  // Canonical ids belong to config/folk references; Electron must receive the
  // exact immutable partition inherited from #567 so an upgrade cannot move
  // a folk into another account. Guest remains a throwaway partition.
...
// The person can take the wheel of a folk's computer from the panel; while
// they hold it, the folk's computer proxies refuse every action. The record
...
const computerControl = new ComputerControl((botId, snapshot) => {
  ...
  if (snapshot.held && /^[A-Za-z0-9_-]{1,120}$/.test(botId)) {
    postDesktopPrivateMessage({ type: "openfolks:browser-control", botId, held: true });
  }
...
/** The loopback endpoint a folk's computer proxy polls before acting. */
...
function askBotAndWait(targetBotId: string, message: string, depth: number, fromBotId?: string): Promise<AskBotOutcome> {
  ...
  if (!target) return Promise.resolve({ status: "error", text: "(no such folk)" });
  ...
  return new Promise((resolve) => {
    ...
    const unsub = bus.subscribe((e: RuntimeEvent) => {
      ...
      if (e.type === "item.completed" && e.itemType === "assistant_text") {
        ...
        if (e.ok) finish({ status: "reply", text: text || "(the folk finished without a text reply)" });
    ...
    startTurn(targetBotId, message, {
      ...
      onDispatchError: (reason) => finish({ status: "error", text: `(couldn't start that folk: ${reason})` }),
    }).catch((err) =>
      finish({ status: "error", text: `(couldn't start that folk: ${err instanceof Error ? err.message : String(err)})` }),
    );
  });
}

// default selection for new folks: first available instance, claude preferred
async function defaultSelection() {
  const described = await registry.describe();
  const available = described.filter((d) => d.snapshot.state === "available");
  // Deliberately NO fallback to described[0]. Handing a folk an engine whose
...
function checkedModelSelection(
  ...
  if (current?.busy && changed) {
    return { ok: false, status: 409, error: "the folk is working — stop it before changing models" };
  }
  ...
  if (target && selection.effort !== undefined && !allowed.includes(selection.effort)) {
    return { ok: false, status: 400, error: `effort "${selection.effort}" is not offered by this folk's engine` };
  }
...
function checkedMemberIds(value: unknown): { ok: true; memberIds: string[] } | { ok: false; error: string } {
  if (!Array.isArray(value)) return { ok: false, error: "memberIds must be a list of folk IDs" };
  ...
  if (!memberIds.length) return { ok: false, error: "a channel needs at least one folk" };
  ...
  if (memberIds.every((id) => store.bot(id)?.hidden)) {
    return {
      ok: false,
      error: "a channel needs at least one active folk — every member given is archived",
    };
  }
...
// OpenFolks: blank first folk on recovery — Job Search stays a Marketplace template.
if (store.bots.length === 0 && isOpenFolksOnboardingComplete()) {
  createFirstFolk(store, bootSelection);
}
// A committed profile cleanup means both its config deletion and folk-reference
...
// in an older build must not let id reuse attach a folk to somebody else's new
...
/** A folk as a client may see it: no provider session bookkeeping.
 *
...
// broadcast. Folk frames are the slim wire shape (no transcript); the few
...
// ── message pages ──────────────────────────────────────────────────────
// GET /api/bots hands back every folk with its entire transcript, which is
// the right answer over loopback and the wrong one over a phone network:
// a long-running folk's thread is megabytes, and a turn-end desktop capture
...
interface SseClient {
   ...
   * while a folk works. A client that isn't showing the computer panel —
...
// item/request ids are only unique within a thread, so two folks acting at
...
// can carry what the folk actually said
...
function notify(notification: Notification | null) {
  ...
  // exactly like {kind:"message", message} and {kind:"bot", folk}
...
// left its folk busy forever. The watchdog stops a turn whose thread has emitted NOTHING for stallMs —
  ...
  onStall: (turn) => {
    ...
    const releaseOwnership = () => {
      ...
      if (currentBot?.busy) {
        ...
        // every kind of work that may have queued behind this folk, including
...
// Folks currently working with nobody at the keyboard — a webhook turn, or a
// turn a webhook-driven folk handed to a teammate. Auto mode is a decision
...
//
// Keyed by FOLK rather than thread because a folk runs one turn at a time, so
...
// flag before the hop that needs to read it. A busy folk never ages out, and a
...
function isUnattended(botId?: string | null): boolean {
  ...
  // more than 30 minutes after the previous one. Only an idle folk may age
...
// A restore mutates and cleans a project work tree. Claim the folk across the
  ...
  switch (event.type) {
    ...
    case "item.completed":
      if (event.itemType === "assistant_text") {
        ...
        // the folk just acted ON ITS SCREEN — refresh the preview now. Only
        // computer tools can change the screen, and each capture competes
        // with the agent for the box's command endpoint, so a folk grinding
    ...
    case "request.opened": {
      ...
      // folk so it keeps working. A QUESTION always reaches the human — the
      ...
      if (verdict?.approve && asker && event.requestId) {
        ...
        void (async () => {
          try {
            ...
            // rather than leaving the folk waiting on nobody
      ...
      if (
        ...
        // to a provider the user did not choose for this folk.
      ...
      const notifyHuman = () => {
        ...
        // the folk is not working now — it is waiting on a person
    ...
    case "turn.retrying":
      ...
      // the activity chip keeps the folk visibly busy through the backoff
      pushMessage({
        role: "bot",
        kind: "activity",
    ...
    case "runtime.error":
      ...
      // a setup error means the engine could not even start: the folk is
    ...
    case "turn.completed": {
      ...
      // group turns run on the room's thread — the speaking folk's task
      ...
      if (bot) {
        ...
        // bank what this turn spent before the folk broadcast carries the
        ...
        // hidden execution task should not light up the folk's 1:1 sidebar too.
        if (!routineReportGroup) store.patchBot(bot.id, { unread: true });
        if (routineRun?.status !== "failed") {
          // the frame carries the folk's avatar so every desktop client can
          // show the notification under that folk's own face
        ...
        if (screenPollers.has(bot.id)) {
          ...
          // still be in flight).
          //
          // The capture is slow (a real screenshot round trip) and the folk
          ...
          void finalScreenFrame(bot.id).then((frame) => {
            // the folk may have been deleted while the capture ran
...
const EXTERNAL_CONTEXT_MARKER_PREFIX = "__openfolks_external_context__:";
 ...
 * Some harness paths settle a busy folk without a provider turn.completed
...
function finalizeDelegationWatch(
  ...
  // folk's check/wait_delegation must see a terminal state even when the
...
// A folk going in circles — the same call with the same arguments, over and
...
const runDelegatedTurn: Parameters<typeof drainDelegations>[3] = (toBotId, text, commsDepth, sourceThreadId, channel, taskId) => {
    // startTurn REJECTS on an ordinary condition — busy target, deleted folk,
    ...
    return startTurn(toBotId, text, {
      ...
      // startTurn schedules provider/integration setup after marking the folk
...
// folk without that event, so every explicit idle release calls this same
// coalesced retry hook. The microtask lets the releasing state machine finish
// before another turn claims the folk.
...
bus.subscribe((event: RuntimeEvent) => {
  ...
  // A settling folk frees itself as a delegation TARGET too: handoffs that
...
// ── steer-queue drain: messages sent while the folk was busy ────────────
// Runs on ANY turn.completed rather than resolving the settling thread: a
// folk busy in a room settles on the room's thread, and by the time this
// subscriber runs the main fold has already dropped the speaker record —
// so the drain matches on "this queue's folk is idle now" instead.
...
// delegation drain above): queued delegations are a folk's fan-out and
...
function drainQueuedSends() {
  drainSteeredMessages(store, (botId, threadId, prompt, userMessage, excludeIds) =>
    ...
    // depth: exactly what typing the same words into an idle folk would run.
...
// ── live screen: poll the folk's computer while it works ───────────────
// Frames stream to clients as SSE {kind:'screen'} (the "Folk's screen"
...
const screenPollers = new Map<
  string,
  {
    ...
    last: Frame | null;
    /** Did this turn actually reach for the screen? A folk that merely HAS
...
/** Event-driven refresh: capture NOW (the folk just acted on its screen)
...
async function startTurn(
  ...
  opts?: {
    ...
    threadId?: string;
    /** Cloud routines run the whole agent inside the folk's Box VM instead
  ...
  const bot = store.bot(botId);
  if (!bot) throw Object.assign(new Error("no such folk"), { status: 404 });
  if (checkpointRestoreLeases.has(botId)) {
    throw Object.assign(new Error("this folk's project files are being restored — wait for the restore to finish"), {
      status: 409,
    });
  }
  if (bot.busy) throw Object.assign(new Error("the folk is already working — interrupt it first"), { status: 409 });
  const threadId = opts?.threadId ?? bot.threadId;
  // a webhook turn, or one inherited from a folk already running unattended
  if (opts?.automationSource === "webhook" || opts?.unattended) markUnattended(bot.id);
  // a person typing into this folk ends the unattended window immediately
  ...
  if (effort && !instance.adapter.capabilities.effortLevels?.includes(effort)) {
    throw Object.assign(
      new Error(`effort "${effort}" is not offered by this folk's engine — choose another level in settings`),
      { status: 409 },
    );
  }
  ...
  // A fresh engine — the user switched this folk's model mid-thread — has no
  ...
  const persona = [
    `You are ${bot.name}, a personal folk in OpenFolks.`,
  ...
  void (async () => {
    try {
      ...
      // this engine can reach them — and only to a folk the user has not
      // switched off: the key is workspace-wide, the grant is per folk.
      ...
      // CLI engines work inside the folk's own workspace directory rather
      // than the user's home: a folk with file tools and acceptEdits gets a
      ...
      // the private folk workspace. A legacy task with an existing provider
      ...
      // folk's folder for a task that runs elsewhere.
      ...
      // user's work. Its private OpenFolks workspace is app-owned and changes
      ...
      if (wants === "vm") {
        ...
        if (!cua) throw new Error("CUA Driver is not ready for this computer — check permissions and restart OpenFolks");
      ...
      const coordinationPrompt = bot.chiefOfStaff
        ? chiefOfStaffSystemPrompt(
            ...
            openFolksStatusSystemPrompt(),
          )
        : integrations.agents && sectionPeers.length > 0
          ? "You can work with the other folks in your section through the agents tools. list_bots shows who's available. Use delegate_bot for assigned or independent work so you remain available; use ask_bot only for a short consultation whose reply is required in your current answer."
          : "";
      ...
      const dispatch = await guardTurnDispatch(instance.adapter.sendTurn({
        ...
        system:
          ...
          // folk whose driver actually mounted the tools
...
// ── routines: persisted definitions → detached folk tasks ───────────────
...
async function cloudRoutineReadiness(): Promise<{ ready: boolean; reason?: string }> {
  ...
  if (!instance) {
    return { ready: false, reason: "The Cloud VM runner is unavailable. Restart OpenFolks and try again." };
  }
  try {
...
const routineRequests = new RoutineRequestService({
  ...
  // Cross-folk routines: the confirmation card can sit open indefinitely, so
...
try {
  ...
  console.log(`openfolks webhook receiver on ${webhookIngress.baseUrl}`);
} catch (error) {
  webhookIngressError = error instanceof Error ? error.message : String(error);
  console.error(`openfolks webhook receiver unavailable: ${webhookIngressError}`);
}
...
// re-checked there as always; a source folk that no longer exists is skipped.
_loadPending();
{
...
async function runGroupMemberTurn(
  ...
  botId: string,
  hop: number,
  // folks that already spoke for this user message — "@Scout ask @Pixel"
  ...
  // One turn per folk at a time, across BOTH engines. Without this a folk
  ...
  // the folk. An interrupt during that setup window must still stop the queued
  // room operation before it starts a process.
  if (isCancelled?.()) return false;
  // A 1:1 or another room turn may have claimed this folk while connected-app
  // setup was in flight. Re-check immediately before the synchronous claim so
  // one folk can never own two provider processes.
  ...
  // claimed the fresh folk record so a deleted profile cannot be resurrected
  ...
  const roster = group.memberIds
    ...
    .join(", ");
  const system = [
    `You are ${bot.name}, a folk in the room "${group.name}" in OpenFolks.`,
    bot.title && `Role: ${bot.title}.`,
    bot.description && `About: ${bot.description}`,
  ...
  // conversation, not a different folk
  const worksInWorkspace = instance.driverKind !== "grok" && instance.driverKind !== "boxAgent";
  ...
  if (outcome === "dispatch_failed") {
    ...
    // queued while this folk briefly owned the room must be retried now.
    drainQueuedSends();
    drainConnectorResumes();
    drainSecretResumes();
  }
...
function startGroupTurn(
  ...
  const goalCoordinator = channelMode === "goal"
    ...
    : null;
  // folk⇄folk channels: chipping in without a tag addresses the last speaker
  ...
  if (!responders.length && !goalCoordinator) {
    ...
    if (!mentionedArchived && !availableMembers.length) {
      unavailableMessage = "No active room members can respond — restore an archived folk or add an active member.";
...
function deliverCalendarCall(call: CalendarCall, scheduledFor: number): void {
  // A one-folk calendar entry remains a reminder that opens that folk's chat.
  // Multi-folk entries are rooms and begin with the shared event prompt.
 ...
 * are folk-scoped and live outside the thread, so deleting the only card
...
function resolveSkillRequest(args: {
  ...
  if (request.botId !== args.botId) {
    return { claimed: true, status: 403, error: "this skill request belongs to a different folk" };
  }
  ...
  if (typeof request.preview !== "string" || typeof request.sha256 !== "string") {
    return {
      claimed: true,
      status: 409,
      error: "this proposal was created by an older build — deny it and ask the folk to create it again",
    };
  }
...
function dispatchConnectorResume(entry: { botId: string; threadId: string; resumeKey: string; labels: string[] }) {
  ...
  const prompt = `OpenFolks connection update: the user securely connected ${names}. Continue the task that paused for this connection. Do not ask them to connect it again.`;
  if (owner.bot.busy) {
...
function dispatchSecretResume(entry: SecretResumeEntry) {
  ...
  const prompt =
    entry.outcome === "provided"
      ? `OpenFolks credential update: the user securely provided ${entry.label}. Continue the task that paused for it. You do not receive the secret and must not ask them to paste it into chat.`
      : `OpenFolks credential update: the user declined to provide ${entry.label}. Continue without it if possible, or briefly explain the limitation. Do not ask them to paste it into chat.`;
  if (owner.bot.busy) {
...
function configStatus() {
  return {
    ...
    openfolks: openfolksOnboardingState(),
    mcpServers: listMcpServersPublic(cfg),
  };
}
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    // The agents-proxy (spawned inside a folk's agent process) calls these to
    ...
    if (path.startsWith("/api/internal/")) {
      ...
      if (method === "POST" && path === "/api/internal/routine-requests") {
        ...
        if (body.action === "create" && body.forBotId !== undefined) {
          ...
          if (!forBotId) {
            return json(res, 400, { error: 'for_bot_id must be a bot id from list_bots, e.g. { "for_bot_id": "folk-abc123" }' });
          }
          if (forBotId !== from.id) {
            const target = store.bot(forBotId);
            if (!target) {
              return json(res, 404, { error: "no folk with that id — call list_bots and copy the exact id from the result" });
            }
            if (sectionKey(target.section) !== sectionKey(from.section)) {
              return json(res, 403, { error: "that folk belongs to a different section" });
            }
      ...
      if (method === "POST" && path === "/api/internal/ask-folk") {
        const body = await readBody(req);
        const fromBotId = String(body.fromBotId ?? "");
        ...
        if (toBotId === fromBotId) return json(res, 400, { error: "a folk cannot message itself" });
        if (depth >= MAX_COMMS_DEPTH) return json(res, 200, { error: "message chains are limited to one hop" });
        const target = store.bot(toBotId);
        if (!target) return json(res, 404, { error: "no such folk" });
        ...
        // the exchange is mirrored into a folk⇄folk channel: it shows up in
        ...
        if (from.approvePeerComms) {
          ...
          if (!freshFrom || !freshTarget) return json(res, 404, { error: "no such folk" });
          if (sectionKey(freshFrom.section) !== sectionKey(freshTarget.section)) {
            return json(res, 200, { error: "that folk moved to a different section" });
          }
        ...
        const prefixed = `[Message from @${currentFrom.name}, another folk in this OpenFolks workspace. Reply to them.]\n\n${message}`;
        ...
        if (outcome.status === "timeout" && !delegationWatch.has(currentTarget.threadId)) {
          ...
          // Losing the reply was the old behavior, and it read as "the folks
        ...
        if (outcome.status === "failed" && !outcome.text.trim()) {
          ...
          return json(res, 200, { botName: currentTarget.name, text: `(the folk's turn failed${why})` });
        }
        const reply = outcome.status === "timeout"
          ? outcome.text || "(timed out waiting for the folk to reply)"
          : outcome.text;
        mirrorReply(commsBus, currentTarget, reply, channel);
        return json(res, 200, { botName: currentTarget.name, text: reply });
      }
      // Async handoff: the source folk queues a task for a peer and goes
      ...
      if (delegationMatch) {
        ...
        // Bounded long-poll: the delegating folk parks ONE cheap HTTP request
      ...
      if (method === "POST" && path === "/api/internal/delegate-folk") {
        const body = await readBody(req);
        const fromBotId = String(body.fromBotId ?? "");
        const toBotId = String(body.toBotId ?? "");
        const message = String(body.message ?? "").trim();
        ...
        if (!toBotId || !message) return json(res, 400, { error: "toBotId and message required" });
        const from = store.bot(fromBotId);
        if (!from) return json(res, 404, { error: "no such folk" });
        const target = store.bot(toBotId);
        if (!target) return json(res, 404, { error: "no such folk" });
        if (sectionKey(from.section) !== sectionKey(target.section)) {
          return json(res, 403, { error: "that folk belongs to a different section" });
      ...
      if (method === "POST" && path === "/api/internal/create-folk") {
        const body = await readBody(req);
        const fromBotId = String(body.fromBotId ?? "");
        ...
        if (!chief.chiefOfStaff) {
          return json(res, 403, { error: "only a section's Chief of Staff can create operator folks" });
        }
        if (store.bots.length >= MAX_WORKSPACE_BOTS) {
          return json(res, 409, { error: `this workspace is limited to ${MAX_WORKSPACE_BOTS} folks` });
        }
      ...
      // ── computer control: proxies read the hold, folks plead for help ──
      if (path === "/api/internal/computer-control") {
        const botId = url.searchParams.get("botId") ?? "";
        const bot = store.bot(botId);
        if (!bot) return json(res, 404, { error: "no such folk" });
        if (method === "GET") {
        ...
        if (method === "POST") {
          ...
          // worth a buzz: the folk is blocked on the person's hands, which
      ...
      if (method === "POST" && path === "/api/internal/connectors/request") {
        ...
        if (!owner) return json(res, 403, { error: "conversation does not belong to this folk" });
        ...
        if (!composio.configured(cfg) || owner.bot.composio === false) {
          return json(res, 409, { error: "connected apps are not enabled for this folk" });
        }
        ...
        for (const slug of slugs) {
          ...
          const message = store.appendMessage(threadId, {
            ...
            connector: {
              slug,
              label: toolkit.label,
              description: toolkit.blurb || `Connect ${toolkit.label} so the folk can continue`,
    ...
    if (calendarCallRoomMatch && method === "POST") {
      ...
      if (call.botIds.length < 2) {
        return json(res, 400, { error: "single-folk events open that folk's chat directly" });
      }
      const group = ensureCalendarCallRoom(call);
    ...
    // relay never has to expose the rest of OpenFolks's control surface.
    ...
    // ── shared files ────────────────────────────────────────────────────
    // Clients may send documents as raw bytes. saveFile writes each
    ...
    // index to maintain. Hits resolve to the folk/room that owns the thread;
    ...
    if (m && method === "GET") {
      ...
      for (const msg of messages) {
        const who = msg.role === "user" ? userName : (msg.from?.name ?? bot?.name ?? "Folk");
    ...
    if (method === "POST" && path === "/api/teams/export") {
      ...
      if (memberIds.length === 0) return json(res, 400, { error: "Create a folk before exporting your team" });
      try {
    ...
    if (method === "GET" && path === "/api/teams/scout") {
      ...
      // nothing. Folks and the room come into being only when the human sends
      ...
      // trust boundary as pointing any folk's working folder at a path.
    ...
    if (method === "GET" && path === "/api/teams/scout/directory") {
      ...
      try {
        ...
        console.warn("folk directory lookup failed:", error instanceof Error ? error.message : String(error));
      }
      return json(res, 200, { directory });
    }
    if (method === "GET" && path === "/api/bots/directory") {
      // Full community directory for Marketplace → Bots browse. Unreachable
      // network degrades to an empty list so the team catalog still works.
      try {
        return json(res, 200, { bots: await fetchBotDirectory() });
      } catch (error) {
        console.warn("folks directory lookup failed:", error instanceof Error ? error.message : String(error));
        return json(res, 200, { bots: [] });
      }
    }
    if (method === "POST" && path === "/api/teams/import") {
      ...
      try {
        ...
        return json(res, 400, { error: error instanceof Error ? error.message : "Invalid folk package" });
      }
      const pkg = packageDocument?.package;
      let packageCwd: string | null = null;
      if (pkg && url.searchParams.has("cwd")) {
        const validated = validateBotCwd(url.searchParams.get("cwd"));
        if (!validated.ok) return json(res, 400, { error: validated.error });
        packageCwd = validated.cwd;
      }
      ...
      try {
        ...
        if (pkg) {
          const imported = importBotPackage({
            store,
            pkg,
            selection,
            cwd: packageCwd,
          ...
          importedBots.push(...imported.importedBots);
          createdGroups.push(...imported.createdGroups);
          createdRoutineIds.push(...imported.routineIds);
        } else {
          const sourceMembers = manifest!.team.members.map((member) => ({ member, playbookKeys: [] as string[] }));
        ...
        // Legacy team manifests: project mode opens a caller-owned room on a folder.
        ...
        for (const created of createdGroups) broadcast({ kind: "group", group: publicGroupState(created) });
    ...
    if (m && method === "POST") {
      ...
      if (group.dm) return json(res, 400, { error: "folk-to-folk channels keep one canonical conversation" });
    ...
    if (m && method === "PATCH") {
      ...
      // same contract as a folk's sidebar section: null/"" clears, 60 chars max
    ...
    if (m && method === "POST") {
      ...
      if (group.dm && channelMode === "goal") {
        return json(res, 400, { error: "goal mode is available in team channels, not folk-to-folk channels" });
      }
    ...
    if (method === "POST" && path === "/api/sidebar-sections") {
      ...
      if (!result.ok) {
        ...
        return json(res, 404, { error: "one or more folks are unavailable" });
      }
      // This files folks under a derived label; it does not create a durable
    ...
    if (method === "POST" && path === "/api/bots") {
      const body = await readBody(req);
      if (!body || typeof body !== "object" || Array.isArray(body)) {
        return json(res, 400, { error: "folk must be a JSON object" });
      }
    ...
    if (m && method === "POST") {
      ...
      if (!existing) return json(res, 404, { error: "no such folk" });
      // Generation is slow and other clients may edit or
      // delete this folk while it is in flight. Snapshot the two fields this
      ...
      if (!current) return json(res, 404, { error: "no such folk" });
      ...
      if (!bot) {
        ...
        return json(res, 404, { error: "no such folk" });
      }
    ...
    if (m && method === "POST") {
      ...
      if (!pending) {
        return json(res, 409, { error: "that grant is not on a pending approval for this folk" });
      }
    ...
    if (m && method === "PATCH") {
      ...
      // request: this is the app's general-purpose folk endpoint, and
      // duplicateBot re-sends the source folk's entire modelSelection beside
      ...
      if (body.avatarSeed !== undefined) {
        if (body.avatarSeed === null || body.avatarSeed === "") {
          patch.avatarSeed = undefined;
        } else if (isPebbleSeed(body.avatarSeed)) {
          patch.avatarSeed = String(body.avatarSeed).trim();
        } else {
          return json(res, 400, { error: "avatarSeed must be a short identity string" });
        }
      }
      ...
      if (body.browser !== undefined) {
        ...
        if (existingBot?.busy && body.browser !== (existingBot.browser !== false)) {
          return json(res, 409, { error: "stop this folk's turn before changing its browser access" });
        }
      ...
      if (body.browserProfile !== undefined) {
        ...
        if (existingBot?.busy && requestedProfile !== existingBot.browserProfile) {
          return json(res, 409, { error: "stop this folk's turn before changing its browser profile" });
        }
      ...
      if (body.hidden === true && existingBot?.chiefOfStaff && body.chiefOfStaff !== false) {
        return json(res, 400, { error: "choose another Chief of Staff before hiding this folk" });
      }
      ...
      // create the combination — a folk curling the loopback API from a tool
      ...
      const chiefChanges =
        body.chiefOfStaff === true || chiefMovedSections
          ...
          : [];
      if (chiefChanges === null) return json(res, 404, { error: "no such folk" });
    ...
    if (m && method === "DELETE") {
      ...
      if (activeRoutine) {
        return json(res, 409, {
          error: "stop this folk's active routine before deleting the folk",
        });
      }
      const activeGroup = activeGroupTurnForBot(bot.id);
      if (activeGroup) {
        return json(res, 409, {
          error: `stop this folk's work in channel ${activeGroup.group.name} before deleting the folk`,
        });
      }
      if (localVmMode(cfg) === "per-bot") {
        const target = perBotLocalVmTarget(bot.id);
        if (localVmActiveThreads.has(target.key) || localVmLifecycleBusy.has(target.key)) {
          return json(res, 409, { error: "stop this folk's Local VM turn or setup action before deleting the folk" });
        }
        const vm = await containerComputerStatus(undefined, undefined, target);
        if (!vm.daemonUp && existsSync(target.workspaceDir)) {
          return json(res, 409, {
            error: "start the container runtime and delete this folk's Local VM before deleting the folk",
          });
        }
        if (vm.container !== "missing") {
          return json(res, 409, { error: "delete this folk's Local VM from its Computer panel before deleting the folk" });
        }
      }
      // Establish a durable cleanup intent before any teardown. A malformed
      // or unreadable journal therefore rejects the delete with the folk and
      ...
      try {
        // a running turn dies with its folk
        const directClaim = cancelDirectTurnDispatch(bot.id);
        ...
        // a peer approval naming this folk can never be meaningfully answered
    ...
    // ── folk skills: imported Agent Skills (SKILL.md) ────────────────────
    ...
    if (m && method === "GET") {
      if (!store.bot(m[1])) return json(res, 404, { error: "no such folk" });
      return json(res, 200, {
    ...
    // Folks receive this in their system context, but no agent tool can write
    // it. That keeps one folk from silently changing every teammate's future
    ...
    // The files already belong to the user (plain markdown in the folk's
    // workspace); these routes only make them visible without a trip to
    // the filesystem. Reads never create the workspace — a folk that has
    ...
    // false for refused folders (home, Desktop…), a missing git, or a folk
    ...
    if (m && method === "POST") {
      ...
      // lease before reserving the folk, so no turn can enter during the
      // awaited Git operation.
      if (bot.busy) return json(res, 409, { error: "the folk is working — stop the turn before restoring files" });
      if (checkpointRestoreLeases.has(bot.id)) {
        return json(res, 409, { error: "this folk's project files are already being restored" });
      }
    ...
    if (m && method === "POST") {
      ...
      if (!store.taskByThread(bot.id, threadId)) {
        return json(res, 409, { error: "the folk switched tasks before it could receive the message" });
      }
      const sendId = parseSendId(body.sendId);
      const replyTo = resolveReplyTarget(threadId, body.replyToId);
      const receipt = await sendSequencer.run(
        sendId ? `folk:${bot.id}:${threadId}:${sendId}` : undefined,
        ...
        async () => {
          ...
          if (!currentAtStart) throw Object.assign(new Error("no such folk"), { status: 404 });
          ...
          if (currentAtStart.threadId !== threadId) {
            throw Object.assign(new Error("the folk switched tasks before it could receive the message"), {
              status: 409,
            });
          }
          ...
          if (currentAtStart.busy) {
            ...
            // switch, or the whole folk can be deleted before its acknowledgement
            ...
            // message into a task the folk no longer owns. A conflict leaves the
            // text in the client's composer/outbox to resend deliberately.
            const current = store.bot(bot.id);
            if (!current) throw Object.assign(new Error("no such folk"), { status: 404 });
            if (!store.taskByThread(bot.id, threadId)) {
              throw Object.assign(new Error("the target task no longer exists"), { status: 409 });
            }
            if (current.threadId !== threadId) {
    ...
    if (m && method === "POST") {
      ...
      if (bot.busy) return json(res, 409, { error: "the folk is working — stop it before editing" });
    ...
    if (m && method === "POST") {
      const bot = store.bot(m[1]);
      if (!bot) return json(res, 404, { error: "no such folk" });
      if (bot.busy) return json(res, 409, { error: "the folk is working — stop it before switching versions" });
      const body = await readBody(req);
    ...
    // too: a member's turn runs on the room's thread, and the folk that
    ...
    if (m && method === "POST") {
      ...
      if (routineRun) {
        if (expectedThreadId !== undefined && routineRun.threadId !== expectedThreadId) {
          return json(res, 409, { error: "this folk is running a routine in another conversation" });
        }
      ...
      const instance = registry.get(bot.modelSelection.instanceId);
      // a folk busy in a ROOM is running on the room's thread — stopping it
      ...
      if (busyGroup) {
        if (expectedThreadId !== undefined && busyGroup.threadId !== expectedThreadId) {
          return json(res, 409, { error: `this folk is working in channel ${busyGroup.group.name}` });
        }
      ...
      if (
        ...
        return json(res, 409, { error: "the folk switched tasks before it could be interrupted" });
      }
    ...
    // ── tasks: a folk's separate contexts ────────────────────────────────
    // The folk record answers with its messages because switching tasks
    ...
    if (m && method === "POST") {
      const bot = store.bot(m[1]);
      if (!bot) return json(res, 404, { error: "no such folk" });
      if (bot.busy) return json(res, 409, { error: "this folk is working — let it finish before starting a task" });
      const body = await readBody(req);
    ...
    if (m && method === "POST") {
      ...
      if (bot.busy) return json(res, 409, { error: "this folk is working — stop it before switching tasks" });
    ...
    if (m && method === "DELETE") {
      ...
      if (!updated) return json(res, 400, { error: "a folk keeps at least one task" });
      rejectDeletedThreadSkillStages(stagedSkillCleanups);
    ...
    if (m && method === "POST") {
      ...
      if (localVmMode(cfg) === "per-bot" && action === "run") {
        return json(res, 409, { error: "Per-folk mode creates each desktop from that folk's Computer panel" });
      }
      ...
      if (vmOwner && (action === "stop" || action === "remove" || action === "run")) {
        return json(res, 409, { error: "the Local VM is being used by a folk — stop that turn first" });
      }
    ...
    if (m && method === "POST") {
      ...
      if (localVmImageBusy || localVmModeChangeBusy || localVmLifecycleBusy.has(target.key)) {
        return json(res, 409, { error: "this folk's Local VM setup action is still running" });
      }
      ...
      if (vmOwner) return json(res, 409, { error: "this folk is using its Local VM — stop the turn first" });
        ...
        if (action === "run") {
          ...
          if (!(await containerComputerExists(before.runtime, target))) {
            ...
            if (count >= localVmMaxInstances(cfg)) {
              return json(res, 409, {
                error: `The per-bot Local VM limit is ${localVmMaxInstances(cfg)} — delete an unused folk VM or raise the limit in App Settings`,
              });
            }
          }
        }
    ...
    if (method === "GET" && path === "/api/health") {
      return json(res, 200, { app: "openfolks", pid: process.pid, static: Boolean(STATIC_DIR) });
    }
    ...
    if (method === "GET" && (path === "/api/openfolks/onboarding" || path === "/api/opencrew/onboarding")) {
      const status = checkToolchain();
      return json(res, 200, {
        ...openfolksOnboardingState(),
        defaultProjectFolder: defaultProjectFolder(),
        toolchain: status,
        toolchainNotes: toolchainSummary(status),
      });
    }
    if (method === "POST" && (path === "/api/openfolks/onboarding/complete" || path === "/api/opencrew/onboarding/complete")) {
      if (store.bots.length > 0) {
        return json(res, 409, { error: "workspace already has folks — use normal folk creation or team import" });
      }
      const body = await readBody(req).catch(() => ({} as Record<string, unknown>));
      const mode = body && typeof body === "object" && body.mode === "crew" ? "crew" : "solo";
      const selection = await defaultSelection();
      bootSelection = selection;
      if (mode === "crew") {
        const folderRaw = typeof body.projectFolder === "string" ? body.projectFolder.trim() : "";
        const folder = folderRaw || defaultProjectFolder();
        try {
          prepareProjectFolder(folder);
          const result = importOpenFolksCrew(store, folder, selection);
          markOpenFolksOnboardingComplete(folder);
          ...
          return json(res, 201, {
            ...
            section: result.section ?? "Job Search",
            mode: "crew",
          });
        } catch (e) {
          return json(res, 500, { error: e instanceof Error ? e.message : "crew setup failed" });
        }
      }
      const bot = createFirstFolk(store, selection);
      markOpenFolksOnboardingComplete();
    ...
    if (method === "GET" && path === "/api/mcp-servers") {
      return json(res, 200, { servers: listMcpServersPublic(cfg) });
    }
    if (method === "PUT" && path === "/api/mcp-servers") {
      const body = await readBody(req);
      const serversRaw = body && typeof body === "object" && !Array.isArray(body) ? (body as { servers?: unknown }).servers : undefined;
      if (!serversRaw || typeof serversRaw !== "object" || Array.isArray(serversRaw)) {
        return json(res, 400, { error: "expected { servers: { <name>: { … } } }" });
      }
      try {
        const next: Record<string, unknown> = {};
        for (const [name, entry] of Object.entries(serversRaw as Record<string, unknown>)) {
          if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
            return json(res, 400, { error: `invalid entry for ${name}` });
          }
          const e = entry as Record<string, unknown>;
          const kind = e.url !== undefined || e.kind === "url" ? "url" : "stdio";
          next[name] = mergeMcpServerEntry(
            name,
            {
              kind,
              ...
              clearEnv: e.clearEnv === true,
              clearHeaders: e.clearHeaders === true,
            },
            cfg.mcpServers?.[name],
          );
        }
        saveMcpServers(next);
        cfg = loadConfig();
        syncCredentialEnv(cfg);
        await reloadProviders();
        broadcast({ kind: "config", ...configStatus() });
        return json(res, 200, { servers: listMcpServersPublic(cfg) });
      } catch (error) {
        const status = typeof (error as { status?: number }).status === "number" ? (error as { status: number }).status : 400;
        return json(res, status, { error: error instanceof Error ? error.message : "Could not save MCP servers" });
      }
    }
        ...
        try {
          for (const bot of store.bots) {
            if (bot.browserProfile && bot.browserProfile !== "guest" && !retained.has(bot.browserProfile)) {
              // The profile list and every folk reference change in the same
              // config request. Non-renderer clients therefore cannot leave a
              // folk pointing at a deleted cookie partition.
              store.patchBot(bot.id, { browserProfile: undefined });
            }
          }
        } catch (error) {
    ...
    // Voice / ElevenLabs TTS was removed from the product UI. Keep the
    // endpoints as 410 so older clients get a clear signal rather than a
    // silent 404 that looks like a routing bug.
    if (method === "POST" && path === "/api/tts/prepare") {
      return json(res, 410, { error: "Voice playback has been removed" });
    }
    ...
    // Inline connection cards are bound to both the folk and the exact task
    ...
    // Who is driving this folk's computer. GET is the panel's initial read;
    // POST take/release/dismiss-help are the person's three moves. The folk
    ...
    if (m && method === "POST") {
      ...
      if (bot.cloudBackend === "vps") {
        if (m[2] === "exec") {
          return json(res, 409, { error: "the VPS console is available to the folk through its scoped computer tools" });
        }
        ...
        if ((m[2] === "sleep" || m[2] === "remove") && (bot.busy || activeVpsThreads.has(botId))) {
          return json(res, 409, { error: "the VPS computer is being used by this folk — interrupt the turn first" });
        }
        if (m[2] === "join") {
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`openfolks server on http://127.0.0.1:${PORT}`);
});
