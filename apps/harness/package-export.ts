...
export function createBotPackageExport(input: {
  ...
  if (!bots.length) throw new Error("Create a folk before exporting your package");

  const packageKeys = new Set<string>();
  const idToKey = new Map<string, string>();
  for (const [index, bot] of bots.entries()) {
    idToKey.set(bot.id, portableKey(bot.name, `folk-${index + 1}`, packageKeys));
  }
  ...
  const id = portableKey(input.name, "openfolks-package", new Set());
  ...
  const definition: BotPackageDefinition = {
    ...
    release: "1.0.0",
    name: input.name,
    tagline: `A portable OpenFolks setup with ${bots.length} ${bots.length === 1 ? "bot" : "bots"}.`,
    summary: "Exported from OpenFolks. Review the roles, rooms, playbooks, connector requirements, and paused routines before sharing or publishing.",
    category: "Community",
    author: { name: input.authorName?.trim() || "OpenFolks user" },
    license: "Unspecified",
    outcomes: ["Recreate this folk setup without copying private runtime state."],
