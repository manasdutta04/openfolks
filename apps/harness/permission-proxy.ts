...
// OpenFolks server and waits for the human's answer.
//
...
const dead = () => {
  for (const resolve of waiting.values()) {
    resolve({ behavior: "deny", message: "OpenFolks: permission broker unavailable — skip this action" });
  }
  waiting.clear();
};
conn.on("error", dead);
...
const TOOLS = [
  {
    name: "approve",
    description: "Ask the OpenFolks user whether a tool use is allowed",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "ask_user",
    description:
      "Ask the human who owns this folk a question and wait for their answer. Use whenever you need a decision, a preference, missing information, or sign-off before doing something consequential — do not guess on things the owner would want to decide. Returns their answer as text.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "openfolks-permissions", version: "1" },
      },
    });
  }
  ...
  if (msg.method === "tools/call") {
    ...
    if (!isQuestion) {
      if (answer.behavior === "allow") {
        ...
        text = JSON.stringify({ behavior: "deny", message: answer.message || "Denied from OpenFolks" });
      }
    }
