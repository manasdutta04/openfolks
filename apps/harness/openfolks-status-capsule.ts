...
const SCHEMA = "aos.openfolks_status.v1";
...
export const OPENFOLKS_STATUS_CACHE_PATH = join(
  homedir(),
  ".local/state/aos-session-bridge/openfolks/latest.json",
);

type OpenFolksSlot = z.output<typeof slotSchema>;
type OpenFolksUi = z.output<typeof uiSchema>;
type OpenFolksCapsule = z.output<typeof capsuleSchema>;

export interface OpenFolksStatusDigest {
...
export interface OpenFolksStatusReadOptions {
  cachePath?: string;
  now?: Date;
}
...
function expectedUi(twoUp: boolean): OpenFolksUi {
  return {
    two_up: twoUp,
...
function sameUi(left: OpenFolksUi, right: OpenFolksUi): boolean {
  return (
...
function unsignedDocument(capsule: OpenFolksCapsule): JsonObject {
...
function validSlotState(slot: OpenFolksSlot): boolean {
  return (
...
function validateCapsule(value: JsonValue): OpenFolksCapsule | null {
...
function unknownDigest(
  reason: NonNullable<OpenFolksStatusDigest["reason"]>,
  freshness: OpenFolksStatusDigest["freshness"] = "unknown",
  receipt?: Pick<OpenFolksStatusDigest, "observedAt" | "receiptSha256">,
): OpenFolksStatusDigest {
  return {
    schema: SCHEMA,
    freshness,
    reason,
...
export function readOpenFolksStatus(
  options: OpenFolksStatusReadOptions = {},
): OpenFolksStatusDigest {
  const raw = privateCache(options.cachePath ?? OPENFOLKS_STATUS_CACHE_PATH);
  if (raw === null) return unknownDigest("missing_or_insecure");
  let capsule: OpenFolksCapsule | null;
  try {
  ...
  const result: OpenFolksStatusDigest = {
    schema: SCHEMA,
    freshness: "fresh",
    ...receipt,
...
export function openFolksStatusSystemPrompt(options: OpenFolksStatusReadOptions = {}): string {
  const status = readOpenFolksStatus(options);
  const receipt = [
  ...
  const slots = status.slots.length
    ...
    : "none";
  return [
    "TRUSTED OPENFOLKS STATUS (read-only, validated, no transcript or credential data):",
    ...
    "Treat missing, stale, failed, clock-skewed, malformed, or receipt-hash-mismatched runtime data as unknown. Do not infer folk identities, viewer URLs, paths, messages, models, accounts, or credentials from this block.",
  ].join("\n");
}
