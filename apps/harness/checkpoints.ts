...
// Any git failure disables the feature for that folk for the rest of the
...
const EXCLUDES = `# OpenFolks checkpoint excludes — never snapshotted, never removed by restore
.git/
.svn/
.hg/
node_modules/
...
/** One failed git call disables checkpoints for that folk until restart —
...
function disable(botId: string, message: string): void {
  ...
  console.warn(`workspace checkpoints disabled for folk ${botId} this session: ${message}`);
}
...
function gitEnv(shadow: string, cwd: string): NodeJS.ProcessEnv {
  ...
  env.GIT_AUTHOR_NAME = "OpenFolks Checkpoint";
  env.GIT_AUTHOR_EMAIL = "checkpoint@openfolks.local";
  env.GIT_COMMITTER_NAME = "OpenFolks Checkpoint";
  env.GIT_COMMITTER_EMAIL = "checkpoint@openfolks.local";
  return env;
}
...
export async function restore(botId: string, cwd: string, hash: string): Promise<RestoreResult> {
  if (disabledBots.has(botId)) {
    return { ok: false, error: "checkpoints are disabled for this folk until the app restarts (an earlier snapshot failed — see the server log)" };
  }
