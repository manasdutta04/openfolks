...
function cleanInput(
  ...
  if (!Array.isArray(input.botIds)) throw new Error("Add at least one folk");
  const botIds = [...new Set(input.botIds.map((id) => String(id ?? "").trim()).filter(Boolean))];
  if (!botIds.length) throw new Error("Add at least one folk");
  if (botIds.length > MAX_BOTS) throw new Error(`Add no more than ${MAX_BOTS} folks`);
  if (botIds.some((botId) => !botExists(botId))) throw new Error("One or more folks no longer exist");
