...
// per-customer: every folk gets one persistent box (deterministic name),
...
/**
 * Find-or-create the folk's persistent box, wait for ready, run the
...
export async function provisionBox(cfg: AppConfig, botId: string, botName: string) {
  if (!boxConfigured(cfg)) {
    throw new Error('box provider not enabled — add {"box":{"token":"…"}} to ~/.openfolks/config.json');
  }
...
/** Wake the folk's box and return a FRESH desktop URL. */
...
/** Archive the folk's box now (billing pauses, disk survives). */
export async function sleepBox(cfg: AppConfig, botId: string) {
  const box = await findBox(cfg, botId);
  if (!box) throw new Error("no computer for this folk");
...
export async function execOnBox(cfg: AppConfig, botId: string, command: string) {
  const box = await findBox(cfg, botId);
  if (!box) throw new Error("no computer for this folk yet");
