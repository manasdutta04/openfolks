...
describe("cloud computer provisioning cleanup", () => {
        ...
        if (url.pathname === "/api/box/v1/boxes" && req.method === "GET") {
          const boxes =
            scenario === "existing-desktop-failure"
              ? [{ id: "existing-box", name: nameFor("existing-folk"), state: "ready" }]
              : [];
          ...
          res.writeHead(200).end(
            JSON.stringify({ ok: true, box: { id: "existing-box", name: nameFor("existing-folk"), state: "ready" } }),
          );
  ...
  it("permanently deletes a newly created box when naming fails", async () => {
    ...
    await expect(provisionBox({ box: { token: "box_test" } } as any, "new-folk", "New Folk")).rejects.toThrow(
  ...
  it("never deletes a pre-existing box when a later step fails", async () => {
    ...
    await expect(
      provisionBox({ box: { token: "box_test" } } as any, "existing-folk", "Existing Folk"),
