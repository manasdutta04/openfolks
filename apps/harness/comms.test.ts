...
// bot A's "agent" reach folk B through the injected agents proxy (list_bots →
...
describe("mentionedBots", () => {
  ...
  it("prefers the longest name so prefixes never half-match", () => {
    expect(mentionedBots("ask @New Folk 2 about it", peers).map((b) => b.id)).toEqual(["2"]);
  });
  it("dedupes repeats and collects multiple folks", () => {
    expect(mentionedBots("@Milind and @New Folk and @Milind", peers).map((b) => b.id)).toEqual(["3", "1"]);
  });
  it("ignores emails, hidden folks, and mid-word @", () => {
...
describe("roomResponders", () => {
  ...
  it("keeps folk-to-folk channels on their last-speaker routing", () => {
...
describe("comms e2e (fake ACP fleet)", () => {
  ...
  beforeAll(async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        instances: {
          // the ask-peer fleet: both folks run "ask-peer" so A can ask B
          ...
          // Normal Gemini 3.x folks use Antigravity print mode rather than
  ...
  it("seals the internal comms endpoints behind the boot token", async () => {
    ...
    const ask = await api("POST", "/api/internal/ask-folk", { toBotId: "x", message: "hi" });
    expect(ask.status).toBe(401);
  });

  it(
    "carries a question from folk A through the agents proxy to folk B and back",
    async () => {
      // deterministic roster: hide the seeded folk, add Asker + Helper
  ...
  it(
    "lets a Gemini Antigravity folk call a peer through the temporary agents MCP mount",
    async () => {
      // A unique section makes list_bots deterministic even though this
      // suite deliberately keeps earlier folks around to exercise the real
      ...
      const send = await api("POST", `/api/bots/${asker.id}/messages`, {
        text: "Ask the other folk for a status check.",
      });
      expect(send.status).toBe(202);
      ...
      // later folk nor the user's own agy session can inherit the capability.
      await expect.poll(
  ...
  // depth >= MAX_COMMS_DEPTH. The exchange mirrors into a folk⇄folk channel
  ...
  it(
    ...
    async () => {
      ...
      // channel mirrors both sides, attributed by folk
      const state = (await api("GET", "/api/bots")).body;
      const channel = state.groups.find((g: any) => g.id === note.comm.groupId);
      expect(channel?.dm).toBe(true);
      expect(channel.memberIds).toContain(asker.id);
  ...
  it(
    ...
    async () => {
      // determinism: the ask-peer CLI asks the FIRST visible folk, so hide
  ...
  // half of "the folks don't respond to each other" (#583's user report).
  ...
  // When the SOURCE folk has approvePeerComms = true, an ask_bot call must
  ...
  it(
    ...
    async () => {
      ...
      await api("PATCH", `/api/bots/${helper.id}`, { name: "Helper", modelSelection: selection });
      const asker = (await api("POST", "/api/bots")).body.bot;
      // approvePeerComms on the SOURCE folk — the test's whole point
      ...
      // into folk.alwaysAllow, and offers Allow / Deny / Always allow.
  ...
  it("refuses ask_bot with a denial chip and never starts B when the user denies", async () => {
    ...
    // which wraps it as "Couldn't reach that folk: denied by user" and
