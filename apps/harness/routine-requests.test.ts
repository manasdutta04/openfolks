...
describe("RoutineRequestService", () => {
  ...
  it("canonicalizes receipt fingerprints and binds them to the card's conversation", async () => {
    ...
    await service.propose({ botId: "folk-a", threadId: "thread-a", proposal: createProposal() });
  ...
  it("never returns an existing routine's credential-shaped text to the proposing folk", async () => {
  ...
  it("denies without changing the scheduler and claims duplicate answers", async () => {
    const { service, routines, store } = harness();
    const proposal = await service.propose({ botId: "folk-a", threadId: "thread-a", proposal: createProposal() });
  ...
  it("applies update, pause, resume, run-now, and delete only to the owning folk", async () => {
    const { service, routines, store } = harness();
    const routine = routines.create({
      botId: "folk-a",
      name: "Old name",
    ...
    const apply = async (proposal: RoutineProposalInput) => {
      const card = await service.propose({ botId: "folk-a", threadId: "thread-a", proposal });
...
describe("cross-folk routine targeting", () => {
  ...
  const forOps = { forBot: { botId: "folk-b", name: "Ops" } };

  it("binds the confirmed routine to the named target folk, not the proposer", async () => {
    ...
    expect(created[0].botId).toBe("folk-b");
  });

  it("still schedules for the proposer when no target is named", async () => {
    const { routines, service } = targetedHarness();
    const proposed = await service.propose({ botId: "folk-a", threadId: "thread-a", proposal: createProposal() });
    service.resolve({ botId: "folk-a", threadId: "thread-a", requestId: proposed.requestId, behavior: "allow" });
    expect(routines.listRoutines()[0]?.botId).toBe("folk-a");
  });
  ...
  it("re-checks the target at confirm time — a deleted folk refuses without creating anything", async () => {
    let gone = false;
