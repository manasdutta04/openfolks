...
describe("VPS computer", () => {
  it("uses a deterministic, folk-id-derived managed container name", () => {
    expect(vpsContainerName(BOT_ID)).toBe(vpsContainerName(BOT_ID));
    expect(vpsContainerName(BOT_ID)).not.toBe(vpsContainerName("another-folk"));
    expect(vpsContainerName(BOT_ID)).toMatch(/^openfolks-vps-[a-z0-9-]+$/);
  });
  ...
  it("serializes concurrent provisioning for the same folk", async () => {
  ...
  it("mounts the official Cua MCP server through the tiny remote exec bridge", () => {
    ...
    expect(vpsContainerMcpArgs("production-vps", vpsContainerName(BOT_ID))).toEqual([
      ...
      "/usr/local/libexec/openfolks/cua-driver",
      "mcp",
      "--socket",
      "/run/user/1000/openfolks-cua.sock",
    ]);
  });
  ...
  it("never removes a container OpenFolks did not create", async () => {
