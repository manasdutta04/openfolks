...
describe("computer proxy (fake box)", () => {
      ...
      if (url.pathname.endsWith("/commands")) {
        ...
        req.on("end", () => {
          ...
          const stdout = isWait
            ...
            : command.includes("openfolks-cdp.mjs snapshot")
              ? JSON.stringify({
                  title: "Account",
              ...
              : command.includes("openfolks-cdp.mjs click") || command.includes("openfolks-cdp.mjs fill")
                ? `GEOM 1920 1080\nHASH ${hash}\nSIZE ${size}\nB64 ${JPEG}\nSEM ok\n`
  ...
  it("uses fresh semantic browser refs without exposing URL secrets", async () => {
    ...
    expect(commands.length - before).toBe(1);
    expect(commands.at(-1)).toContain("openfolks-cdp.mjs fill");
  ...
  it("uses a private Chrome profile, strips URL credentials, and reports redirects", async () => {
    ...
    expect(issued[0]).toContain('profile="$HOME/.openfolks/chrome-profile"');
    ...
    expect(issued[0]).toContain('--user-data-dir="$HOME/.openfolks/chrome-profile"');
