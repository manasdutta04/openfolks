...
const bot = { id: "folk-1", name: "Scout", threadId: "thread-1" };

describe("buildNotification", () => {
  it("names the folk and carries the detail, per kind", () => {
  ...
  it("stays silent for a folk whose notifications are off", () => {
  ...
  it("uses the thread it was raised on, not the folk's current one", () => {
  ...
  it("carries the folk's avatar when one is given", () => {
    ...
    expect(frame).toMatchObject({ botId: "folk-1", body: "pushed the branch", avatarUrl });
