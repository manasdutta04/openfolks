...
export const BOT_PACKAGE_FORMAT = "openfolks.package" as const;
...
const packageSchema = z.object({
  format: z.literal(BOT_PACKAGE_FORMAT, { error: "This is not an OpenFolks package" }),
...
function markdownDocument(markdown: string): ParsedBotPackage {
  if (Buffer.byteLength(markdown) > 1_000_000) throw new Error("The folk playbook is too large");
...
export function parseBotPackage(value: JsonValue | ParsedBotPackage): ParsedBotPackage {
  ...
  if (!parsed.success) throw new Error(schemaIssue(parsed.error, "This is not a folk package"));
 ...
 * run without OpenFolks or another proprietary parser. */
