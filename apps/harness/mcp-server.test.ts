...
afterEach(() => {
  ...
  delete process.env.OPENFOLKS_TOKEN;
...
describe("MCP JSON-RPC protocol", () => {
  it("negotiates supported and newer protocol versions", async () => {
    ...
    expect(supported.result).toMatchObject({
      ...
      serverInfo: { name: "openfolks-mcp", version: "1.1.0" },
      capabilities: { tools: {} },
    });
  ...
  it("returns structured and text tool results", async () => {
    const handler = vi.fn(async () => ({ bots: [{ id: "folk-1" }] })) as any;
    const response = JSON.parse((await processMcpMessage(JSON.stringify({
      jsonrpc: "2.0",
      id: 3,
      method: "tools/call",
    ...
    expect(response.result.structuredContent).toEqual({ bots: [{ id: "folk-1" }] });
  ...
  it("rejects unknown tools and malformed arguments as invalid params", async () => {
    for (const params of [
      ...
      { name: "send_bot_message", arguments: { bot_id: "folk-1", text: { not: "text" } } },
  ...
  it("cancels an in-flight tool call with the MCP cancellation notification", async () => {
    ...
    const pending = processMcpMessage(JSON.stringify({
      ...
      params: {
        ...
        arguments: { target_type: "bot", target_id: "folk-1" },
      },
    }), handler);
...
describe("MCP tool execution", () => {
  it("lists folks without fetching transcripts", async () => {
    const fetcher = vi.fn(async (path: string) => {
      expect(path).toBe("/api/bots?messages=0");
      return {
        bots: [{
          id: "folk-1", name: "Deckard", title: "Detective", busy: false, activity: "idle",
    ...
    expect(result.bots[0]).toMatchObject({ id: "folk-1", activeTaskId: "task-1", activity: "idle" });
    expect(result.bots[0].tasks[0]).toMatchObject({ taskId: "task-1", active: true });
    expect(result.bots[0]).not.toHaveProperty("messages");
  });

  it("reads a bounded folk task and removes pixels and approval grant keys", async () => {
    const fetcher = vi.fn(async (path: string) => {
      if (path === "/api/bots?messages=0") return {
        bots: [{ id: "folk-1", name: "Deckard", threadId: "task-1", tasks: [{ threadId: "task-1", title: "Case" }] }],
      };
    ...
    const result: any = await handleToolCall("get_bot_messages", { bot_id: "folk-1", limit: 200 }, fetcher);
  ...
  it("pins folk and channel sends to the active task", async () => {
    const fetcher = vi.fn(async (path: string, options?: RequestInit) => {
      if (path === "/api/bots?messages=0") return {
        bots: [{
          id: "folk-1", threadId: "folk-task", tasks: [
            { threadId: "folk-task" },
            { threadId: "folk-old" },
          ],
        }],
    ...
    await expect(handleToolCall("send_bot_message", {
      bot_id: "folk-1", task_id: "folk-old", text: "Wrong task",
    ...
    await expect(handleToolCall("send_bot_message", {
      bot_id: "folk-1", text: "Investigate",
    }, fetcher)).resolves.toMatchObject({ success: true, taskId: "folk-task" });
    await expect(handleToolCall("send_channel_message", {
      channel_id: "channel-1", text: "Ship it",
    }, fetcher)).resolves.toMatchObject({ success: true, taskId: "channel-task" });
  });

  it("creates a folk through the safe profile boundary", async () => {
    const fetcher = vi.fn(async (path: string, options?: RequestInit) => {
      if (path === "/api/bots" && options?.method === "POST") {
        ...
        return { bot: { id: "folk-new", name: "Mira", title: "Researcher", section: "Work", threadId: "task-new", tasks: [] } };
      }
      throw new Error(`unexpected path ${path}`);
    });
    const result: any = await handleToolCall("create_bot", { name: "Mira", title: "Researcher", section: "Work" }, fetcher);
    expect(result.bot).toMatchObject({ id: "folk-new", name: "Mira", section: "Work" });
  });

  it("leaves single-request folk-create failures to the server without destructive rollback", async () => {
    const calls: string[] = [];
    const fetcher = vi.fn(async (path: string, options?: RequestInit) => {
    ...
    const fetcher = vi.fn(async (path: string, options?: RequestInit) => {
      if (path === "/api/groups") {
        expect(JSON.parse(String(options?.body))).toEqual({
          name: "Launch",
          memberIds: ["folk-1", "folk-2"],
          section: "Work",
          setup: { bulletin: "Ship safely", defaultResponder: { kind: "everyone" } },
        });
        return { group: { id: "channel-1", name: "Launch", memberIds: ["folk-1", "folk-2"], threadId: "task-1", tasks: [] } };
      }
      throw new Error(`unexpected path ${path}`);
    });
    const result: any = await handleToolCall("create_channel", {
      name: "Launch", member_ids: ["folk-1", "folk-2"], section: "Work", bulletin: "Ship safely",
      default_responder: { kind: "everyone" },
    }, fetcher);
    expect(result.channel.id).toBe("channel-1");
  });

  it("routes folk and channel task mutations", async () => {
    const fetcher = vi.fn(async (path: string, options?: RequestInit) => {
      if (path === "/api/groups/channel-1/tasks") {
        return { task: { threadId: "task-2", title: "Fresh" }, group: { threadId: "task-2" } };
      }
      if (path === "/api/bots/folk-1/tasks/task-2?messages=0") {
        return {
          bot: {
            id: "folk-1", name: "Mira", threadId: "task-2",
    ...
    const switched: any = await handleToolCall("switch_task", {
      target_type: "bot", target_id: "folk-1", task_id: "task-2",
    }, fetcher);
    expect(fetcher).toHaveBeenLastCalledWith("/api/bots/folk-1/tasks/task-2?messages=0", expect.objectContaining({ method: "POST" }));
  ...
  it("rejects incomplete mutation responses before projecting them", async () => {
    ...
    await expect(handleToolCall("update_bot_profile", {
      bot_id: "folk-1", name: "Mira",
    }, fetcher)).rejects.toThrow("OpenFolks did not return the updated folk");
    await expect(handleToolCall("update_channel", {
      channel_id: "channel-1", name: "Launch",
    }, fetcher)).rejects.toThrow("OpenFolks did not return the updated channel");
    await expect(handleToolCall("create_task", {
      target_type: "bot", target_id: "folk-1", title: "Fresh",
    }, fetcher)).rejects.toThrow("OpenFolks did not return the created task");
    await expect(handleToolCall("rename_task", {
      target_type: "bot", target_id: "folk-1", task_id: "task-1", title: "Renamed",
    }, fetcher)).rejects.toThrow("OpenFolks did not return the renamed task");
  });
  ...
  it("requires an exact available model and refuses changes while busy", async () => {
    const busyFetcher = vi.fn(async () => ({ bots: [{ id: "folk-1", busy: true }] }));
    await expect(handleToolCall("set_bot_model", {
      bot_id: "folk-1", instance_id: "codex", model: "gpt-5.6-sol",
    }, busyFetcher)).rejects.toThrow("let it finish");

    const fetcher = vi.fn(async (path: string, options?: RequestInit) => {
      if (path === "/api/bots?messages=0") return { bots: [{ id: "folk-1", busy: false }] };
    ...
    await expect(handleToolCall("set_bot_model", {
      bot_id: "folk-1", instance_id: "codex", model: "made-up",
    }, fetcher)).rejects.toThrow("not offered");
    const result: any = await handleToolCall("set_bot_model", {
      bot_id: "folk-1", instance_id: "codex", model: "gpt-5.6-sol", effort: "high",
    }, fetcher);
  ...
  it("waits on a folk conversation and returns a compact attention tail", async () => {
    const fetcher = vi.fn(async (path: string) => {
      if (path === "/api/bots?messages=0") return {
        bots: [{ id: "folk-1", name: "Mira", busy: true, activity: "waiting-on-you", threadId: "task-1", tasks: [] }],
      };
    ...
    const result: any = await handleToolCall("wait_for_conversation", {
      target_type: "bot", target_id: "folk-1", timeout_seconds: 1,
    }, fetcher);
  ...
  it("reports no-signal as stalled and keeps the frozen task tail", async () => {
    const fetcher = vi.fn(async (path: string) => {
      if (path === "/api/bots?messages=0") return {
        bots: [{
          id: "folk-1", name: "Mira", busy: true, activity: "no-signal",
    ...
    const historical: any = await handleToolCall("wait_for_conversation", {
      target_type: "bot", target_id: "folk-1", task_id: "task-old", timeout_seconds: 1,
    }, fetcher);
    ...
    const stalledFetcher = vi.fn(async (path: string) => {
      if (path === "/api/bots?messages=0") return {
        bots: [{ id: "folk-1", name: "Mira", busy: true, activity: "no-signal", threadId: "task-active", tasks: [] }],
        groups: [],
      };
  ...
  it("reports asynchronous folk and channel dispatch failures", async () => {
    const errorMessages = {
      messages: [
    ...
    const botFetcher = vi.fn(async (path: string) => {
      if (path === "/api/bots?messages=0") return {
        bots: [{ id: "folk-1", name: "Mira", busy: false, activity: "idle", threadId: "task-1", tasks: [] }],
        groups: [],
      };
  ...
  it("detects durable channel blockers and interrupts the exact target thread", async () => {
    const fetcher = vi.fn(async (path: string, options?: RequestInit) => {
      if (path === "/api/bots?messages=0") return {
        bots: [{ id: "folk-1", activity: "waiting-on-you" }],
        groups: [{ id: "channel-1", name: "Launch", memberIds: ["folk-1"], threadId: "task-1", tasks: [], busyBotId: "folk-1" }],
      };
      if (path === "/api/threads/task-1/messages?limit=10") return {
...
describe("connection security and discovery", () => {
  ...
  it("skips a foreign process and discovers the real fallback port", async () => {
    globalThis.fetch = vi.fn(async (url: any) => {
      if (String(url).includes(":8799")) return jsonResponse({ app: "not-openfolks" });
      if (String(url).includes(":18799")) return jsonResponse({ app: "openfolks" });
  ...
  it("rejects successful non-JSON responses and sends an optional bearer token", async () => {
    process.env.OPENFOLKS_TOKEN = "proxy-token";
  ...
  it("requires an explicit destination before sending a bearer token", async () => {
    process.env.OPENFOLKS_TOKEN = "proxy-token";
    await expect(resolveBaseUrl()).rejects.toThrow("OPENFOLKS_URL or OMB_PORT");
  });

  it("validates direct tool arguments", () => {
    expect(() => validateToolArguments("send_bot_message", { bot_id: "folk-1", text: "hello" })).not.toThrow();
    expect(() => validateToolArguments("send_bot_message", { bot_id: "folk-1", text: "hello", extra: true })).toThrow("not supported");
  });
});
