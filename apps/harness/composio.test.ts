  ...
  api = createServer(async (req, res) => {
    ...
    if (req.method === "GET" && url.pathname === "/api/v3.1/tool_router/session/trs_legacy") {
      ...
      return res.end(JSON.stringify({
        ...
        config: { user_id: "openfolks_legacy" },
      }));
    }
...
describe.sequential("Composio Sessions", () => {
  ...
  it("ignores credential sync without access and clears only on explicit null", () => {
    const messageType = "openfolks:managed-composio";
    setManagedBrokerAccess({ url: "http://127.0.0.1:3210/", token: "a".repeat(64) });
  ...
  it("creates one stable per-installation session and reuses it", async () => {
    const created = await prepareProjectSession("ak_test", { userId: "openfolks_existing" });
    expect(created).toEqual({
      apiKey: "ak_test",
      userId: "openfolks_existing",
      sessionId: "trs_test",
    });
  ...
  it("names the project's own auth configs at creation and rebuilds a Session that predates them", async () => {
    ...
    sessionAuthConfigs = {};
    try {
      const current = { apiKey: "ak_test", userId: "openfolks_existing", sessionId: "trs_test" };
      ...
      expect(creates[0].body).toMatchObject({ user_id: "openfolks_existing", auth_configs: { twitter: "ac_twitter" } });
  ...
  it("rebuilds the Session and retries when a toolkit needs the project's own auth config", async () => {
    ...
    try {
      ...
      expect(cfg.composio).toMatchObject({ userId: "openfolks_existing", sessionId: "trs_test" });
    } finally {
      customAuthConfigs = [];
      sessionAuthConfigs = {};
    }
  ...
  it("says what to create when the project has no auth config for the toolkit", async () => {
    const cfg: AppConfig = {
      composio: { apiKey: "ak_test", userId: "openfolks_existing", sessionId: "trs_test" },
    };
    const before = calls.length;
