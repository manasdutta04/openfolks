...
// the person's three moves (take, release, dismiss) all work, the folk's one
...
describe("computer control", () => {
  it("starts disengaged for an unknown folk", () => {
  ...
  it("an empty reason still reads as a plea", () => {
    const { control } = tracked();
    expect(control.requestHelp("b1", undefined).helpReason).toBe("the folk asked you to take over");
  });
  ...
  it("forgetting an unknown folk is silent", () => {
    const { control, changes } = tracked();
