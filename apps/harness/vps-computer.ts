...
export const VPS_MANAGED_LABEL = "com.openfolks.vps";
export const VPS_CONTAINER_LABEL = "com.openfolks.container";
export const VPS_VIEWER_LABEL = "com.openfolks.vps-viewer";
export const VPS_CONTAINER_PREFIX = "openfolks-vps";
...
const SCREENSHOT_PATH = "/tmp/openfolks-vps-preview.png";
...
function containerNamePart(botId: string): string {
  return botId.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 12) || "folk";
}

/** Stable across restarts and independent of the folk's editable display name. */
...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned OpenFolks Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No OpenFolks container exists for this folk on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted OpenFolks image";
  if (!status.managed) return "The VPS container name is occupied by a container OpenFolks did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing OpenFolks safety limits";
  if (status.container === "stopped") return "The OpenFolks VPS container is stopped";
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        if (before.container === "missing") throw Object.assign(new Error("No VPS container exists for this folk"), { status: 409 });
        ...
        // IMAGE_LAYER_VERSION bump otherwise bricks the folk: provision 409s
        ...
        // never docker-rm a container OpenFolks did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container OpenFolks did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
