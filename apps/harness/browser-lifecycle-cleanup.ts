...
const browserCleanupResultSchema = z.object({
  type: z.literal("openfolks:browser-lifecycle-result"),
  requestId: z.string().regex(REQUEST_ID),
  ok: z.boolean(),
}).strict();
...
export type BrowserCleanupWireRequest = {
  type: "openfolks:browser-bot-deleted" | "openfolks:browser-profile-deleted";
  requestId: string;
  botId?: string;
  partitionId?: string;
};
...
export function requireBrowserCleanupAcknowledged(ok: boolean, target: string): void {
  ...
  const error = Object.assign(new Error(
    `${target} was removed, but OpenFolks could not confirm its local browser data was erased. `
...
export class BrowserCleanupCoordinator {
   ...
   * is replayed at boot, the server clears every folk that still names one of
  ...
  receive(message: BrowserCleanupIncomingMessage | undefined): boolean {
    if (message?.type !== "openfolks:browser-lifecycle-result") return false;
  ...
  async #attempt(request: BrowserCleanupRequest): Promise<boolean> {
    ...
    const sent = this.#send({
      type: current.kind === "bot"
        ? "openfolks:browser-bot-deleted"
        : "openfolks:browser-profile-deleted",
