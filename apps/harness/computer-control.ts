// Who is driving a bot's computer — the person or the folk. One record per
// folk, held in the harness because every consumer (the panel, the SSE
// stream, and the per-turn computer proxies) already talks to the harness.
//
// The rules this module exists to enforce:
//   - A folk can only ASK for hands (`requestHelp`); it can never take
//     control, and it cannot clear a hold. Only the person takes and
//     releases, from the computer panel.
//   - While the person holds control, the folk's computer actions are
...
export interface ControlSnapshot {
  /** True while the person is driving; the folk's hands are refused. */
  held: boolean;
...
export class ComputerControl {
   ...
   * the waiting folk resumes from this one state change. */
  ...
  /** The folk asks the person to take over. Never grants anything by
  ...
  requestHelpLease(botId: string, reason: unknown): { snapshot: ControlSnapshot; requestId: string } {
    ...
    if (entry.helpReason === null) {
      entry.helpReason = text || "the folk asked you to take over";
  ...
  /** The person declines without taking over; the waiting folk is told. */
