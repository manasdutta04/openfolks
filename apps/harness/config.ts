// Config + data dirs. One file, ~/.openfolks/config.json, env fallbacks:
...
interface StoredBrowserProfileMigration {
   ...
   * inherently ambiguous, so folks deterministically retain the first one. */
...
function migrateStoredBrowserProfiles(
  ...
  // reinterpret a previously migrated folk reference.
  ...
  // the old->new folk aliases below remain fixed points across repeated starts.
  ...
  const canonical: BrowserProfile[] = profiles.map((profile, index) => {
    ...
    // later duplicate records get isolated ids but existing folk references
...
const featureConfigSchema = z.object({
  ...
  /** Experimental built-in browser. Off until explicitly enabled; each folk
...
export interface AppConfig {
  ...
  /** Shared preserves the historical singleton. Per-folk gives every folk a
   * separate container, durable workspace, viewer and lease. */
  localVm?: { mode?: "shared" | "per-bot"; maxInstances?: number };
  /** Opt-in product experiments. Every flag defaults to disabled. */
  features?: { skillRecorder?: boolean; showToolCalls?: boolean; browser?: boolean };
  /** Named browser sessions any folk can be pointed at. */
  browserProfiles?: BrowserProfile[];
  instances?: InstanceConfigMap;
  /** OpenFolks-first-run state. Absent keys mean not yet configured. */
  openfolks?: { onboardingComplete?: boolean; projectFolder?: string };
}
...
export function parseStoredConfig(value: JsonValue): AppConfig {
  ...
  if (data.openfolks) return data;
  const raw = value && typeof value === "object" && !Array.isArray(value)
    ? value as Record<string, unknown>
    : {};
  const legacy = raw.opencrew;
  if (legacy && typeof legacy === "object" && !Array.isArray(legacy)) {
    const rec = legacy as Record<string, unknown>;
    data.openfolks = {
      onboardingComplete: rec.onboardingComplete === true ? true : undefined,
      projectFolder: typeof rec.projectFolder === "string" ? rec.projectFolder : undefined,
    };
  }
  return data;
}
 ...
 * hydration uses this to migrate folk references in the same write that
 * resets other transient folk state. Invalid/non-legacy config is inert. */
...
/** Workspace-level gate for the experimental built-in browser. A folk's own
...
function defaultDataDir(): string {
  const canonical = join(homedir(), ".openfolks");
  if (existsSync(canonical)) return canonical;
  for (const name of [".opencrew", ".openmausbot", ".opengrokbot"]) {
...
// OPENFOLKS_DATA_DIR / OCREW_DATA_DIR / OMB_DATA_DIR isolate test rigs.
export const DATA_DIR =
  process.env.OPENFOLKS_DATA_DIR
  ?? process.env.OCREW_DATA_DIR
...
/** Merge a partial config into ~/.openfolks/config.json (secrets never
...
// stdio mounts are passed through; URL mounts become a harness-owned stdio
// bridge (mcp-url-proxy) so every engine keeps one spawn shape.
// Validate-with-skip so one bad entry never takes the fleet down.
...
function urlMcpBridge(url: string, headers: Record<string, string>): CustomMcpServer {
  return {
    command: process.execPath,
    args: [SPAWNED_PROXIES.mcpUrl],
    env: {
      ELECTRON_RUN_AS_NODE: "1",
      OMB_MCP_UPSTREAM_URL: url,
      OMB_MCP_UPSTREAM_HEADERS: JSON.stringify(headers),
    },
  };
}

/** Redacted list for Settings UI — secrets stay on disk only. */
export function listMcpServersPublic(cfg: AppConfig): McpServerPublic[] {
  const out: McpServerPublic[] = [];
  for (const [name, raw] of Object.entries(cfg.mcpServers ?? {})) {
    if (!CUSTOM_MCP_NAME.test(name) || RESERVED_MCP_NAMES.has(name)) continue;
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) continue;
    const entry = raw as Record<string, unknown>;
    if ("url" in entry) {
      const parsed = customMcpUrlSchema.safeParse(entry);
      if (!parsed.success) continue;
      out.push({
        name,
        kind: "url",
        url: parsed.data.url,
        headerKeys: Object.keys(parsed.data.headers ?? {}),
        envKeys: [],
        enabled: parsed.data.enabled !== false,
      });
      continue;
    }
    const parsed = customMcpStdioSchema.safeParse(entry);
    if (!parsed.success) continue;
    out.push({
      name,
      kind: "stdio",
      ...
      envKeys: Object.keys(parsed.data.env ?? {}),
      headerKeys: [],
      enabled: parsed.data.enabled !== false,
    });
  }
...
/** Replace the whole mcpServers map (full unit of change, like browserProfiles). */
export function saveMcpServers(servers: Record<string, unknown>): void {
  const p = join(DATA_DIR, "config.json");
  let disk: JsonObject = {};
  try {
    const parsed = jsonObjectSchema.safeParse(parseJson(readFileSync(p, "utf8")));
  ...
  for (const name of Object.keys(servers)) {
    if (!CUSTOM_MCP_NAME.test(name)) throw Object.assign(new Error(`Invalid MCP server name: ${name}`), { status: 400 });
    if (RESERVED_MCP_NAMES.has(name)) {
      throw Object.assign(new Error(`MCP server name “${name}” is reserved`), { status: 400 });
    }
  }
  disk.mcpServers = servers as JsonObject;
  mkdirSync(DATA_DIR, { recursive: true });
  writeFileAtomic(p, JSON.stringify(disk, null, 2), { mode: 0o600 });
}

/** Validate one UI write payload into a durable config entry (keeps prior secrets when blank). */
export function mergeMcpServerEntry(
  ...
  if (!CUSTOM_MCP_NAME.test(name)) throw Object.assign(new Error("Invalid MCP server name"), { status: 400 });
  if (RESERVED_MCP_NAMES.has(name)) {
    throw Object.assign(new Error(`MCP server name “${name}” is reserved`), { status: 400 });
  }
  const prior =
    previous && typeof previous === "object" && !Array.isArray(previous)
      ...
      : {};
  if (patch.kind === "url") {
    const url = (patch.url ?? (typeof prior.url === "string" ? prior.url : "")).trim();
    if (!url) throw Object.assign(new Error("URL is required"), { status: 400 });
    const priorHeaders =
      prior.headers && typeof prior.headers === "object" && !Array.isArray(prior.headers)
        ? (prior.headers as Record<string, string>)
        : {};
    const headers = patch.clearHeaders
      ? {}
      : { ...priorHeaders, ...(patch.headers ?? {}) };
    const entry = { url, headers, enabled: patch.enabled !== false };
    const parsed = customMcpUrlSchema.safeParse(entry);
    if (!parsed.success) {
      throw Object.assign(new Error(parsed.error.issues[0]?.message ?? "Invalid URL MCP entry"), { status: 400 });
    }
    return parsed.data.enabled === false
      ? { url: parsed.data.url, headers: parsed.data.headers ?? {}, enabled: false }
      : { url: parsed.data.url, ...(Object.keys(parsed.data.headers ?? {}).length ? { headers: parsed.data.headers } : {}) };
  }
  const command = (patch.command ?? (typeof prior.command === "string" ? prior.command : "")).trim();
  if (!command) throw Object.assign(new Error("Command is required"), { status: 400 });
  const priorEnv =
    prior.env && typeof prior.env === "object" && !Array.isArray(prior.env)
      ? (prior.env as Record<string, string>)
      : {};
  const env = patch.clearEnv ? {} : { ...priorEnv, ...(patch.env ?? {}) };
  const entry = {
    command,
    args: patch.args ?? (Array.isArray(prior.args) ? prior.args.filter((a): a is string => typeof a === "string") : []),
    env,
    enabled: patch.enabled !== false,
  };
  const parsed = customMcpStdioSchema.safeParse(entry);
  if (!parsed.success) {
    throw Object.assign(new Error(parsed.error.issues[0]?.message ?? "Invalid stdio MCP entry"), { status: 400 });
  }
  return {
    command: parsed.data.command,
    args: parsed.data.args ?? [],
    ...(Object.keys(parsed.data.env ?? {}).length ? { env: parsed.data.env } : {}),
    ...(parsed.data.enabled === false ? { enabled: false } : {}),
  };
}
...
export function customMcpServers(cfg: AppConfig): Record<string, CustomMcpServer> {
  ...
  for (const [name, raw] of Object.entries(cfg.mcpServers ?? {})) {
    ...
    if (raw && typeof raw === "object" && "url" in raw) {
      const parsed = customMcpUrlSchema.safeParse(raw);
      if (!parsed.success) {
        skipMcpEntry(name, `invalid URL entry (${parsed.error.issues[0]?.message ?? "schema mismatch"}) — expected { "url": "https://…", "headers": { … } }`);
        continue;
      }
      ...
      try {
        ...
        if (parsedUrl.protocol !== "https:" && parsedUrl.protocol !== "http:") {
          skipMcpEntry(name, "URL MCP servers must use http or https");
          continue;
        }
        if (parsedUrl.protocol === "http:" && parsedUrl.hostname !== "127.0.0.1" && parsedUrl.hostname !== "localhost") {
          skipMcpEntry(name, "remote URL MCP servers must use https (http is only allowed on localhost)");
          continue;
        }
      } catch {
        skipMcpEntry(name, "URL is not valid");
        continue;
      }
      out[name] = urlMcpBridge(parsed.data.url, parsed.data.headers ?? {});
      continue;
    }
    const parsed = customMcpStdioSchema.safeParse(raw);
    if (!parsed.success) {
