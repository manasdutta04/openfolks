...
import { OpenFolksSetup } from "@/components/OpenFolksSetup";
...
function Shell() {
  ...
  const [openfolksSetupOpen, setOpenFolksSetupOpen] = useState(false);

  useEffect(() => {
    if (!state.connected) return;
    const complete = state.config?.openfolks?.complete;
    const needsCrew = state.bots.length === 0;
    setOpenFolksSetupOpen(needsCrew && complete !== true);
  }, [state.connected, state.config?.openfolks?.complete, state.bots.length]);

  // Nothing on this machine can run a folk. A missing cloud login does not
  ...
  // App-wide shortcuts: ⌘N new folk · ⌘, settings · ⌘1–9 jump · ⌘⇧[ / ⌘⇧] prev/next.
  ...
  useEffect(() => {
    return window.ogb?.onMenuCommand?.((command) => {
      if (command === "new-folk") dispatch({ type: "newBot" });
      if (command === "open-settings") dispatch({ type: "toggleAppSettings", open: true });
      if (command === "open-desk") dispatch({ type: "showDesk" });
      if (command === "open-marketplace") dispatch({ type: "togglePlugins", open: true, tab: "plugins" });
      if (command === "open-marketplace-folks") dispatch({ type: "togglePlugins", open: true, tab: "folks" });
      if (command === "open-skill-recorder") dispatch({ type: "showSkillRecorder" });
    });
  }, [dispatch]);
  ...
  // here (always mounted) when a folk's viewer closes. release() is idempotent.
  useEffect(() => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      ...
      {state.activeView === "desk" ? (
        ...
        <main className="flex h-full min-w-0 flex-1 flex-col items-center justify-center gap-3 bg-app text-ink-secondary">
          ...
          <div className="text-[14px]">
            {state.connected ? "No folks yet" : "Connecting to the folk server…"}
          </div>
          {!state.connected && (
      ...
      <CommandPalette />
      {openfolksSetupOpen && <OpenFolksSetup onDone={() => setOpenFolksSetupOpen(false)} />}
      </div>
    </div>
  );
}
