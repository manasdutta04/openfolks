...
export function teamImportPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format === "openfolks.package") return packagePreview(root, manifest);
  if (root.format !== "openfolks.team") throw new Error("This is not a BotMRR playbook or legacy OpenFolks team.");
...
function markdownPackage(markdown: string): unknown {
  ...
  return { format: "openfolks.package", version: 1, package: pkg };
}
...
function packagePreview(root: Record<string, unknown>, manifest: unknown): PendingTeamImport {
  ...
  if (!Array.isArray(pkg.agents) || pkg.agents.length === 0) throw new Error("This playbook has no folks.");
  if (pkg.agents.length > 200) throw new Error("This playbook has too many folks.");
  const members = pkg.agents.map((agent, index) => {
    if (!agent || typeof agent !== "object" || Array.isArray(agent)) throw new Error(`Folk ${index + 1} is invalid.`);
    const value = agent as Record<string, unknown>;
    if (typeof value.name !== "string" || !value.name.trim()) throw new Error(`Folk ${index + 1} does not have a name.`);
