...
const KEY = "openfolks-drafts";
const KEY_LEGACY = "omb-drafts";
const ATTACHMENTS_KEY = "openfolks-draft-attachments";
const ATTACHMENTS_KEY_LEGACY = "omb-draft-attachments";
const SEND_IDS_KEY = "openfolks-draft-send-ids";
const SEND_IDS_KEY_LEGACY = "omb-draft-send-ids";
...
function isBrowserStore(store: Store): boolean {
...
function readItem(store: NonNullable<Store>, key: string, legacyKey: string): string | null {
  if (isBrowserStore(store)) return readMigratedStorage(key, legacyKey);
  try {
...
function writeItem(store: NonNullable<Store>, key: string, value: string): void {
  if (isBrowserStore(store)) {
...
function read(store: Store, key: string, legacyKey: string): Values {
  if (!store) return {};
  try {
    const raw = readItem(store, key, legacyKey);
...
export function setDraftAttachments(store: Store, id: string, attachments: Attachment[]): void {
  if (!store) return;
  const drafts = read(store, ATTACHMENTS_KEY, ATTACHMENTS_KEY_LEGACY);
  if (attachments.length) drafts[id] = attachments;
  else delete drafts[id];
  writeItem(store, ATTACHMENTS_KEY, JSON.stringify(drafts));
}
...
export function restoredSendId(draftId: string): string | undefined {
  ...
  const stored = read(getStore(), SEND_IDS_KEY, SEND_IDS_KEY_LEGACY)[draftId];
...
function setStoredSendId(store: Store, draftId: string, sendId: string | undefined): void {
  if (!store) return;
  const ids = read(store, SEND_IDS_KEY, SEND_IDS_KEY_LEGACY);
  if (sendId) ids[draftId] = sendId;
  else delete ids[draftId];
  writeItem(store, SEND_IDS_KEY, JSON.stringify(ids));
}
...
/** useState for the composer text, persisted under `id` (a folk or room). */
