...
describe("desktop notifications", () => {
  ...
  it("shows a notification after permission is granted", () => {
    ...
    expect(notices[0]).toMatchObject({ title: frame.title, options: { body: frame.body, tag: `openfolks:${frame.botId}` } });
  });
  ...
  it("groups under the folk, not the thread", () => {
    const { notices } = installNotification("granted");
    ...
    expect(notices[0]?.options?.tag).toBe(`openfolks:${frame.botId}`);
    expect(notices[1]?.options?.tag).toBe(`openfolks:${frame.botId}`);
    showNotification({ ...frame, botId: "folk-2" }, vi.fn());
    expect(notices[2]?.options?.tag).toBe(`openfolks:folk-2`);
  });

  it("carries the folk's avatar when its profile has one", () => {
    const { notices } = installNotification("granted");
...
describe("buildNotificationOptions", () => {
  it("keys coalescing on botId and omits a missing avatar", () => {
    expect(buildNotificationOptions({ id: "folk-9" })).toEqual({
      tag: "openfolks:folk-9",
      icon: undefined,
    });
  });
});
