export interface FolksRadarBot {
  id: string;
  name: string;
  hidden?: boolean;
  section?: string;
...
export interface FolksRadarSnapshot {
...
export interface FolksRadarSection<T extends FolksRadarBot = FolksRadarBot> {
...
export interface FolksRadarStatus {
  label: string;
  tone: "success" | "warning" | "danger" | "idle";
}

export const EMPTY_FOLKS_RADAR_SNAPSHOT: FolksRadarSnapshot = {
  collaborations: [],
  queued: [],
  running: [],
};

export function buildFolksRadarSections<T extends FolksRadarBot>(bots: T[]): FolksRadarSection<T>[] {
...
export function buildFolksRadarEdges(bots: FolksRadarBot[], snapshot: FolksRadarSnapshot): FolksRadarEdge[] {
  const visible = new Set(bots.filter((bot) => !bot.hidden).map((bot) => bot.id));
  const edges = new Map<string, FolksRadarEdge>();
...
export function folksRadarStatus(bot: FolksRadarBot): FolksRadarStatus {
