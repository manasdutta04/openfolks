...
export function browserProfileDeletionBlockReason(
  ...
  return running.length === 1
    ? `${names} is still running. Stop that folk before deleting this browser profile.`
    : `${names} are still running. Stop those folks before deleting this browser profile.`;
}
