...
describe("sidebar virtual sections", () => {
  it("keeps reserved labels separate from identically named user sections", () => {
    for (const name of ["Pinned", "pinned", "channels", "folks", "folk-chats", "Folk Chats"]) {
  ...
  it("shows pinned folks once without erasing their saved context", () => {
  ...
  it("shows DMs in Folk Chats without rewriting their comms context", () => {
    const dm = { id: "dm", dm: true, section: "Work" };
    const namedBotChats = { id: "named", section: "Folk Chats" };
    ...
    expect(namedBotChats.section).toBe("Folk Chats");
  });
  ...
  it("forces filtered views open and non-reorderable", () => {
    ...
    expect(sidebarSectionCollapsed(PINNED_SECTION_ID, [PINNED_SECTION_ID], "comfortable", "")).toBe(true);
    expect(sidebarSectionCollapsed(PINNED_SECTION_ID, [PINNED_SECTION_ID], "comfortable", "writer")).toBe(false);
