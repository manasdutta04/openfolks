...
export function groupResponseHint(group: Group, members: Bot[]): string {
  if (group.dm) return "Reply here to continue the folk-to-folk conversation.";
  const value = effectiveDefaultResponder(group, members);
  if (value.kind === "everyone") return "Everyone responds unless you @mention specific folks.";
  if (value.kind === "mentions") return "Mention a folk with @ to bring them in.";
  const name = defaultResponderName(group, members) ?? "The lead folk";
...
export function groupComposerHint(group: Group, members: Bot[]): string {
  ...
  if (value.kind === "mentions") return "@ to bring a folk in";
