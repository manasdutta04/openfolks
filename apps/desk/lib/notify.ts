...
import type { Notification } from "../../harness/notify.ts";
...
/** The identity a notification groups under: one folk, wherever it was
 * working. Keyed by folk rather than thread so a single folk running across
...
/** Presentation options for one folk's notifications: the stable per-bot
...
export function buildNotificationOptions(bot: NotificationBotIdentity): NotificationOptions {
  return { tag: `openfolks:${bot.id}`, icon: bot.avatarUrl ?? undefined };
}
