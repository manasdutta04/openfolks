...
type SkillRecordingPayload = {
  ...
  interface Window {
    ogb?: {
      ...
      /** Receives a GitHub package URL opened through openfolks://install. */
      ...
      /** Open a native submenu under the Windows title strip. */
      popupMenu?(label: string, x: number, y: number): Promise<boolean>;
      /** Opens a live desktop as a sandboxed window owned by OpenFolks. */
      desktopViewer?: {
        open(url: string, title: string, contextId: string): Promise<boolean>;
        /** Closes the live-desktop window, but only when it belongs to this folk. */
      ...
      /** Asks where to save a folk-created file (inside ~/.openfolks), copies
      ...
      setCredential?(
        name: "composioApiKey" | "xaiApiKey" | "boxToken" | "opencodeGoApiKey" | "openaiCompatApiKey" | "ttsKey" | "openaiImageApiKey",
        value: string,
