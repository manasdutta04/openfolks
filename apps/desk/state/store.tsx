...
import type { CloudBackend, EffortLevel } from "../../harness/contracts.ts";
...
export interface Message {
  ...
  /** screen messages: a frame of the folk's computer (base64) */
  png?: string;
  mime?: string;
  at: number;
  ...
  /** comm chips: "Messaged @X" linking to the folk⇄folk channel. */
  comm?: { groupId: string; withBotId: string; withName: string; withColor: MausColor };
  /** sent while the folk was mid-turn; auto-sends when the turn settles.
   * Rendered only while the folk is busy, so a flag stranded by a server
...
/** A room: several folks + you in one shared thread. */
export interface Group {
  ...
  unread: boolean;
  createdAt: number;
  /** auto-created folk⇄folk channel (ask_bot exchanges mirror here) */
  dm?: boolean;
  ...
  /** sidebar section heading this room is filed under (shared with folks) */
  section?: string;
...
/** One of a folk's separate contexts: its own thread, transcript and
...
export interface Bot {
  ...
  /** Body silhouette slug; unused after Pebble avatars. */
  mascotShape?: string | null;
  /** SpaceUI Pebble identity. Unique per auto-created folk. */
  ...
  unread: boolean;
  busy?: boolean;
  /** what the folk is doing, as the harness sees it; busy is derived from it */
  activity?: "working" | "waiting-on-you" | "idle" | "no-signal" | "dead";
  modelSelection: ModelSelection;
  /** Where this folk's computer runs; unset = auto (cloud box if one exists, else local). */
  ...
  /** this folk's own voice id (falls back to the app-wide one) */
  voice?: string;
  pinned?: boolean;
  hidden?: boolean;
   ...
   * allowed for existing folks; imported folks start with this disabled. */
  composio?: boolean;
  /** Whether this folk gets the app's built-in browser (Browser tab). On unless switched off. */
  browser?: boolean;
...
/** Newest descendant of `messageId` — same walk as switching versions. */
export function newestLeafFrom(messages: Message[], messageId: string): string {
  let cur = messageId;
  for (;;) {
    const children = messages.filter((m) => m.parentId === cur);
...
/** An edited prompt is a sibling of the visible one, not a child of the
 * current leaf (usually the old reply). Follow that fork so the transcript
 * shows the latest edit instead of staying on version 1. */
function isEditedPromptFork(visible: Message[], message: Message): boolean {
  return (
    message.role === "user" &&
    message.kind === "text" &&
    visible.some(
...
export interface ConfigStatus {
  ...
  /** Optional legacy profile fields — no longer collected in the UI. */
  ...
  openfolks?: { complete: boolean; projectFolder: string | null };
  /** Redacted custom MCP servers (no env/header values). */
...
export type ConfigStatusFrame = Pick<
  ConfigStatus,
  "xai" | "composio" | "box" | "vps" | "rooms" | "localVm" | "opencodeGo" | "openaiCompat" | "tts" | "imageGen" | "profile" | "language" | "features" | "browserProfiles" | "openfolks" | "mcpServers"
>;
...
export interface AppState {
  ...
  /** selected chat — a folk id OR a group id */
  selectedId: string;
  activeView: "chat" | "desk" | "skill-recorder";
  routines: Routine[];
  ...
  pluginsOpen: boolean;
  /** Spotlight-style universal search overlay */
  universalSearchOpen: boolean;
  /** Marketplace modal top-level tab when pluginsOpen */
  marketplaceTab: "plugins" | "folks";
  /** Deep-link GitHub/package URL to preview under Folks → Import */
  ...
  /** latest live frame of a folk's computer, per botId */
  screens: Record<string, { png: string; mime: string }>;
  /** bots whose cloud computer is being provisioned */
  provisioning: Record<string, boolean>;
  /** who is driving each bot's computer: held = the person has the wheel
   * (the folk's hands are refused server-side); helpReason = the bot's open
...
export type Action =
  ...
  | { type: "togglePlugins"; open?: boolean; tab?: "plugins" | "folks"; installUrl?: string | null }
  | { type: "toggleUniversalSearch"; open?: boolean }
...
export function openNotificationTarget(
  ...
  // A room's approval/question notification carries the asker folk with the
  ...
  // bot's own lands on a plain folk select instead of an error banner.
...
/** First-run quiz still sitting on this folk's thread. */
  ...
  switch (action.type) {
    ...
    case "messageAdded": {
      ...
      const next = updateBot(state, bot.id, (b) => {
        ...
        // append). An edited prompt is a sibling of the visible one, so it
        // must also take the leaf or the transcript stays on version 1.
        // Anything else parented elsewhere is a late turn artifact
        // (settle-time screenshot) — the leaf stays put so a raced
        // follow-up does not fall off the active branch.
        const visible = visibleMessages(b);
        const parent = action.message.parentId
          ? b.messages.find((m) => m.id === action.message.parentId)
          : undefined;
        const adoptsLeaf =
          (action.message.parentId ?? null) === (b.activeLeafId ?? null) ||
          isEditedPromptFork(visible, action.message) ||
          Boolean(parent && isEditedPromptFork(visible, parent));
        ...
        return {
          ...b,
          messages,
          activeLeafId: adoptsLeaf ? newestLeafFrom(messages, action.message.id) : b.activeLeafId,
        };
      });
      const motion =
    ...
    case "togglePlugins": {
      ...
      return {
        ...state,
        pluginsOpen: open,
        ...(action.tab !== undefined ? { marketplaceTab: action.tab } : {}),
        marketplaceInstallUrl: !open
          ? null
          : action.installUrl !== undefined
            ? action.installUrl
            : state.marketplaceInstallUrl,
      };
    }
    case "toggleUniversalSearch": {
      const open = action.open ?? !state.universalSearchOpen;
      return { ...state, universalSearchOpen: open };
    }
    case "focusMessage":
      return {
        ...state,
    ...
    case "toggleAppSettings": {
      ...
      return {
        ...
        marketplaceInstallUrl: open ? null : state.marketplaceInstallUrl,
      };
    }
    case "updateBot": {
      const mascotChanged =
        Object.prototype.hasOwnProperty.call(action.patch, "color") ||
        Object.prototype.hasOwnProperty.call(action.patch, "avatarSeed") ||
    ...
    case "threadActive": {
      const bot = state.bots.find((b) => b.threadId === action.threadId);
      if (!bot) return state;
      // The server may announce the edited prompt before its reply has
      // been folded in. Walk to the newest child so we do not hide a
      // response that already arrived.
      const activeLeafId = newestLeafFrom(bot.messages, action.activeLeafId);
      return updateBot(state, bot.id, (b) => ({ ...b, activeLeafId }));
    }
    // optimistic leaf move; the server's thread frame confirms it later
    case "switchBranch": {
      const bot = state.bots.find((b) => b.id === action.botId);
      if (!bot) return state;
      const cur = newestLeafFrom(bot.messages, action.messageId);
...
const StoreContext = createContext<{
  ...
  /** Commit any debounced profile edits before an operation reads the folk. */
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const dispatch = useMemo(() => {
    ...
    const wrapped: React.Dispatch<Action> = (action) => {
      ...
      // Same for folk delete: an optimistic remove that the server rejects
      // (busy turn, Local VM still present, …) used to leave a ghost that
      // came back on the next hydrate and felt like "delete does nothing".
      if (
      ...
      switch (action.type) {
        ...
        case "deleteBot": {
          // Archive-panel flow does DELETE then dispatch(deleteBot). Without
          // this guard the second call 404s and never updates local state, so
          // the folk looks immortal. Treat "already gone" as success.
          if (!stateRef.current.bots.some((bot) => bot.id === action.botId)) break;
          void api(`/api/bots/${action.botId}`, { method: "DELETE" })
            .then(() => rawDispatch(action))
            .catch((error) => {
              const message = error instanceof Error ? error.message : String(error);
              if (/no such folk|not found|404/i.test(message)) {
                rawDispatch(action);
                return;
              }
              showError(error);
        ...
        case "interrupt":
          ...
          break;
        // tasks: the server answers with the folk AND the live transcript,
        ...
        case "deleteTask":
          api(`/api/bots/${action.botId}/tasks/${action.threadId}`, { method: "DELETE" })
            ...
            .catch(showError);
          break;
        // Channel tasks mirror folk tasks, but hydrate the whole channel so
  ...
  useEffect(() => {
    ...
    handleFrame = (frame) => {
      ...
      switch (frame.kind) {
        ...
        // in each folk's settings is what gates it, server-side
        case "notify":
