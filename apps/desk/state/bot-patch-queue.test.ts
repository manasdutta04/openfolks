...
describe("folk patch queue", () => {
  ...
  it("coalesces upload then remove so an older avatar can never resurrect", async () => {
    ...
    queue.enqueue("folk-1", { avatarUrl: null, avatarCrop: "mascot" }, bot());
  ...
  it("serializes in-flight profile edits and overlays only the later values", async () => {
    ...
    queue.enqueue("folk-1", { name: "First" }, bot());
  ...
  it("reconciles a rejected optimistic profile value to the server folk", async () => {
    const authoritative = vi.fn();
    const onError = vi.fn();
    ...
    queue.enqueue("folk-1", { name: "x".repeat(101) }, bot());
    await vi.advanceTimersByTimeAsync(400);
  ...
  it("does not restore a folk whose queued mutation was cancelled for deletion", async () => {
    ...
    queue.enqueue("folk-1", { name: "Pending" }, bot());
    await vi.advanceTimersByTimeAsync(400);
    queue.cancel("folk-1");
    ...
    expect(queue.overlayFor("folk-1")).toEqual({});
  });

  it("does not restore a folk cancelled while a failed mutation is reconciling", async () => {
  ...
  it("carries acknowledgeLocalAuto to the wire but never into a state overlay", async () => {
    ...
    // reach the HTTP body — and must never fold back into renderer folk state.
    const sent: BotUpdatePatch[] = [];
    ...
    queue.enqueue("folk-1", { computer: "local", acknowledgeLocalAuto: true }, bot());
    queue.enqueue("folk-1", { title: "Ops" }, bot());
    expect(queue.overlayFor("folk-1")).toEqual({ computer: "local", title: "Ops" });
    await vi.advanceTimersByTimeAsync(400);
    await queue.flush("folk-1");
  ...
  it("revive undoes a dispose, so StrictMode's dev probe cannot kill saving", async () => {
    ...
    queue.dispose();
    queue.revive();
    queue.enqueue("folk-1", { title: "still saves" }, bot());
    await vi.advanceTimersByTimeAsync(400);
    await queue.flush("folk-1");
