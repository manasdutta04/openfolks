...
describe("browser panel address and profile helpers", () => {
  ...
  it("coalesces native focus and input and still reopens a controlled compact page", () => {
    ...
    expect(browserInteractionPlan({ ...first, eventBotId: "folk-2" })).toBe("ignore");
    expect(browserInteractionPlan({ ...first, eventProfile: "profile-personal" })).toBe("ignore");
  });

  it("locks browser profile changes while a folk turn or local browser transition is active", () => {
  ...
  it("never leaks the old profile's page or address into a new selection", () => {
    ...
    expect(browserSurfaceForProfile(work, "folk-1", "profile-work")).toBe(work);
    expect(browserSurfaceForProfile(work, "folk-1", "profile-personal")).toBeNull();
    expect(browserSurfaceForProfile(work, "folk-2", "profile-work")).toBeNull();
    expect(browserSurfacePresentation({
      surface: work,
      botId: "folk-1",
      profile: "profile-personal",
    })).toBe("connecting");
    expect(shouldAcceptBrowserSurfaceState(work, "folk-1", "profile-personal")).toBe(false);
  ...
  it("derives explicit connecting, empty, loading, ready, and failed page states", () => {
    const common = { botId: "folk-1", profile: "profile-work" };
