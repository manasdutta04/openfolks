...
export function SkillRecorderPage() {
  ...
  const start = async () => {
    ...
    if (!bridge || !window.ogb?.beginScreenPreviewIntent || !navigator.mediaDevices?.getDisplayMedia) {
      setError("Skill recording requires the OpenFolks desktop app on macOS.");
      return;
    }
    if (!transcriptionConfigured || !window.ogb.transcription) {
      setError("Add your AssemblyAI key under Settings → Experimental before recording.");
      return;
    }
    ...
    <main className="flex min-w-0 flex-1 flex-col overflow-hidden bg-app text-ink">
      ...
      <header className="flex h-[60px] shrink-0 items-center justify-between border-b border-hairline px-6">
        <div>
          ...
          <p className="text-[11px] text-ink-secondary">Show it once. Let every folk repeat it.</p>
        </div>
        {recording && (
        ...
        <div className="mx-auto w-full max-w-4xl px-6 py-8">
          {phase === "idle" || phase === "starting" ? (
            <div className="mx-auto max-w-2xl">
              <div className="rounded-3xl border border-hairline bg-panel p-8 shadow-sm">
                ...
                <p className="mt-2 max-w-xl text-[14px] leading-6 text-ink-secondary">
                  Speak naturally while you work. OpenFolks lines up your clicks, app changes, screenshots, and narration, then turns the reviewed demonstration into a reusable local skill.
                </p>
                ...
                <div className="mt-4 flex items-center gap-3 rounded-2xl border border-hairline bg-card p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[11px] leading-4 text-ink-secondary">
                      {transcriptionConfigured ? "AssemblyAI is ready for live narration." : "Add an AssemblyAI key under Settings → Experimental before recording."}
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => dispatch({ type: "toggleAppSettings", open: true, section: "experimental" })}
            ...
            <div>
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-1 text-[24px] font-semibold tracking-[-0.02em]">Review what the folk learned</h2>
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
                ...
                <aside className="h-fit rounded-2xl border border-hairline bg-panel p-5 lg:sticky lg:top-0">
                  ...
                  <label className="mt-4 block text-[11px] font-medium text-ink-secondary" htmlFor="skill-description">When should folks use it?</label>
            ...
            <div className="mx-auto max-w-xl rounded-3xl border border-hairline bg-panel p-9 text-center">
              ...
              <p className="mt-2 text-[13px] leading-5 text-ink-secondary">Folks will load <span className="text-ink">{name}</span> automatically when a request matches it.</p>
