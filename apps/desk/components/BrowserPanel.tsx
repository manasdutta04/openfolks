...
export function BrowserPanel({
  ...
  // Async profile creation must only observe committed folk state. Updating the
  ...
  // a profile that was deleted falls back to the folk's own session; Guest is
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // The tab is gone; the page stays alive (a folk mid-task keeps its tab)
    ...
    async (action: "take" | "release"): Promise<boolean> => {
      ...
      try {
        ...
        const setLocalControl = async (held: boolean): Promise<boolean> => {
          try {
            if (!bridge?.setHumanControl) throw new Error("Update OpenFolks before using browser takeover.");
        ...
        if (result.failed === "durable-take") {
          ...
          setError("Browser control could not be confirmed. The folk remains paused here for safety — retry Take control.");
        } else if (result.failed === "durable-release") {
          setError("Control could not be handed back. The folk remains paused here for safety — retry Hand back.");
  ...
  const moveHistory = useCallback(async (direction: "back" | "forward") => {
    ...
    try {
      ...
      else throw new Error("Update OpenFolks before using Forward.");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : String(cause));
    } finally {
      setBusy(false);
  ...
  const chooseProfile = async (value: string) => {
    ...
    setError(null);
    try {
      // Let the server serialize this against turn start. An optimistic folk
  ...
  if (!bridge) {
    return (
      <div className="rounded-xl bg-card p-4 text-[13px] text-ink-secondary">
        The built-in browser needs the OpenFolks desktop app.
      </div>
    );
  }
    ...
    <div className={cn("flex h-full min-h-0 flex-col", !expanded && "overflow-y-auto overscroll-contain pr-0.5")}>
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        <div className="min-w-0 text-[12px] leading-relaxed text-ink-secondary">
          {control.held
            ? "You have the wheel — the folk waits until you hand it back."
            : "Click into the page to take over any time; the folk pauses while you drive."}
        </div>
        <button
          type="button"
      ...
      {/* Profile: which session (cookies, logins) this folk's tab uses. */}
      <div className="mt-3 rounded-xl border border-hairline/30 bg-card p-3">
        ...
        <div className="mt-1.5 text-[11.5px] leading-relaxed text-ink-secondary">
          {profileChangesLocked
            ...
            : `Profiles keep logins separate. “${bot.name}'s own” is private to this folk; Guest is cleared when you switch away.`}
        </div>
      </div>
      {error && (
