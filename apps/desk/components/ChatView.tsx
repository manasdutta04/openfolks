 ...
 * bury the conversation; folks get full markdown. */
...
function DaySeparator({ at }: { at: number }) {
  return (
    <div data-day-sep className="py-3 text-center text-[13px] text-ink-secondary">
...
/** Hover/focus-revealed copy control shared by user + folk bubbles. */
  ...
  return (
    <div className="flex w-full items-center justify-end gap-1.5">
      ...
      <button
        ...
        title="Send (Enter)"
        className="rounded-md p-1.5 text-ink-secondary hover:bg-raised hover:text-ink disabled:opacity-30"
      >
        <Check size={14} />
      </button>
      <div className="relative w-fit min-w-[8rem] max-w-[min(42rem,78%)] rounded-2xl bg-bubble-user">
        <div
          aria-hidden
          className="invisible whitespace-pre-wrap break-words px-4 py-2.5 text-[15px] leading-relaxed"
        >
          {draft || "\u00a0"}
          {draft.endsWith("\n") ? "\n" : null}
        </div>
        <textarea
          ...
          rows={1}
          className="absolute inset-0 resize-none bg-transparent px-4 py-2.5 text-[15px] leading-relaxed text-ink focus:outline-none"
        />
      </div>
    </div>
  );
        ...
        >
          ...
          {user && webhookView ? (
            ...
            <>
              ...
              {message.steered && (
                <div className="mt-1 text-[11px] text-ink-secondary/70" title="Sent while the folk was working — it saw this before its next step, inside the same turn.">
                  sent mid-turn
                </div>
              )}
              {collapsible && (
...
function ActivityChip({ message }: { message: Message }) {
  ...
  if (!tool) return null;
  // folk⇄folk comm chip: opens the channel where the exchange lives
  ...
  if (comm) {
    return (
      <div className="flex justify-start">
        ...
        >
          <MausAvatar seed={comm.withName} size={16} />
 ...
 * (folk/messages only change on real message events), so the whole list —
...
const MessagesList = memo(function MessagesList({
  ...
  /** This folk's engine, for rendering setup help on a `setup` error. */
      ...
      {messages.length === 0 && !bot.busy && (
        <div className="flex flex-1 flex-col items-center justify-center gap-3 py-24 text-center">
          <button
            ...
            className="rounded-2xl p-1 hover:bg-raised/50"
            aria-label={`Open ${bot.name}'s settings`}
          >
          ...
          <button
            type="button"
            onClick={() => dispatch({ type: "toggleSettings", open: true })}
            className="text-[17px] font-semibold text-ink hover:underline"
          >
            {bot.name}
          </button>
        ...
        const row = (() => {
          switch (m.kind) {
            ...
            case "activity": {
              ...
              // folk⇄folk comm chips stay because they link to another conversation.
...
export function ChatView({ bot }: { bot: Bot }) {
  ...
  // full threads DOM-heavy). The boundary is anchored per folk+task; a
  ...
  // turn starts, cleared when it settles, reset on folk switch.
      ...
      >
        <div className="flex min-w-0 items-center gap-2.5 rounded-lg px-1.5 py-1">
          ...
          <button
            type="button"
            onClick={() => dispatch({ type: "toggleSettings", open: true })}
            className="min-w-0 truncate text-left text-[15px] font-semibold text-ink hover:underline"
            title="Open folk settings"
          >
            {bot.name}
          </button>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            title="Backstage — live turns and engine protocol for this thread"
          >
            <Radio size={18} />
          </button>
        </div>
          ...
          {provisioning && (
            <div className="flex justify-start">
              <div className="flex items-center gap-2 rounded-full border border-hairline/40 bg-panel px-3 py-1.5 text-[13px] text-ink-secondary">
                <WorkingDots size={3.5} />
                Setting up this folk's computer…
              </div>
            </div>
          )}
          <TurnPresence
