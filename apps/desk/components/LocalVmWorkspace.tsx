...
function LocalVmPane({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!bridge) {
        setError("The two-desktop workspace requires the OpenFolks desktop app.");
        return;
      }
      try {
        ...
        setError(
          cause instanceof Error && cause.message === "layout-unavailable"
            ...
            : "OpenFolks could not connect this Local VM viewer.",
        );
      }
    };
  ...
  const updateLayout = useCallback(() => {
    ...
    void bridge
      ...
      .catch(() => setError("OpenFolks could not position this Local VM viewer."));
    ...
    >
      <div className="flex min-h-[68px] items-center gap-3 border-b border-hairline/40 px-3 py-2.5">
        <div className="min-w-0 flex-1">
          ...
          >
            <option value="">Choose a Local VM folk</option>
            {bots.map((candidate) => (
        ...
        {bot && active ? (
          ...
          <button
            ...
            title="Pause only this folk and enable keyboard and pointer input in this pane"
          >
            {controlPending ? <Loader2 size={13} className="animate-spin" /> : <Hand size={13} />}
        ...
        <div className="absolute inset-0 flex items-center justify-center p-6 text-center">
          {!bot ? (
            <div className="flex max-w-[260px] flex-col items-center gap-2 text-ink-secondary">
              <Monitor size={22} />
              <span className="text-[12px]">Choose another folk configured for a Local VM.</span>
            </div>
          ) : !status && !error ? (
...
export function LocalVmWorkspace({
  ...
  const handBack = useCallback(async () => {
    ...
    try {
      ...
      setControlError("OpenFolks could not hand control back. The workspace stayed open.");
      return false;
    } finally {
