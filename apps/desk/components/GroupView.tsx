...
import { ArrowDown, Check, ChevronDown, Clock, Folder, FolderOpen, Loader2, MessageSquareReply, Pin, Plus, X } from "lucide-react";
import {
  api,
  useStore,
  useStreaming,
...
import { avatarSeedFor } from "../../../shared/pebble-avatars";
...
import { shortPath, workingFolderLabel } from "@/lib/short-path";
...
function ClusterLabel({ bot, name }: { bot?: Bot; name: string; color: string }) {
  return (
    <div className="mt-1 flex items-center gap-1.5 pl-0.5">
      <MausAvatar
        seed={bot ? avatarSeedFor(bot) : name}
        size={16}
        animated={false}
      />
...
const Transcript = memo(function Transcript({
  ...
  // Several folks working at once turn a room into a wall of chips; fold the
    ...
    <>
      {items.map((item, i) => {
        ...
        const row =
          ...
          // card never rendered here and the folk waited out its timeout.
  ...
  const title =
    responder.kind === "everyone"
      ...
      : responder.kind === "mentions"
        ? "Only explicitly @mentioned folks respond"
        : `Plain messages go to ${lead?.name ?? "the lead folk"}; @mentions override this`;
 ...
 * overriding each folk's own folder for room turns. The room pins its own
...
function RoomWorkingFolder({ group }: { group: Group }) {
  ...
  return (
    <div className="rounded-xl bg-card p-4">
      ...
      <div className="mt-0.5 text-[13px] text-ink-secondary">Where every folk in this channel runs its shell and file tools.</div>
      {locked ? (
        <div className="mt-3">
          <div className="truncate rounded-lg border border-hairline/40 bg-inset px-3 py-2 font-mono text-[12.5px] text-ink" title={shownCwd}>
            {shownCwd ? shortPath(shownCwd, home) : <span className="text-ink-secondary">Each folk's own folder</span>}
          </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          <div className="min-w-0 flex-1 truncate rounded-lg border border-hairline/40 bg-inset px-3 py-2 font-mono text-[12.5px] text-ink" title={group.cwd}>
            {group.cwd ? shortPath(group.cwd, home) : <span className="text-ink-secondary">Each folk's own folder</span>}
          </div>
        ...
        >
          <input
            ...
            placeholder="Each folk's own folder — or an absolute path"
...
function RoomWorkingFolderChip({ group, onToggle }: { group: Group; onToggle: () => void }) {
  ...
  const name = workingFolderLabel(folder);
  return (
    <button
      onClick={onToggle}
      className="flex max-w-[160px] items-center gap-1.5 rounded-full border border-hairline/40 bg-raised/60 px-2.5 py-1 text-[12px] text-ink-secondary hover:bg-raised hover:text-ink"
        ...
        <label className="block">
          ...
          <div className="mt-2 flex gap-2">
            <input
              ...
              placeholder="Each folk's own folder"
                  ...
                  <div className="max-h-48 overflow-y-auto p-1.5">
                    {members.map((member) => {
                      ...
                      return (
                        ...
                        >
                          <BotAvatar bot={member} size={24} animated={false} />
...
export function GroupView({ group }: { group: Group }) {
  ...
  const memberMauses = members.map((b) => (
    ...
    >
      <BotAvatar bot={b} size={24} animated={false} />
        ...
        <div className="flex items-center gap-1.5">
          ...
          {group.dm ? (
            ...
            <button
              ...
              aria-label={`Manage members — ${members.length} ${members.length === 1 ? "folk" : "folks"} in this channel`}
        ...
        {bulletinOpen ? (
          <div className="mb-1 rounded-lg border border-hairline/40 bg-panel p-2">
            <textarea
              ...
              placeholder="Channel instructions — every folk in this channel follows them (who does what, tone, goals, a task checklist…)"
              rows={4}
          ...
          <button
            ...
            title="Channel bulletin — shared instructions for every folk here"
          >
        ...
        >
            ...
            <div className="flex flex-1 flex-col items-center justify-center gap-3 py-24 text-center">
              <div className="flex -space-x-2">
                {members.slice(0, 3).map((b) => (
                  <BotAvatar key={b.id} bot={b} size={44} animated={false} />
                ))}
              </div>
          ...
          {(speaker || presenceVisible) && (
            <TurnPresence
              avatar={
                <MausAvatar
                  seed={presenceSpeaker ? avatarSeedFor(presenceSpeaker) : "folk"}
                  size={36}
                  animated={toolInFlight}
                />
              }
