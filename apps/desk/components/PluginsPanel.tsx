// Marketplace modal: Plugins (Composio connected apps) + Bots (team playbooks
// and community bots). Plugin catalog comes from /api/connectors/catalog —
// full toolkit list with logos when a Composio API key is configured, curated
// otherwise. Icons resolve logo → favicon → monogram.
import { useCallback, useEffect, useRef, useState } from "react";
import { Bot, Check, Loader2, Plug, RefreshCw, Search, TriangleAlert, X } from "lucide-react";
...
export function PluginsPanel() {
  ...
  const marketplaceTab = state.marketplaceTab;
  ...
  const [folksFeedback, setFolksFeedback] = useState<string | null>(null);
  const [pendingInstallUrl, setPendingInstallUrl] = useState<string | null>(
    () => state.marketplaceInstallUrl,
  );

  useEffect(() => {
    if (!state.marketplaceInstallUrl) return;
    setPendingInstallUrl(state.marketplaceInstallUrl);
    dispatch({ type: "togglePlugins", open: true, tab: "folks", installUrl: null });
  }, [state.marketplaceInstallUrl, dispatch]);

  useEffect(() => {
    if (!folksFeedback) return;
    const timer = window.setTimeout(() => setFolksFeedback(null), 4000);
    return () => window.clearTimeout(timer);
  }, [folksFeedback]);

  const setMarketplaceTab = (next: "plugins" | "folks") => {
    dispatch({ type: "togglePlugins", open: true, tab: next });
  };

  const onFolksImported = (result: TeamImportResult) => {
    setFolksFeedback(
      `${result.name} loaded · ${result.members} ${result.members === 1 ? "folk" : "folks"}`,
    );
    setPendingInstallUrl(null);
    if (result.importedGroupIds.length > 0 || result.members > 1) {
  ...
  const visible = matching.filter((card) =>
    tab === "browse" || status[card.slug]?.connected || Boolean(status[card.slug]?.accounts?.length)
  );
      ...
      >
        <header className="flex items-start justify-between gap-4 px-6 pb-3 pt-6 sm:px-8 sm:pt-7">
          <div className="min-w-0">
            <h2 id="marketplace-title" className="text-[22px] font-semibold tracking-[-0.01em] text-ink">Marketplace</h2>
            <p className="mt-1 text-[13px] text-ink-secondary">
              {marketplaceTab === "folks"
                ? "Browse team playbooks and community folks."
                : "Connect the apps your folks can use."}
            </p>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <div className="flex rounded-full bg-raised/70 p-1" role="tablist" aria-label="Marketplace section">
              <button
                role="tab"
                aria-selected={marketplaceTab === "plugins"}
                onClick={() => setMarketplaceTab("plugins")}
                className={cn(
                  "flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[13px] transition-colors",
                  marketplaceTab === "plugins" ? "bg-card text-ink shadow-sm" : "text-ink-secondary hover:text-ink",
                )}
              >
                <Plug size={14} />
                Plugins
              ...
              <button
                role="tab"
                aria-selected={marketplaceTab === "folks"}
                onClick={() => setMarketplaceTab("folks")}
                className={cn(
                  "flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[13px] transition-colors",
                  marketplaceTab === "folks" ? "bg-card text-ink shadow-sm" : "text-ink-secondary hover:text-ink",
                )}
              >
                <Bot size={14} />
                Folks
        ...
        {marketplaceTab === "folks" ? (
          <>
            {folksFeedback && (
              <div
                role="status"
                className="mx-6 mb-2 rounded-lg border border-hairline/40 bg-raised/50 px-3 py-2 text-[12.5px] text-ink sm:mx-8"
              >
                {folksFeedback}
              </div>
            )}
        ...
        {stale && (
          ...
          <div className="mx-6 mb-1 flex items-start gap-2 rounded-lg border border-warning/30 bg-warning/10 px-3 py-2 text-[12.5px] text-warning sm:mx-8">
            ...
            <span>
              ...
              could not be re-checked. Your apps are still connected; restarting OpenFolks usually clears this.
            </span>
          </div>
        )}

        <div className="flex flex-col gap-3 px-6 pb-4 pt-5 sm:flex-row sm:items-center sm:justify-between sm:px-8">
          <div className="flex w-fit rounded-xl bg-raised/70 p-1" role="tablist" aria-label="Plugins view">
            <button
              role="tab"
              ...
              className={cn(
                ...
                tab === "browse" ? "bg-card text-ink shadow-sm" : "text-ink-secondary hover:text-ink",
              )}
            >
              Browse
            </button>
