...
// keeps every message a departing folk already sent.
...
export function ManageMembersPanel({
  ...
  // Archived folks stay listed while they are still members — otherwise a
  ...
  return (
    ...
    >
      ...
      >
        ...
        <BotPickerList bots={bots} picked={picked} onToggle={toggle} emptyHint="Create a folk first — channels are made of folks." />
        {!memberIds.length && <div className="mt-2 text-[12px] text-ink-secondary">A channel needs at least one folk.</div>}
        {saveError && (
        ...
        <div className="mt-3 flex gap-2">
          ...
          >
            Save{memberIds.length ? ` · ${memberIds.length} ${memberIds.length === 1 ? "folk" : "folks"}` : ""}
          </button>
        </div>
      </div>
    </div>
