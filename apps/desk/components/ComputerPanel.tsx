...
import { readMigratedStorage, writeStorage } from "@/lib/storage-key.js";
...
const PANEL_WIDTH_KEY = "openfolks-computer-panel-width";
const PANEL_WIDTH_KEY_LEGACY = "omb-computer-panel-width";
...
function readPanelWidth(): number {
  try {
    const stored = Number(readMigratedStorage(PANEL_WIDTH_KEY, PANEL_WIDTH_KEY_LEGACY));
...
export function ComputerPanel({
  ...
  // bumped to re-run spin-up / readiness after provisioning actions
  ...
  // Pause the screenshot poll while this folk's viewer is open; seed from the
  ...
  useEffect(() => {
    // Browser owns its own live surface. Do not provision a VM,
    // wake a box, or churn preview state behind that tab.
    ...
    if (cloudBackend === "vps") {
      ...
      api(`/api/bots/${bot.id}/computer`)
        .then((rawStatus) => {
          ...
          // App updates can bump IMAGE_LAYER_VERSION while this folk still has
          ...
          setError(
            bot.autoStartVps
              ? `${status.problem ?? "No ready VPS container"}. Auto will prepare or wake it when this folk next works.`
  ...
  // cloud preview: SSE frames win while the folk works; otherwise poll.
  // Every preview poll below gates on visibility and slows way down for an
  // idle folk — a drawer left open overnight must not keep shooting.
  ...
  useEffect(() => {
    ...
    void shoot();
    // A real ScreenCaptureKit capture + PNG encode per tick: idle folks get a
  ...
  const openDesktop = async () => {
    ...
    try {
      ...
      if (window.ogb?.desktopViewer) {
        ...
        if (!opened) throw new Error("OpenFolks could not open the live desktop");
      } else if (fallbackTab) {
        ...
        if (!opened) throw new Error("OpenFolks could not open the live desktop link");
  ...
  const replaceVpsComputer = async () => {
    if (!window.confirm(`Replace ${bot.name}'s VPS computer with the version required by this OpenFolks update? Files stored only inside the disposable container will be deleted.`)) return;
  ...
  const openVmSettings = () => {
    window.sessionStorage.setItem("openfolks.settings.section", "computer");
  ...
  const emptyState = {
    checking: "Checking…",
    starting: "Starting your folk's computer…",
    unconfigured: "No cloud computer configured",
    "vps-unconfigured": "No managed VPS computer is configured for this folk",
    "vps-incompatible": "This VPS computer belongs to an earlier OpenFolks version",
    "vps-stopped": "The managed VPS computer is stopped",
    "local-unavailable": localDisabledReason ?? "Local computer control isn't ready.",
    "vm-unavailable": "The Local VM isn't available for this folk",
    off: "This folk's computer is off",
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5">
            ...
            <div className="flex flex-col items-center gap-2 px-6 text-center text-ink-secondary">
              ...
              <span className="text-[12px]">
                {phase === "ready"
                  ...
                  : phase === "local"
                    ? isLinux
                      ? "Ready for approved folk actions. Start the separate preview below when you want to watch the screen."
                      : localMisses >= 3
        ...
        {phase === "unconfigured" && (
          <div className="mt-3 rounded-xl bg-card p-4">
            <div className="mb-3 text-[13px] text-ink-secondary">
              Add a Box API key in App Settings → Connections to give this folk a cloud computer.
            </div>
            <button
              onClick={openConnectionSettings}
        ...
        {phase === "vm" &&
          ...
          onOpenVmWorkspace && (
            <button
              ...
              title="Watch two Local VM desktops together without pausing either folk"
            >
              <Columns2 size={14} />
              Open two desktops
            </button>
        ...
        {(phase === "ready" || phase === "vm") && control.held && (
          <div className="mt-3 rounded-xl border border-accent/25 bg-accent/10 p-4">
            <div className="text-[13px] leading-relaxed text-ink">
              You have the wheel — the folk's clicks and keystrokes are refused until you hand it back.
        ...
        {phase === "vm" && vmViewerUrl && control.held && (
          <button
            ...
            title="Open the Local VM's live desktop inside OpenFolks"
          >
        ...
        {phase === "vm" && !control.held && !control.helpReason && (
          <button
            ...
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-control py-2 text-[13px] text-ink hover:bg-raised-hover disabled:opacity-50"
            title="Pause the folk's hands and open the Local VM's live desktop"
          >
            {pending === "join" ? <Loader2 size={14} className="animate-spin" /> : <Hand size={14} />}
            Take control
          </button>
        ...
        {phase === "vm" && vmStatus?.mode === "per-bot" && (
          <button
            ...
            title={bot.busy ? "Stop this folk's turn before deleting its VM" : `Delete ${bot.name}'s Local VM`}
          >
            {pending === "vm-delete" ? <Loader2 size={14} className="animate-spin" /> : <Power size={14} />}
            Delete this folk's VM
          </button>
        )}
        {/* Cloud-only actions */}
        {phase === "ready" && (
          <div className="mt-3 flex gap-2">
            {!control.held && !control.helpReason && (
              <button
                ...
                title="Pause the folk's hands and drive this computer yourself"
              >
                {pending === "join" ? <Loader2 size={14} className="animate-spin" /> : <Hand size={14} />}
                Take control
              </button>
        ...
        <div className="mt-4">
          <ComputerDestinationPicker
            ...
            onSelectCloudBackend={(backend) =>
              dispatch({ type: "updateBot", botId: bot.id, patch: { cloudBackend: backend } })
            }
