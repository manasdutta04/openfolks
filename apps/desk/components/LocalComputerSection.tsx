...
export function LocalComputerSection() {
  ...
  return (
    <>
      <Card
        title="Local VM"
        subtitle={`Isolated Linux desktop on this ${host} (Docker / Podman / WSL). Folks can browse and work here without touching your own desktop.`}
      >
          ...
          >
            ...
            {loading
              ...
              : unavailable
                ? "Status unavailable"
                : perBot && headerReady
                  ? "Ready for per-folk desktops"
                  : perBotRuntimeUnsupported
                    ? "Per-folk mode requires Docker or Podman"
                  : ready
                    ? "Ready"
      ...
      <details className="group rounded-xl bg-card open:pb-4">
        <summary className="cursor-pointer list-none px-4 py-3.5 marker:content-none [&::-webkit-details-marker]:hidden">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[15px] font-medium text-ink">Advanced: isolation</div>
              <div className="mt-0.5 text-[13px] text-ink-secondary">
                Shared (one desktop) or per folk (private containers). Most people can leave this on Shared.
              </div>
            </div>
            <span className="text-[12px] text-ink-secondary group-open:hidden">Show</span>
            <span className="hidden text-[12px] text-ink-secondary group-open:inline">Hide</span>
          </div>
        </summary>
        <div className="space-y-3 px-4">
          <div className="flex overflow-hidden rounded-lg border border-hairline/40">
            {(["shared", "per-bot"] as const).map((mode, index) => (
              ...
              >
                {mode === "shared" ? "Shared" : "Per folk"}
              </button>
            ))}
          </div>
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[13px] text-ink">Maximum per-folk desktops</div>
            ...
            <select
              aria-label="Maximum per-folk desktops"
          ...
          {perBot && (
            <p className="text-[11.5px] leading-relaxed text-ink-secondary">
              Each folk gets its own container and durable workspace. Idle desktops stop after 8 hours.
            </p>
          )}
        </div>
      </details>

      <Card title="Setup" subtitle="Install Docker, Podman, or WSL once; OpenFolks prepares the desktop image for you.">
        <div className="flex flex-col gap-4">
          ...
          <Step
            n={4}
            title={perBot ? "Create a private desktop from each folk's Computer panel" : needsRecreate ? "Replace the older or unsafe VM" : "Create and start the Local VM"}
            done={!perBot && ready}
          >
            {perBot ? (
              <div className="text-[13px] leading-relaxed text-ink-secondary">
                {perBotRuntimeUnsupported
                  ? "Apple container requires an explicit host port, so OpenFolks will not guess or expose one. Install or start Docker or Podman for safe per-folk dynamic loopback ports."
                  : <>
                      Choose <b className="text-ink">Local VM</b> for a folk, open that folk's Computer panel, then create its desktop there. OpenFolks assigns a private workspace and an available loopback viewer port automatically.
                    </>}
              </div>
            ) : needsRecreate ? (
              <>
      ...
      {unavailable && (
        <Card>
          <div className="flex gap-2 text-[13px] text-ink-secondary">
            ...
            <span>OpenFolks could not inspect the container runtime. Re-check, or review the app logs.</span>
          </div>
        </Card>
      )}
