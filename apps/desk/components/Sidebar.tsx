...
import { buildDesk, deskNeedsYouCount } from "@/lib/desk";
...
function groupPreview(group: Group, bots: Bot[]): string {
  if (group.busyBotId) {
    return `${bots.find((b) => b.id === group.busyBotId)?.name ?? "A folk"} is working…`;
  }
...
/** Desk rail glyph — same pane both ways; filled strip = open, empty = collapsed. */
function SidebarRailIcon({ expanded }: { expanded: boolean }) {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true">
      <rect
        ...
        strokeWidth="1.55"
      />
      <path d="M8 3.25v13.5" stroke="currentColor" strokeWidth="1.55" />
      <rect
        x="2.75"
        y="3.25"
        width="5.25"
...
/** Room avatar: 2–3 overlapping mauses in the same 56px slot a folk gets. */
function StackedMauses({ members, density }: { members: Bot[]; density: SidebarDensity }) {
  const iconOnly = density === "icons";
  const slotSize = iconOnly ? "size-12" : "size-14";
  const singleSize = iconOnly ? 44 : 56;
  if (members.length <= 1) {
    const b = members[0];
    return (
  ...
  return (
    <button
      ...
      className={cn(
        ...
        iconOnly ? "justify-center px-1 py-1.5" : "gap-3 px-3 py-2.5",
    ...
    >
      <div className="w-[340px] rounded-2xl border border-hairline/50 bg-card p-4 shadow-2xl">
        ...
        <BotPickerList
          ...
          picked={picked}
          onToggle={toggle}
          emptyHint="Create a folk first — channels are made of folks."
        />
        <button
          onClick={create}
        ...
        >
          Create Channel{picked.size ? ` · ${picked.size} ${picked.size === 1 ? "folk" : "folks"}` : ""}
        </button>
      </div>
    </div>
  );
...
function SectionPicker({
  ...
  // Channels and folks share one namespace, so Work or Personal can hold both.
  const sections = [
    ...new Set([
...
function BotContextMenu({
  ...
  const archiveHint = bot.chiefOfStaff
    ? "Choose another Leader first"
    : visibleBotCount <= 1
      ? "Keep at least one active folk"
      : undefined;
  ...
  return (
    ...
    >
      {[
        ...
        item(
          <Star size={16} className={bot.chiefOfStaff ? "text-accent" : "text-ink-secondary"} />,
          bot.chiefOfStaff ? "Remove Leader" : "Make Leader",
...
function BotListItem({
  ...
  const avatarSize = iconOnly ? 44 : 56;
  const visible = visibleMessages(bot);
  const last = visible.at(-1);
  const rowClass = cn(
    "flex w-full items-center rounded-xl border border-transparent text-left",
    iconOnly ? "justify-center px-1 py-1.5" : "gap-3 px-3 py-2.5",
    selected ? "bg-raised" : "hover:bg-raised/50",
  );
  const body = (
    <>
      <BotAvatar
        ...
        // Motion means something is happening. A resting folk holds a resting
...
function ArchivedBotsPanel({
  ...
  const panelBusy = restoringAll || deletingAll || Boolean(busyId);
  ...
  const restoreAll = async () => {
    ...
    try {
      ...
      onRestored(`${bots.length} ${bots.length === 1 ? "folk" : "folks"} restored`);
      onClose();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : String(cause));
    } finally {
  ...
  const deleteAll = async () => {
    ...
    const confirmed = window.confirm(
      count === 1
        ? `Permanently delete “${bots[0]?.name}”? This cannot be undone.`
        : `Permanently delete ${count} archived folks? Conversations cannot be recovered.`,
    );
    if (!confirmed) return;
    setDeletingAll(true);
    setError("");
    const results = await Promise.allSettled(
      bots.map(async (bot) => {
        ...
        // State-only remove: the DELETE above already happened. Going through
        // dispatch(deleteBot) would fire a second DELETE and 404.
    ...
    if (failures.length === 0) {
      onRestored(`${deleted} ${deleted === 1 ? "folk" : "folks"} deleted`);
      onClose();
    } else {
      const first = failures[0]?.reason;
      const detail = first instanceof Error ? first.message : String(first);
      setError(deleted > 0 ? `Deleted ${deleted}. ${detail}` : detail);
    }
  ...
  return createPortal(
    <div
      ...
      onMouseDown={(event) => event.target === event.currentTarget && !panelBusy && onClose()}
    >
      <div
        ref={dialogRef}
        role="dialog"
        aria-modal="true"
        aria-labelledby="archived-folks-title"
        tabIndex={-1}
      ...
      >
        <header className="flex items-start justify-between gap-4 px-6 pb-4 pt-6 sm:px-8 sm:pt-7">
          <div>
            <h2 id="archived-folks-title" className="text-[22px] font-semibold tracking-[-0.01em] text-ink">Archived folks</h2>
            <p className="mt-1 text-[13px] text-ink-secondary">Conversations are kept until you choose to delete a folk.</p>
          </div>
          <div className="flex items-center gap-1">
            ...
            {bots.length > 0 && (
              <button
                ...
                disabled={panelBusy}
                className="flex items-center gap-1.5 rounded-full bg-raised px-3.5 py-2 text-[12.5px] text-danger hover:bg-danger/10 disabled:opacity-40"
              >
                {deletingAll && <Loader2 size={13} className="animate-spin" />}
                Delete all
              </button>
            )}
            <button
              ...
              aria-label="Close archived folks"
            >
              <X size={21} />
            </button>
          </div>
            ...
            {bots.map((bot) => (
              <div key={bot.id} className="flex min-h-[82px] items-center gap-3 border-b border-hairline/35 px-1 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 truncate text-[12.5px] text-ink-secondary">{bot.title || "Folk"}</div>
                </div>
                <button
...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [density, setDensity] = useState<SidebarDensity>(() => loadSidebarDensity());
  ...
  const toggleCollapsed = () => {
    setSidebarDensity(density === "icons" ? "comfortable" : "icons");
  };
  const iconOnly = density === "icons";
  const deskWaiting = deskNeedsYouCount(buildDesk(state.bots, state.groups));

  // Esc closes the drawer, mirroring ApiKeys.tsx:75-85. Bound only while the
  // drawer is open — on mobile, exactly when a folk/room context menu or the
  ...
  useEffect(() => {
    return window.ogb?.onPackageInstall?.((url) => {
      dispatch({ type: "togglePlugins", open: true, tab: "folks", installUrl: url });
    });
  }, [dispatch]);
  ...
  const exportAllBots = async () => {
    ...
    try {
      ...
      setTeamFeedback({ error: false, text: `${exported.members} folks exported` });
    } catch (cause) {
      setTeamFeedback({
        error: true,
  ...
  const archiveBot = async (bot: Bot) => {
    ...
    if (bot.chiefOfStaff) {
      setTeamFeedback({ error: true, text: "Choose another Leader before archiving this folk" });
      return;
    }
    if (activeBots.length <= 1) {
      setTeamFeedback({ error: true, text: "Keep at least one active folk" });
      return;
    }
    setTeamFeedback(null);
    try {
      ...
      if (!response?.bot) throw new Error("Archive did not return the updated folk");
      // Ensure the sidebar filter sees the archive even if the payload omits a
      // falsey field — wireBot may leave `hidden` unset on older records.
      dispatch({ type: "botPatched", bot: { ...response.bot, hidden: true } });
      ...
      setTeamFeedback({
        error: false,
        text: `${bot.name} archived — open Archived folks to restore`,
  ...
  const matchingBots = state.bots.filter((b) => !b.hidden);
  ...
  const dropSection = (event: React.DragEvent<HTMLDivElement>) => {
    event.preventDefault();
    const from =
      event.dataTransfer.getData("application/x-openfolks-sidebar-section") ||
    ...
    >
      ...
      {(macInset || browser) && (
        <div
          className={cn(
            ...
            iconOnly ? "flex-col gap-1 px-2" : "justify-between px-4",
          )}
          style={windowDragStyle}
        >
          {macInset ? (
            <div className={iconOnly ? "h-5 w-full" : "w-14"} />
          ) : (
      ...
      <div
        className={cn(
          ...
          iconOnly ? "flex-col gap-1 px-2" : "gap-1.5 px-3",
          macInset || browser ? "pt-2" : "pt-3.5",
        )}
      ...
      >
        {!iconOnly && (
          <button
            type="button"
            onClick={() => dispatch({ type: "toggleUniversalSearch", open: true })}
            aria-label="Search OpenFolks"
            title="Search"
            className="flex min-w-0 flex-1 items-center gap-2 rounded-lg bg-raised/70 px-3 py-2 text-left hover:bg-raised"
          >
            <Search size={16} className="shrink-0 text-ink-secondary" />
            <span className="min-w-0 flex-1 truncate text-[14px] text-ink-secondary">Search</span>
            <kbd className="hidden shrink-0 rounded border border-hairline/40 px-1.5 py-0.5 text-[10.5px] text-ink-secondary sm:inline">
        ...
        <button
          ...
          aria-label={iconOnly ? "Expand sidebar" : "Collapse sidebar to avatars"}
          title={iconOnly ? "Expand sidebar" : "Collapse sidebar to avatars"}
          className="flex size-10 shrink-0 items-center justify-center rounded-lg text-ink-secondary hover:bg-raised hover:text-ink"
        >
          <SidebarRailIcon expanded={!iconOnly} />
        </button>
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  {exportingTeam ? "Exporting…" : "Export all folks"}
                </button>
                {archivedBots.length > 0 && (
                  ...
                  >
                    <Archive size={16} className="text-ink-secondary" />
                    <span className="flex-1">Archived folks</span>
      ...
      <div className="flex-1 overflow-y-auto px-2">
        <div className="flex flex-col gap-0.5">
          {matchingBots.length === 0 && visibleGroups.length === 0 && (
            <div className="px-3 py-6 text-center text-[13px] text-ink-secondary">No folks yet</div>
          )}
          {unsectionedChief && (
                ...
                {!iconOnly && (
                  <SidebarSectionHeader
                    ...
                    onDragStart={(event) => {
                      ...
                      event.dataTransfer.setData("application/x-openfolks-sidebar-section", id);
      ...
      {/* Footer */}
      <div className={cn("pb-3 pt-2", iconOnly ? "px-2" : "px-3")}>
        <button
          onClick={() => dispatch({ type: "showDesk" })}
          aria-label={iconOnly ? "Desk" : undefined}
          title={iconOnly ? "Desk — needs you, crew map, handoffs" : undefined}
          className={cn(
        ...
        >
          <span className="relative shrink-0">
            <Inbox size={20} className={state.activeView === "desk" ? "text-accent" : "text-ink-secondary"} />
            {deskWaiting > 0 && (
              <span className="absolute -right-1.5 -top-1.5 min-w-[16px] rounded-full bg-warning px-1 text-center text-[10px] font-semibold leading-[16px] text-canvas">
                {deskWaiting > 9 ? "9+" : deskWaiting}
              </span>
            )}
          </span>
          <span className={cn("flex-1 text-[14px]", iconOnly && "hidden")}>Desk</span>
          {!iconOnly && deskWaiting > 0 && (
            <span className="rounded-full bg-warning/15 px-1.5 py-0.5 text-[11px] font-medium tabular-nums text-warning">
              {deskWaiting}
            </span>
          )}
        </button>
        {skillRecorderEnabled(state.config) && (
          <button
            ...
            aria-label={iconOnly ? "Teach a skill" : undefined}
            title={iconOnly ? "Teach a skill" : undefined}
            className={cn(
              "flex min-h-10 w-full items-center rounded-xl py-2 text-left transition-colors",
              iconOnly ? "justify-center px-2" : "gap-3 px-3",
          ...
          >
            ...
            <span className={cn("flex-1 text-[14px]", iconOnly && "hidden")}>Teach a skill</span>
          </button>
        )}
        <button
          onClick={() => dispatch({ type: "togglePlugins", open: true, tab: "plugins" })}
          aria-label={iconOnly ? "Marketplace" : undefined}
          title={iconOnly ? "Marketplace" : undefined}
          className={cn(
            "flex min-h-10 w-full items-center rounded-xl py-2 text-left hover:bg-raised/50",
            iconOnly ? "justify-center px-2" : "gap-3 px-3",
          )}
        >
          <Store size={20} className="text-ink-secondary" />
          <span className={cn("text-[14px] text-ink", iconOnly && "hidden")}>Marketplace</span>
        </button>
        <button
        ...
        >
          ...
          <span className={cn("text-[14px] text-ink", iconOnly && "hidden")}>Settings</span>
        </button>
      </div>
