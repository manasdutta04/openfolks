...
export function SecretRequestCard({
  ...
  const description = provided
    ? secret.resumed
      ? "Saved securely. Your folk is continuing the task."
      : "Saved securely. Your folk will continue when its current turn settles."
    : declined
      ? "You chose not to provide this credential. OpenFolks could not resume the folk yet."
      : secret.description;
  const footerLabel = declined
    ? "Continuing without this credential failed"
    : secret.resumed
      ? "Folk resumed without seeing the key"
      : error
          ...
          <div className="min-w-0 flex-1">
            ...
            {!provided && !declined && (
              <p className="mt-1 flex items-center gap-1 text-[11.5px] text-ink-secondary/80">
                <LockKeyhole size={11} /> Stored securely by OpenFolks and never added to chat.
              </p>
            )}
