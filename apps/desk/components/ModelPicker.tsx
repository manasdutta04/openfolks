...
import { isCustomOnly, isLlmEngine, splitEngineRail } from "@/lib/engine-rail";
...
export function ModelPicker({
  ...
  const llmInstances = state.instances.filter(isLlmEngine);
  const active = state.instances.find((instance) => instance.instanceId === selection.instanceId);
  const preferredRailId = railId ?? (isLlmEngine(active) ? selection.instanceId : null);
  const railInstance =
    llmInstances.find((instance) => instance.instanceId === preferredRailId) ?? llmInstances[0];
  ...
  const trigger = (
    <button
      type="button"
      onClick={() => {
        const start = isLlmEngine(active) ? active : llmInstances[0];
        setRailId(start?.instanceId ?? null);
        setOpen((wasOpen) => {
          const next = !wasOpen;
          if (next) openFor(start);
          return next;
        });
      }}
      aria-expanded={open}
      ...
      {open && (
        ...
        >
          <div className="flex w-14 shrink-0 flex-col gap-1 overflow-y-auto border-r border-hairline/40 bg-panel p-2">
            {(() => {
              const { subscription, custom: local } = splitEngineRail(llmInstances);
