...
description: Build, verify, and publish the Windows desktop build (NSIS installer + latest.yml) to the openfolks-releases repo. Use when cutting a release, shipping a new version to Windows users, or when a Windows user reports they are stuck on an old version. Windows only — does not cover the macOS dmg/notarization flow.
---

# Windows release

Ships `OpenFolks-<version>-setup.exe` and its update feed to
[manasdutta04/openfolks](http://github.com/manasdutta04/openfolks).
...
| File | Purpose |
|---|---|
| `OpenFolks-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `OpenFolks-<version>-setup.exe.blockmap` | differential updates |
| `OpenFolks-<version>-x64.zip` | portable, not used by the updater |
...
- `app-update.yml` must point at `manasdutta04/openfolks` and, while the
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\OpenFolks\logs\server.log`.
...
```powershell
Copy-Item release/OpenFolks-<version>-setup.exe release/OpenFolks-setup.exe
gh release upload v<version> --repo manasdutta04/openfolks `
  release/OpenFolks-<version>-setup.exe `
  release/OpenFolks-setup.exe `
  release/OpenFolks-<version>-setup.exe.blockmap `
  release/latest.yml
```

Both names are required, for different consumers:

- **`OpenFolks-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`OpenFolks-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/OpenFolks-setup.exe` button a stable URL. This
  mirrors `OpenFolks.dmg` sitting beside `OpenFolks-<version>.dmg`.
