# OpenFolks control plane

Isolated Cloudflare Worker for cloud account identity, installation
ownership, and per-installation companion endpoints. It does **not** store
or move local folks, chats, desktop SQLite state, prompts, or tool output.

## What it covers

- Better Auth 1.7.1 with email OTP, signed bearer sessions, hashed OTP storage,
  and D1-backed IP plus recipient rate limits.
- A Cloudflare Email Sending binding that produces HTML and plain-text OTP
  messages. Authentication responses stay generic even when delivery fails.
  Email addresses, OTPs, secrets, and provider errors are never logged.
- Owner-scoped desktop installations and independently revocable installation
  credentials. Account bearer tokens are never accepted as installation
...
counter used for scheduled retry backoff. Endpoint rows do not cascade away
with a hard installation deletion: losing the tunnel and DNS IDs would make
operator cleanup impossible.
...
`lastSeenAt`. Rotations are serialized with a one-minute cooldown.

### Managed endpoint contract

Endpoint methods require an installation credential bearer token. Account
...
covered by the zone's edge certificate so the endpoint does not depend on
...
response shape before persisting an ID.
...
delay credential revocation. A five-minute cron also processes at most four
expired-lease rows per run. Failed scheduled cleanups back off from five
minutes through 15 minutes, one hour, six hours, and then 24 hours.
...
For local development, copy `.dev.vars.example` to `.dev.vars`, replace
...
```sh
pnpm --filter @openfolks/control-plane exec wrangler d1 migrations apply DB --local --config wrangler.jsonc
pnpm --filter @openfolks/control-plane exec wrangler dev --config wrangler.jsonc
```
...
The checked-in Wrangler file is intentionally non-deployable scaffolding. No
remote resource was created while preparing it. Before a production deployment,
an operator must:

1. Choose and route an HTTPS hostname, then replace `BETTER_AUTH_URL`. The
   Worker has `workers_dev` disabled and no production route in this tree.
2. Generate a strong production `BETTER_AUTH_SECRET` and add it with Wrangler's
   interactive secret command. Add `CLOUDFLARE_API_TOKEN` the same way.
...
4. Complete Cloudflare Email Sending domain onboarding, replace the placeholder
   ...
   deployment identity access to the binding.
5. Create a least-privilege Cloudflare API token scoped to the selected account
   and zone. It needs Cloudflare Tunnel connector **Write** plus DNS **Read**
   and **Write** for that zone. Set `CLOUDFLARE_ACCOUNT_ID`,
   `CLOUDFLARE_ZONE_ID`, and add the token through `wrangler secret put`. Never
   put the token in `vars`, `.dev.vars.example`, logs, or CI output.
6. Set `COMPANION_HOST_SUFFIX` to the certificate-covered DNS suffix where
   opaque `c-*` records may be created.
7. Replace `ALLOWED_ORIGINS` with a comma-separated allow-list of exact HTTPS
   application origins. Wildcards are unsupported.
8. Deploy the Worker and verify that `GET <BETTER_AUTH_URL>/healthz` returns
   exactly `{ "ok": true, "service": "openfolks-control-plane" }` over HTTPS
   before shipping the desktop build.

The control-plane API token is never handed to a desktop. A desktop receives
only its tunnel connector token. The public companion service still enforces
its own pairing and application authentication; the tunnel is transport, not
user authentication. This control plane does not collect marketing consent.
