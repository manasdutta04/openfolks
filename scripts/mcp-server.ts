#!/usr/bin/env node
// Model Context Protocol (MCP) Server for OpenFolks
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid OpenFolks URL: '${url}'`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("OpenFolks URL must use http:// or https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("OpenFolks URL must not contain credentials; use OPENFOLKS_TOKEN instead");
  }
  if ((parsed.pathname !== "/" && parsed.pathname !== "") || parsed.search || parsed.hash) {
    throw new Error("OpenFolks URL must be an origin without a path, query, or fragment");
  }
...
const configuredUrl = process.env.OPENFOLKS_URL || process.env.OPENMAUSBOT_URL ||
...
export function log(msg: string) {
  process.stderr.write(`[openfolks-mcp] ${msg}\n`);
}

export const DEFAULT_REQUEST_TIMEOUT_MS = 30_000;

function requestTimeoutMs(): number {
  const raw = Number(process.env.OPENFOLKS_MCP_TIMEOUT_MS);
...
function requestHeaders(options: RequestInit): NonNullable<RequestInit["headers"]> {
  const token = process.env.OPENFOLKS_TOKEN?.trim();
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`OpenFolks API error (${response.status}): ${text || response.statusText}`);
  }
  try {
    return await response.json();
  } catch {
    throw new Error(`OpenFolks API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  for (const unvalidated of candidates) {
    ...
    try {
      ...
      if (health?.app !== "openfolks") {
        failures.push(`${candidate} answered, but it was not OpenFolks`);
        continue;
      }
      return candidate;
    } catch (error) {
  ...
  throw new Error(`Could not find a running OpenFolks server. ${failures.join("; ")}`);
}

export async function resolveBaseUrl(): Promise<string> {
  if (discoveredBaseUrl) return discoveredBaseUrl;
  if (process.env.OPENFOLKS_TOKEN?.trim() && !configuredUrl) {
    throw new Error("Set OPENFOLKS_URL or OMB_PORT when using OPENFOLKS_TOKEN so credentials are never sent during port discovery");
  }
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the OpenFolks server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid OpenFolks ID`);
  return value;
}
...
export async function handleToolCall(
  ...
  switch (name) {
    case "get_system_health": {
      ...
      if (res?.app !== "openfolks") throw new Error("The configured endpoint is not an OpenFolks server");
      return {
        status: "connected",
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("OpenFolks did not return the created bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("OpenFolks did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("OpenFolks did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("OpenFolks did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("OpenFolks did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
  ...
  try {
    ...
    if (method === "initialize") {
      ...
      return formatResponse(id, {
        ...
        instructions: "Use bounded read tools before mutating the OpenFolks team. Approval grants, deletion, and computer lifecycle are intentionally unavailable.",
      });
    }
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("OpenFolks MCP server running on stdio");
}
