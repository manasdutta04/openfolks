...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "openfolks", `cua-driver-${release.version}`);
  ...
  const response = await fetch(url, { headers: { "user-agent": "OpenFolks-packager" } });
...
await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "openfolks-cua-entry.mjs",
    loader: "js",
  },
  bundle: true,
  platform: "node",
  ...
  format: "esm",
  banner: {
    js: 'import { createRequire as __openfolksCreateRequire } from "node:module"; const require = __openfolksCreateRequire(import.meta.url);',
  },
  outfile: bundle,
  logLevel: "silent",
});
...
await writeFile(
  ...
  bundledSource.replace(
    resolverPattern,
    `${resolvers[0]}\n      if (process.env.OPENFOLKS_CUA_SDK_LIBRARY) return resolveOverride(opts.crateName, process.env.OPENFOLKS_CUA_SDK_LIBRARY);`,
  ),
);
