...
const LOCALES_DIR = join(dirname(SCRIPT_PATH), "..", "apps", "desk", "locales");
...
function translationPrompt(source, label, code) {
  return [
    `Translate this JSON UI catalog for OpenFolks, a multi-agent desktop workbench, into ${label} (${code}).`,
    ...
    "Keep placeholders such as {name} exactly, including duplicates. Keep OpenFolks, CLI, and AI unchanged.",
...
function runModel(prompt) {
  const workDir = mkdtempSync(join(tmpdir(), "openfolks-locale-"));
  try {
...
export function main(argv = process.argv.slice(2)) {
  ...
  writeJsonAtomically(join(LOCALES_DIR, SOURCE_HASH_FILE), state);
  console.error(`wrote ${outFile}; review every changed string and register new locale "${code}" in apps/desk/locales/index.ts`);
}
