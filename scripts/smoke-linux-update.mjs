...
// AppImage, checking the real openfolks-releases feed, downloading the real
...
async function main() {
  ...
  const launched = path.join(installDir, "OpenFolks-0.0.1-x86_64.AppImage");
  copyFileSync(packaged, launched);
  const desktopEntry = path.join(applications, "com.openfolks.app.desktop");
  writeFileSync(
    desktopEntry,
    `[Desktop Entry]\nName=OpenFolks\nExec=${launched} %U\nType=Application\n`,
  );
  ...
  const bundle =
    ...
    path.join(root, "release/linux-unpacked/resources/app.asar/apps/shell/vendor/electron-updater.cjs");
