#!/usr/bin/env node
// Reads the skin blocks out of apps/desk/styles.css and measures every text/surface
...
const css = readFileSync(join(root, "apps/desk/styles.css"), "utf8");
 ...
 * inherited untested by every skin, and the run stays green while a dark
 * skin wears another skin's focus ring. That is exactly what happened when
 * --color-focus was introduced without per-skin overrides. */
...
const PAIRS = [
  ...
  // gives two surfaces the same value (e.g. raised === card), so every chip,
  // hover fill and answered row painted in `raised` on a card would be
  // invisible while this file stayed green. A surface is not text — it only
  // has to be seen at all — so the bar is a just-perceptible step rather
  // than a WCAG ratio.
  //
