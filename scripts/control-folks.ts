...
import { removeTempDir, waitForExit } from "../apps/harness/testing/cleanup.ts";
import { freePortBlock } from "../apps/harness/testing/ports.ts";

const ROOT = join(dirname(fileURLToPath(import.meta.url)), "..");
const FAKE_CLI = join(ROOT, "apps/harness", "testing", "fake-claude-cli.ts");
const MUTATING = new Set(["new-bot", "new-channel", "send", "send-channel", "interrupt"]);

export class ControlFolksError extends Error {
...
export interface ControlFolksDependencies {
  callTool?: ToolCaller;
  request?: Requester;
  env?: NodeJS.ProcessEnv;
}

const HELP = `control-folks — verify a running OpenFolks instance through its shared MCP core
...
mutating (an explicit --url or OPENFOLKS_URL/OPENFOLKS_PORT is required):
...
function parse(
  ...
  try {
    ...
    throw new ControlFolksError(
      error instanceof Error ? error.message : String(error),
      `run control-folks help for the ${command} syntax`,
    );
  }
}

function required(value: unknown, name: string): string {
  if (typeof value !== "string" || !value.trim()) throw new ControlFolksError(`${name} is required`);
  return value.trim();
}
...
function positiveInteger(value: unknown, name: string, fallback: number, maximum: number): number {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > maximum) {
    throw new ControlFolksError(`${name} must be an integer from 1 to ${maximum}`);
  }
  return parsed;
}
...
function configuredUrl(raw: unknown, env: NodeJS.ProcessEnv, requiredForMutation: boolean): string | undefined {
  const explicit = typeof raw === "string" && raw.trim()
    ? raw.trim()
    : env.OPENFOLKS_URL?.trim()
      || (env.OPENFOLKS_PORT ? `http://127.0.0.1:${env.OPENFOLKS_PORT}` : "")
      || env.OPENMAUSBOT_URL?.trim()
      || (env.OMB_PORT ? `http://127.0.0.1:${env.OMB_PORT}` : "");
  if (!explicit) {
    if (requiredForMutation) {
      throw new ControlFolksError(
        "mutating commands require an explicit OpenFolks instance",
        "start `control-folks launch`, then pass its URL with --url",
      );
    }
    return undefined;
  }
...
function target(values: Record<string, unknown>): { type: "bot" | "channel"; id: string } {
  ...
  if (Boolean(bot) === Boolean(channel)) {
    throw new ControlFolksError("provide exactly one of --bot ID or --channel ID");
  }
...
export async function runControlFolks(
  argv: string[],
  dependencies: ControlFolksDependencies = {},
): Promise<unknown> {
  const [command = "help", ...args] = argv;
  if (command === "help" || command === "--help" || command === "-h") return HELP;
  if (command === "launch") throw new ControlFolksError("launch is available only from the executable CLI");
  ...
  if (command === "new-channel") {
    ...
    if (!memberIds.length || new Set(memberIds).size !== memberIds.length) {
      throw new ControlFolksError("--members must contain unique comma-separated bot IDs");
    }
  ...
  if (command === "send" || command === "send-channel") {
    ...
    if (destination.type !== expected) throw new ControlFolksError(`${command} requires --${expected} ID`);
  ...
  throw new ControlFolksError(`unknown command ${JSON.stringify(command)}`, "run control-folks help");
}
...
export async function launchVerificationServer(
  ...
  if (signal?.aborted) throw new ControlFolksError("verification launch cancelled");
  const url = `http://127.0.0.1:${port}`;
  const dataDir = mkdtempSync(join(tmpdir(), "openfolks-verify-data-"));
  ...
  const evidenceDir = join(tmpdir(), "openfolks-verification-evidence");
  ...
  Object.assign(childEnv, {
    ...
    OPENFOLKS_DATA_DIR: dataDir,
    OPENFOLKS_PORT: String(port),
  ...
  const child = spawn(process.execPath, ["--experimental-strip-types", join(ROOT, "apps/harness", "index.ts")], {
    cwd: ROOT,
    env: childEnv,
...
async function main() {
  ...
  if (command === "launch") {
    if (
      process.env.npm_lifecycle_event === "control:folks"
    ) {
      throw new ControlFolksError(
        "launch must own the terminal directly so Ctrl-C can clean up its child",
        "run `node --experimental-strip-types scripts/control-folks.ts launch`",
      );
    }
  ...
  const result = await runControlFolks(process.argv.slice(2));
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    const failure = error instanceof ControlFolksError
