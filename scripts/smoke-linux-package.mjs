...
const executable = path.resolve(
  process.env.OMB_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "openfolks"),
);
...
mkdirSync(path.join(home, ".openfolks"), { recursive: true });
mkdirSync(xdgConfig, { recursive: true });
mkdirSync(xdgRuntime, { recursive: true, mode: 0o700 });
chmodSync(xdgRuntime, 0o700);
writeFileSync(
  path.join(home, ".openfolks", "config.json"),
  JSON.stringify({ instances: { ghost: { driver: "not-a-real-driver", displayName: "Ghost" } } }),
);
for (const appName of ["openfolks", "OpenFolks"]) {
...
const metadata = {
  ...
  pid: process.pid,
  embedded: true,
  host_bundle_id: "com.openfolks.app",
};
...
try {
  ...
  if (health?.app !== "openfolks" || health.static !== true) {
    throw new Error(`unexpected embedded health response: ${JSON.stringify(health)}`);
  }
  if (!String(title).includes("OpenFolks")) throw new Error(`unexpected renderer title: ${title}`);
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["openfolks", "OpenFolks"]
    ...
    const userData = ["openfolks", "OpenFolks"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
