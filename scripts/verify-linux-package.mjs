...
const { validateDriverCandidate } = require("../apps/shell/cua-linux.cjs");
...
function verifyCompliance(licenses, label) {
  ...
  for (const component of registry) {
    const packageId = component.properties?.find(
      (property) => property.name === "openfolks:cargo:package-id",
    )?.value;
...
const executable = path.join(unpacked, "openfolks");
...
try {
  ...
  const debAppRoot = path.join(extracted, "opt", "OpenFolks");
  ...
  for (const expected of [
    "Name=OpenFolks",
    "Exec=/opt/OpenFolks/openfolks %U",
    "Icon=openfolks",
    "StartupWMClass=com.openfolks.app",
    "Categories=Utility;",
  ]) {
