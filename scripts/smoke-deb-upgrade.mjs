...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "openfolks"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing OpenFolks installation");
} catch (error) {
...
const legacyApp = path.join(legacyRoot, "opt", "OpenFolks");
const legacyResources = path.join(legacyApp, "resources");
const legacyDeb = path.join(temporary, "openfolks_0.1.7_amd64.deb");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: OpenFolks CI <ci@openfolks.invalid>",
      "Description: Legacy OpenFolks directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/OpenFolks", "/opt/OpenFolks/resources"]) {
  ...
  for (const directory of [
    "/opt/OpenFolks",
    "/opt/OpenFolks/resources",
    "/opt/OpenFolks/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/OpenFolks/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/OpenFolks/chrome-sandbox";
  ...
  const installedVersion = execFileSync("dpkg-query", ["-W", "-f=${Version}", "openfolks"], {
    encoding: "utf8",
  }).trim();
  console.log(
